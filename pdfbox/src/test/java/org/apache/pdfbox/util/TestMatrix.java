...
public class TestMatrix extends TestCase {
   ...
   */
  public TestMatrix(final String name) throws IOException {
    super(name);
  }
  ...
  public void testMultiplication() throws Exception {
    ...
    TestCase.assertNotSame(m1, product);
    TestCase.assertNotSame(m2, product);
    ...
    // Multiply two matrices together with the result being written to a third
    // matrix
    ...
    TestCase.assertSame(retVal, resultMatrix);
    // Operand 1 should not have changed
    assertMatrixValuesEqualTo(new float[] { 0, 1, 2, 1, 2, 3, 2, 3, 4 }, m1);
    // Operand 2 should not have changed
    assertMatrixValuesEqualTo(new float[] { 0, 1, 2, 1, 2, 3, 2, 3, 4 }, m2);
    ...
    // Multiply two matrices together with the result being written into the other
    // matrix
    retVal = m1.multiply(m2, m2);
    TestCase.assertSame(retVal, m2);
    // Operand 1 should not have changed
    assertMatrixValuesEqualTo(new float[] { 0, 1, 2, 1, 2, 3, 2, 3, 4 }, m1);
    assertMatrixValuesEqualTo(new float[] { 5, 8, 11, 8, 14, 20, 11, 20, 29 }, retVal);

    // Multiply two matrices together with the result being written into 'this'
    // matrix
    m1 = testMatrix.clone();
    m2 = testMatrix.clone();

    retVal = m1.multiply(m2, m1);
    TestCase.assertSame(retVal, m1);
    // Operand 2 should not have changed
    assertMatrixValuesEqualTo(new float[] { 0, 1, 2, 1, 2, 3, 2, 3, 4 }, m2);
    assertMatrixValuesEqualTo(new float[] { 5, 8, 11, 8, 14, 20, 11, 20, 29 }, retVal);

    // Multiply the same matrix with itself with the result being written into
    // 'this' matrix
    m1 = testMatrix.clone();
  ...
  /**
   * This method asserts that the matrix values for the given {@link Matrix}
  ...
  private void assertMatrixValuesEqualTo(final float[] values, final Matrix m) {
    ...
    for (int i = 0; i < values.length; i++) {
      ...
      TestCase.assertEquals(failureMsg.toString(), values[i], m.getValue(row, column), delta);
    }
  }
  ...
  public static void main(final String[] args) {
    final String[] arg = { TestMatrix.class.getName() };
