...
public abstract class TestCOSBase extends TestCase {
  ...
  public void testGetCOSObject() {
    TestCase.assertEquals(testCOSBase, testCOSBase.getCOSObject());
  }
  ...
  public void testIsSetDirect() {
    ...
    TestCase.assertTrue(testCOSBase.isDirect());
    testCOSBase.setDirect(false);
    TestCase.assertFalse(testCOSBase.isDirect());
  }
  ...
  protected void testByteArrays(final byte[] byteArr1, final byte[] byteArr2) {
    TestCase.assertEquals(byteArr1.length, byteArr1.length);
    for (int i = 0; i < byteArr1.length; i++) {
      TestCase.assertEquals(byteArr1[i], byteArr2[i]);
    }
  }
}
