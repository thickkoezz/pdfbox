 ...
 * FILE SET VALIDATION
 *
 * This test suite is designed to test PDFTextStripper using a set of PDF files
 * and known good output for each. The default mode of testAll() is to process
 * each *.pdf file in "src/test/resources/input". An output file is created in
 * "target/test-output" with the same name as the PDF file, plus an additional
 * ".txt" suffix.
 *
 ...
 * the additional ".txt" suffix). The process is performed both with and without
 * sorting enabled. The sorted files have a "-sorted.txt" suffix.
 *
 * So for the file "src/test/resources/input/hello.pdf", an output file will be
 * generated named "target/test-output/hello.pdf.txt". Then that file will be
 * compared to the known good file "src/test/resources/input/hello.pdf.txt", if
 * it exists.
 *
 * To support testing with files that are not officially distributed with
 * PDFBox, this test will also look in the "target/test-input-ext" directory.
 *
 * Any errors are logged, and at the end of processing all *.pdf files, if there
 * were any errors, the test fails. The logging is at INFO, as the general goal
 * is overall validation, and on failure, the indication of which file or files
 * failed.
 *
 ...
 *
 * To further research individual failures, the
 * org.apache.pdfbox.util.TextStripper.file system property may be set with the
 * name of a single file in the "test/input" directory. In this mode, testAll()
 * will evaluate only that file, and will do so with DEBUG level logging.
 *
...
public class TestTextStripper extends TestCase {
   ...
   */
  public TestTextStripper(final String name) throws IOException {
    super(name);
  ...
  public void setUp() {
    ...
    //
    // System.setProperty("org.apache.pdfbox.util.TextStripper.file",
    // "FVS318Ref.pdf");
  }
   ...
   * @return <code>true</code> is the strings are both null, or if their contents
    ...
    if (expected == null && actual == null)
      ...
      while (expectedIndex < expectedArray.length && actualIndex < actualArray.length) {
        if (expectedArray[expectedIndex] != actualArray[actualIndex]) {
          equals = false;
          TestTextStripper.log.warn("Lines differ at index" + " expected:" + expectedIndex + "-"
      ...
      if (equals) {
        if (expectedIndex != expectedArray.length) {
          equals = false;
          TestTextStripper.log.warn("Expected line is longer at:" + expectedIndex);
        }
        if (actualIndex != actualArray.length) {
          equals = false;
          TestTextStripper.log.warn("Actual line is longer at:" + actualIndex);
        }
      }
    } else {
   ...
   */
  public void doTestFile(final File inFile, final File outDir, final boolean bLogResult, final boolean bSort)
      throws Exception {
    if (bSort) {
      TestTextStripper.log.info("Preparing to parse " + inFile.getName() + " for sorted test");
    } else {
      TestTextStripper.log.info("Preparing to parse " + inFile.getName() + " for standard test");
    }
    ...
    try (PDDocument document = PDDocument.load(inFile)) {
      ...
      try (OutputStream os = new FileOutputStream(outFile)) {
        ...
        try (Writer writer = new BufferedWriter(new OutputStreamWriter(os, TestTextStripper.ENCODING))) {
      ...
      if (bLogResult) {
        TestTextStripper.log.info("Text for " + inFile.getName() + ":");
        TestTextStripper.log.info(stripper.getText(document));
      }

      if (!expectedFile.exists()) {
        bFail = true;
        TestTextStripper.log
            .error("FAILURE: Input verification file: " + expectedFile.getAbsolutePath() + " did not exist");
        return;
      }
      ...
      try (
          LineNumberReader expectedReader = new LineNumberReader(
              new InputStreamReader(new FileInputStream(expectedFile), TestTextStripper.ENCODING));
          LineNumberReader actualReader = new LineNumberReader(
              new InputStreamReader(new FileInputStream(outFile), TestTextStripper.ENCODING))) {
        while (true) {
          ...
          if (!stringsEqual(expectedLine, actualLine)) {
            bFail = true;
            localFail = true;
            TestTextStripper.log.error("FAILURE: Line mismatch for file " + inFile.getName() + " (sort = " + bSort + ")"
      ...
      if (!localFail) {
        ...
        final List<String> original = TestTextStripper.fileToLines(expectedFile);
        final List<String> revised = TestTextStripper.fileToLines(outFile);

        // Compute diff. Get the Patch object. Patch is the container for computed
        // deltas.
        final Patch patch = DiffUtils.diff(original, revised);

        try (PrintStream diffPS = new PrintStream(diffFile, TestTextStripper.ENCODING)) {
          for (final Object delta : patch.getDeltas()) {
  ...
  private static List<String> fileToLines(final File file) {
    final List<String> lines = new LinkedList<>();
    String line;
    try {
      try (BufferedReader in = new BufferedReader(
          new InputStreamReader(new FileInputStream(file), TestTextStripper.ENCODING))) {
  ...
  private int findOutlineItemDestPageNum(final PDDocument doc, final PDOutlineItem oi) throws IOException {
    ...
    TestCase.assertEquals(indexOfPage, pageNum);

    return pageNum;
  }

  /**
   * Test whether stripping controlled by outline items works properly. The test
   * file has 4 outline items at the top level, that point to 0-based pages 0, 2,
   * 3 and 4. We are testing text stripping by outlines pointing to 0-based pages
   * 2 and 3, and also text stripping of the 0-based page 2. The test makes sure
   * that the output is different to a complete strip, not empty, different to
   * each other when different bookmark intervals are used, but identical from
   * bookmark intervals to strips with page intervals. When fed with orphan
   * bookmarks, stripping must be empty.
   *
   * @throws IOException
   * @throws URISyntaxException
   */
  public void testStripByOutlineItems() throws IOException, URISyntaxException {
    final PDDocument doc = PDDocument.load(new File(TestPDPageTree.class.getResource("with_outline.pdf").toURI()));
    ...
    TestCase.assertEquals(0, findOutlineItemDestPageNum(doc, oi0));
    TestCase.assertEquals(2, findOutlineItemDestPageNum(doc, oi2));
    TestCase.assertEquals(3, findOutlineItemDestPageNum(doc, oi3));
    TestCase.assertEquals(4, findOutlineItemDestPageNum(doc, oi4));

    final String textFull = stripper.getText(doc);
    TestCase.assertFalse(textFull.isEmpty());

    final String expectedTextFull = "First level 1\n" + "First level 2\n" + "Fist level 3\n" + "Some content\n"
        + "Some other content\n" + "Second at level 1\n" + "Second level 2\n" + "Content\n" + "Third level 1\n"
        + "Third level 2\n" + "Third level 3\n" + "Content\n" + "Fourth level 1\n" + "Content\n" + "Content\n";
    TestCase.assertEquals(expectedTextFull, textFull.replaceAll("\r", ""));
    ...
    TestCase.assertFalse(textoi23.isEmpty());
    TestCase.assertFalse(textoi23.equals(textFull));

    final String expectedTextoi23 = "Second at level 1\n" + "Second level 2\n" + "Content\n" + "Third level 1\n"
        + "Third level 2\n" + "Third level 3\n" + "Content\n";
    TestCase.assertEquals(expectedTextoi23, textoi23.replaceAll("\r", ""));
    ...
    TestCase.assertFalse(textp34.isEmpty());
    TestCase.assertFalse(textoi23.equals(textFull));
    TestCase.assertTrue(textoi23.equals(textp34));
    ...
    TestCase.assertFalse(textoi2.isEmpty());
    TestCase.assertFalse(textoi2.equals(textoi23));
    TestCase.assertFalse(textoi23.equals(textFull));

    final String expectedTextoi2 = "Second at level 1\n" + "Second level 2\n" + "Content\n";
    TestCase.assertEquals(expectedTextoi2, textoi2.replaceAll("\r", ""));
    ...
    TestCase.assertFalse(textp3.isEmpty());
    TestCase.assertFalse(textp3.equals(textp34));
    TestCase.assertFalse(textoi23.equals(textFull));
    TestCase.assertTrue(textoi2.equals(textp3));
    ...
    TestCase.assertTrue(textOiOrphan.isEmpty());
  }
  ...
  private void doTestDir(final File inDir, final File outDir) throws Exception {
    final File[] testFiles = inDir.listFiles((FilenameFilter) (dir, name) -> name.endsWith(".pdf"));
  ...
  public void testExtract() throws Exception {
    ...
    if (bFail) {
      TestCase.fail("One or more failures, see test log for details");
    }
  }
  ...
  public static void main(final String[] args) {
    final String[] arg = { TestTextStripper.class.getName() };
