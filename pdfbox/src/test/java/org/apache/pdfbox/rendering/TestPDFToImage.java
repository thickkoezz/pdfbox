 ...
 * FILE SET VALIDATION
 *
 * This test is designed to test PDFToImage using a set of PDF files and known
 * good output for each. The default mode is to process all *.pdf and *.ai files
 * in "src/test/resources/input/rendering". An output file is created in
 * "target/test-output/rendering" with the same name as the PDF file, plus an
...
public class TestPDFToImage {
  ...
  public static Collection<Object[]> data() {
    final File[] testFiles = new File(TestPDFToImage.inDir).listFiles(
        (FilenameFilter) (dir, name) -> (name.toLowerCase().endsWith(".pdf") || name.toLowerCase().endsWith(".ai")));
  ...
  public void testRenderImage() throws IOException {
    new File(TestPDFToImage.outDir).mkdirs();

    if (!doTestFile(new File(TestPDFToImage.inDir, filename), TestPDFToImage.inDir, TestPDFToImage.outDir)) {
      Assert.fail("failure, see test log for details");
    }
  }

  /**
   * Create an image; the part between the smaller and the larger image is painted
   * black, the rest in white
   *
   ...
   * @return
   */
  private BufferedImage createEmptyDiffImage(final int minWidth, final int minHeight, final int maxWidth,
      final int maxHeight) {
   ...
   * differences are xored, the highest bit of each color is reset to avoid colors
   * that are too light
   *
   * @param bim1
   * @param bim2
   * @return If the images are different, the function returns a diff image If the
   *         images are identical, the function returns null If the size is
    ...
    for (int x = 0; x < minWidth; ++x) {
      for (int y = 0; y < minHeight; ++y) {
        ...
        if (rgb1 != rgb2
            ...
            && (Math.abs((rgb1 & 0xFF) - (rgb2 & 0xFF)) > 3 || Math.abs((rgb1 >> 8 & 0xFF) - (rgb2 >> 8 & 0xFF)) > 3
   ...
   * @return false if the test failed (not identical or other problem), true if
   *         the test succeeded (all identical)
   * @throws IOException when there is an exception
   */
  public boolean doTestFile(final File file, final String inDir, final String outDir) throws IOException {
    ...
    try {
      ...
      final File[] outFiles = new File(outDir).listFiles((FilenameFilter) (dir, name) -> name.endsWith(".png")
          && name.startsWith(file.getName(), 0) && !name.endsWith(".png-diff.png"));
      ...
      for (final File outFile : outFiles) {
        ...
        if (!inFile.exists()) {
          ...
          if (bim3 != null) {
            failed = true;
            TestPDFToImage.LOG
                .warn("*** TEST FAILURE *** Input and output not identical for file: " + inFile.getName());
            ImageIO.write(bim3, "png", new File(outFile.getAbsolutePath() + "-diff.png"));
            System.err.println(
                "Files differ: " + inFile.getAbsolutePath() + "\n" + "              " + outFile.getAbsolutePath());
          } else {
  ...
  private boolean filesAreIdentical(final File left, final File right) throws IOException {
    ...
    /*
     * -- I reworked ASSERT's into IF statement -- dwilson assert left != null;
     * assert right != null; assert left.exists(); assert right.exists();
     */
