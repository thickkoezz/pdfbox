...
public class TestFontEmbedding extends TestCase {
  ...
  protected void setUp() {
    TestFontEmbedding.OUT_DIR.mkdirs();
  }
  ...
  public void testCIDFontType2VerticalSubsetMonospace() throws IOException {
    ...
    final File pdf = new File(TestFontEmbedding.OUT_DIR, "CIDFontType2VM.pdf");
    ...
    try (PDDocument document = new PDDocument()) {
      ...
      TestCase.assertEquals(7392, cid); // it's 441 without substitution
      // Check the dictionaries
      final COSDictionary fontDict = vfont.getCOSObject();
      TestCase.assertEquals(COSName.IDENTITY_V, fontDict.getDictionaryObject(COSName.ENCODING));
      ...
      TestCase.assertNull(dw2); // This font uses default values for DW2
      final COSArray w2 = (COSArray) descFontDict.getDictionaryObject(COSName.W2);
      TestCase.assertEquals(0, w2.size()); // Monospaced font has no entries
    }

    // Check text extraction
    final String extracted = getUnicodeText(pdf);
    TestCase.assertEquals(expectedExtractedtext, extracted.replaceAll("\r", "").trim());
  }
  ...
  public void testCIDFontType2VerticalSubsetProportional() throws IOException {
    final String text = "「ABC」";
    final String expectedExtractedtext = "「\nA\nB\nC\n」";
    final File pdf = new File(TestFontEmbedding.OUT_DIR, "CIDFontType2VP.pdf");
    ...
    try (PDDocument document = new PDDocument()) {
      ...
      final byte[] encode = vfont.encode(text);
      final int cid = ((encode[0] & 0xFF) << 8) + (encode[1] & 0xFF);
      TestCase.assertEquals(12607, cid); // it's 12461 without substitution
      // Check the dictionaries
      final COSDictionary fontDict = vfont.getCOSObject();
      TestCase.assertEquals(COSName.IDENTITY_V, fontDict.getDictionaryObject(COSName.ENCODING));
      ...
      final COSArray w2 = (COSArray) descFontDict.getDictionaryObject(COSName.W2);
      TestCase.assertEquals(2, w2.size());
      TestCase.assertEquals(12607, w2.getInt(0)); // Start CID
      final COSArray metrics = (COSArray) w2.getObject(1);
      int i = 0;
      for (final int n : new int[] { -570, 500, 450, -570, 500, 880 }) {
        TestCase.assertEquals(n, metrics.getInt(i++));
      }
    }
  ...
  public void testBengali() throws IOException {
    ...
    final File pdf = new File(TestFontEmbedding.OUT_DIR, "Bengali.pdf");
    ...
    if (!testPDFToImage.doTestFile(pdf, IN_DIR.getAbsolutePath(), TestFontEmbedding.OUT_DIR.getAbsolutePath())) {
      // don't fail, rendering is different on different systems, result must be
      // viewed manually
      System.err.println(
          "Rendering of " + pdf + " failed or is not identical to expected rendering in " + IN_DIR + " directory");
    }
  ...
  public void testMaxEntries() throws IOException {
    File file;
    String text;
    text = "あいうえおかきくけこさしすせそたちつてとなにぬねのはひふへほまみむめもやゆよらりるれろわをん" + "アイウエオカキクケコサシスセソタチツテトナニヌネノハヒフヘホマミムメモヤユヨラリルレロワヲン"
        + "１２３４５６７８";
    ...
    TestCase.assertEquals(ToUnicodeWriter.MAX_ENTRIES_PER_OPERATOR, set.size());
    ...
    try (PDDocument document = new PDDocument()) {
      ...
      file = new File(TestFontEmbedding.OUT_DIR, "PDFBOX-4302-test.pdf");
      document.save(file);
    }

    // check that the extracted text matches what we wrote
    final String extracted = getUnicodeText(file);
    TestCase.assertEquals(text, extracted.trim());
  }
  ...
  private void validateCIDFontType2(final boolean useSubset) throws IOException {
    ...
    try (PDDocument document = new PDDocument()) {
      ...
      file = new File(TestFontEmbedding.OUT_DIR, "CIDFontType2.pdf");
      document.save(file);
    }
