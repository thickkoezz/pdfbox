...
public class TestPDFunctionType4 extends TestCase {
  ...
  private PDFunctionType4 createFunction(final String function, final float[] domain, final float[] range)
    ...
    try (OutputStream out = stream.createOutputStream()) {
      final byte[] data = function.getBytes("US-ASCII");
  ...
  public void testFunctionSimple() throws Exception {
    ...
    final PDFunctionType4 function = createFunction(functionText, new float[] { -1.0f, 1.0f, -1.0f, 1.0f },
        new float[] { -1.0f, 1.0f });

    float[] input = new float[] { 0.8f, 0.1f };
    float[] output = function.eval(input);

    TestCase.assertEquals(1, output.length);
    TestCase.assertEquals(0.9f, output[0], 0.0001f);

    input = new float[] { 0.8f, 0.3f }; // results in 1.1f being outside Range
    output = function.eval(input);

    TestCase.assertEquals(1, output.length);
    TestCase.assertEquals(1f, output[0]);
  ...
  public void testFunctionArgumentOrder() throws Exception {
    ...
    TestCase.assertEquals(1, output.length);
    TestCase.assertEquals(-0.7f, output[0], 0.0001f);
  }
