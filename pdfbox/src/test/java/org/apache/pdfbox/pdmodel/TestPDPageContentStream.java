...
public class TestPDPageContentStream extends TestCase {
  public void testSetCmykColors() throws IOException {
    try (PDDocument doc = new PDDocument()) {
      ...
      // [3] = COSFloat{0.4}
      // [4] = PDFOperator{"k"}
      TestCase.assertEquals(0.1f, ((COSFloat) pageTokens.get(0)).floatValue());
      TestCase.assertEquals(0.2f, ((COSFloat) pageTokens.get(1)).floatValue());
      TestCase.assertEquals(0.3f, ((COSFloat) pageTokens.get(2)).floatValue());
      TestCase.assertEquals(0.4f, ((COSFloat) pageTokens.get(3)).floatValue());
      TestCase.assertEquals("k", ((Operator) pageTokens.get(4)).getName());
      ...
      // [3] = COSFloat{0.8}
      // [4] = PDFOperator{"K"}
      TestCase.assertEquals(0.5f, ((COSFloat) pageTokens.get(0)).floatValue());
      TestCase.assertEquals(0.6f, ((COSFloat) pageTokens.get(1)).floatValue());
      TestCase.assertEquals(0.7f, ((COSFloat) pageTokens.get(2)).floatValue());
      TestCase.assertEquals(0.8f, ((COSFloat) pageTokens.get(3)).floatValue());
      TestCase.assertEquals("K", ((Operator) pageTokens.get(4)).getName());
    }
  }
  ...
  public void testMissingContentStream() throws IOException {
    ...
    final PDFStreamParser parser = new PDFStreamParser(page.getContents());
    parser.parse();
    final List<Object> tokens = parser.getTokens();
    TestCase.assertEquals(0, tokens.size());
  }
