...
public class PDFontTest {
  ...
  public void testPDFBox988() throws IOException, URISyntaxException {
    try (PDDocument doc = PDDocument.load(new File(PDFontTest.class.getResource("F001u_3_7j.pdf").toURI()))) {
  ...
  /**
   * PDFBOX-3747: Test that using "-" with Calibri in Windows 7 has "-" in text
   * extraction and not \u2010, which was because of a wrong ToUnicode mapping
   * because prior to the bugfix, CmapSubtable#getCharCodes provided values in
   * random order.
   *
   * @throws IOException
   */
  ...
  /**
   * PDFBOX-3826: Test ability to reuse a TrueTypeFont created from a file or a
   * stream for several PDFs to avoid parsing it over and over again. Also check
   * that full or partial embedding is done, and do render and text extraction.
   *
   * @throws IOException
   * @throws URISyntaxException
   */
  ...
  public void testPDFBox3826() throws IOException, URISyntaxException {
    final URL url = PDFont.class.getResource("/org/apache/pdfbox/resources/ttf/LiberationSans-Regular.ttf");
  ...
  /**
   * PDFBOX-4115: Test ability to create PDF with german umlaut glyphs with a type
   * 1 font. Test for everything that went wrong before this was fixed.
   *
   * @throws IOException
   */
  @Test
  public void testPDFBOX4115() throws IOException {
    ...
    final File outputFile = new File(PDFontTest.OUT_DIR, "FontType1.pdf");
