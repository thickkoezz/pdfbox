...
public class AlignmentTest {
  ...
  public void setUp() throws IOException {
    document = PDDocument.load(new File(AlignmentTest.IN_DIR, AlignmentTest.NAME_OF_PDF));
    acroForm = document.getDocumentCatalog().getAcroForm();
    AlignmentTest.OUT_DIR.mkdirs();
  }

  @Test
  public void fillFields() throws IOException {
    PDTextField field = (PDTextField) acroForm.getField("AlignLeft");
    field.setValue(AlignmentTest.TEST_VALUE);
    ...
    // compare rendering
    final File file = new File(AlignmentTest.OUT_DIR, AlignmentTest.NAME_OF_PDF);
    document.save(file);
    final TestPDFToImage testPDFToImage = new TestPDFToImage(TestPDFToImage.class.getName());
    if (!testPDFToImage.doTestFile(file, AlignmentTest.IN_DIR.getAbsolutePath(),
        AlignmentTest.OUT_DIR.getAbsolutePath())) {
      ...
      System.err.println("Rendering of " + file + " failed or is not identical to expected rendering in "
          + AlignmentTest.IN_DIR + " directory");
    }
  }
