...
public class FDFAnnotationTest {
  ...
  public void loadXFDFAnnotations() throws IOException, URISyntaxException {
    ...
    try (FDFDocument fdfDoc = FDFDocument.loadXFDF(f)) {
      ...
      Assert.assertEquals(18, fdfAnnots.size());

      // test PDFBOX-4345 and PDFBOX-3646
      // before the fix, the richtext output was
      // <body style="font:12pt Helvetica; color:#D66C00;"
      // xfa:APIVersion="Acrobat:7.0.8" xfa:spec="2.0.2"
      // xmlns="http://www.w3.org/1999/xhtml"
      // xmlns:xfa="http://www.xfa.org/schema/xfa-data/1.0/"><p dir="ltr"><span
      // style="text-decoration:word;font-family:Helvetica">P&2</span></p></body>
      ...
      for (final FDFAnnotation ann : fdfAnnots) {
        if (ann instanceof FDFAnnotationFreeText) {
          ...
          if ("P&1 P&2 P&3".equals(annotationFreeText.getContents())) {
            ...
            Assert.assertEquals(
                "<body style=\"font:12pt Helvetica; " + "color:#D66C00;\" xfa:APIVersion=\"Acrobat:7.0.8\" "
                ...
                annotationFreeText.getRichContents().trim());
          }
        }
      }
