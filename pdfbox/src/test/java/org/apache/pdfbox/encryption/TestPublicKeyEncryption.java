...
/**
 * Tests for public key encryption. These tests are not perfect - to be sure,
 * encrypt a file by using a certificate exported from your digital id in Adobe
 * Reader, and then open that file with Adobe Reader. Do this with every key
 * length.
 *
...
public class TestPublicKeyEncryption {
  ...
  public void setUp() throws Exception {
    if (Cipher.getMaxAllowedKeyLength("AES") != Integer.MAX_VALUE) {
      ...
      Assert.fail("JCE unlimited strength jurisdiction policy files are not installed");
    }
  ...
  private PDDocument reload(final File file, final String decryptionPassword, final InputStream keyStore)
    ...
    final PDDocument doc2 = PDDocument.load(file, decryptionPassword, keyStore, null,
  ...
  /**
   * Returns a recipient specification with the given access permissions and an
   * X.509 certificate read from the given classpath resource.
   *
   ...
   */
  private PublicKeyRecipient getRecipient(final String certificate, final AccessPermission permission)
      throws Exception {
