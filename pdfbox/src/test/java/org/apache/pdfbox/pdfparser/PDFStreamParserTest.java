...
public class PDFStreamParserTest extends TestCase {
  ...
  /**
   * Tests for inline images, whether the EI is correctly identified as ending. To
   * test hasNoFollowingBinData(), the amount of data after EI nust be at least
   * PDFStreamParser.MAX_BIN_CHAR_TEST_LENGTH
   *
   * @throws IOException
   */
  ...
  // operator
  private void testInlineImage2ops(final String s, final String imageDataString, final String opName)
      throws IOException {
    final List<Object> tokens = parseTokenString(s);

    TestCase.assertEquals(2, tokens.size());

    TestCase.assertEquals(OperatorName.BEGIN_INLINE_IMAGE_DATA, ((Operator) tokens.get(0)).getName());
    TestCase.assertEquals(imageDataString.length(), ((Operator) tokens.get(0)).getImageData().length);
    Assert.assertArrayEquals(imageDataString.getBytes(), ((Operator) tokens.get(0)).getImageData());

    TestCase.assertEquals(opName, ((Operator) tokens.get(1)).getName());
  }
  ...
  private void testInlineImage1op(final String s, final String imageDataString) throws IOException {
    ...
    TestCase.assertEquals(1, tokens.size());
