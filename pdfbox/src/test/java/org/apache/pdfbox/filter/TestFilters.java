...
public class TestFilters extends TestCase {
  /**
   * This will test all of the filters in the system. There will be COUNT of
   * deterministic tests and COUNT of non-deterministic tests, see also the
    ...
    for (int iter = 0; iter < COUNT * 2; iter++) {
      ...
      try {
        ...
        for (final Filter filter : FilterFactory.INSTANCE.getAllFilters()) {
          ...
          if (filter instanceof DCTFilter || filter instanceof CCITTFaxFilter || filter instanceof JPXFilter
              || filter instanceof JBIG2Filter || filter instanceof RunLengthDecodeFilter) {
            continue;
          }
  ...
  /**
   * This will test the use of identity filter to decode stream and string. This
   * test threw an IOException before the correction.
   * 
   * @throws IOException
   */
  ...
  public void testPDFBOX1777() throws IOException {
    ...
    final byte[] byteArray = IOUtils.toByteArray(this.getClass().getResourceAsStream("PDFBOX-1777.bin"));
  ...
  private void checkEncodeDecode(final Filter filter, final byte[] original) throws IOException {
    ...
    filter.decode(new ByteArrayInputStream(encoded.toByteArray()), decoded, new COSDictionary(), 0);

    TestCase.assertTrue(
        "Data that is encoded and then decoded through " + filter.getClass() + " does not match the original data",
