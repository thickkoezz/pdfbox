...
public class TestRandomAccessBuffer extends TestCase {
  ...
  /**
   * This test checks two corner cases where the last read ends exactly at the end
   * of a chunck (remainingBytes == 0)
   * 
   * @throws IOException
   */
  public void testRemainingByteZero() throws IOException {
    RandomAccessBuffer buffer = new RandomAccessBuffer();
    byte[] byteArray = new byte[TestRandomAccessBuffer.CHUNK_SIZE + 2];
    // fill the second chunk with "1"
    for (int i = 0; i < 2; i++) {
      byteArray[TestRandomAccessBuffer.CHUNK_SIZE + i] = 1;
    }
    buffer.write(byteArray);
    buffer.seek(TestRandomAccessBuffer.CHUNK_SIZE - 2);
    ...
    TestCase.assertEquals(2, byteArray[0] + byteArray[1]);
    buffer.close();

    buffer = new RandomAccessBuffer();
    byteArray = new byte[2 * TestRandomAccessBuffer.CHUNK_SIZE + 2];
    // fill the second chunk with "1"
    for (int i = 0; i < TestRandomAccessBuffer.CHUNK_SIZE; i++) {
      byteArray[TestRandomAccessBuffer.CHUNK_SIZE + i] = 1;
    }
    // fill the third chunk with "2"
    for (int i = 0; i < 2; i++) {
      byteArray[2 * TestRandomAccessBuffer.CHUNK_SIZE + i] = 2;
    }
    buffer.write(byteArray);
    buffer.seek(700);
    final byte[] bytesRead = new byte[1348];
    buffer.read(bytesRead, 0, bytesRead.length);
    TestCase.assertEquals(2, buffer.read());
    buffer.close();
  }

  /**
   * Test the {@link RandomAccessBuffer#read()} and
   * {@link RandomAccessBuffer#write(int)} method.
   * 
   * @throws IOException is thrown if something went wrong.
   */
  public void testSimpleReadWrite() throws IOException {
    // create a buffer filled with 10 figures from 0 to 9
    final RandomAccessBuffer buffer = new RandomAccessBuffer();
    for (int i = 0; i < 10; i++) {
      buffer.write(i);
    }
    ...
    TestCase.assertEquals(45, result);
    buffer.close();
  }

  /**
   * Test the {@link RandomAccessBuffer#read(byte[], int, int)} and
   * {@link RandomAccessBuffer#write(byte[])} method.
   * 
   * @throws IOException is thrown if something went wrong.
   */
  ...
  /**
   * Test the {@link RandomAccessBuffer#read(byte[], int, int)} and
   * {@link RandomAccessBuffer#write(byte[])} method using a couple of data to
  ...
  public void testArrayReadWrite() throws IOException {
    ...
    byte[] byteArray = new byte[2 * TestRandomAccessBuffer.CHUNK_SIZE + 100];
    for (int i = TestRandomAccessBuffer.CHUNK_SIZE; i < 2 * TestRandomAccessBuffer.CHUNK_SIZE; i++) {
      byteArray[i] = 1;
    }
    for (int i = 2 * TestRandomAccessBuffer.CHUNK_SIZE; i < 2 * TestRandomAccessBuffer.CHUNK_SIZE + 100; i++) {
      byteArray[i] = 2;
    }
    ...
    TestCase.assertEquals(0, buffer.read());

    // jump to the last byte of the first chunk, it should be "0"
    buffer.seek(TestRandomAccessBuffer.CHUNK_SIZE - 1);
    TestCase.assertEquals(0, buffer.read());

    // jump to the first byte of the second chunk, it should be "1"
    buffer.seek(TestRandomAccessBuffer.CHUNK_SIZE);
    TestCase.assertEquals(1, buffer.read());

    // jump to the end-5 of the first chunk
    buffer.seek(TestRandomAccessBuffer.CHUNK_SIZE - 5);
    ...
    buffer.seek(2 * TestRandomAccessBuffer.CHUNK_SIZE - 5);
  ...
  public void testOverwrite() throws IOException {
    ...
    byte[] byteArray = new byte[TestRandomAccessBuffer.CHUNK_SIZE + 100];
    final RandomAccessBuffer buffer = new RandomAccessBuffer();
    for (int i = TestRandomAccessBuffer.CHUNK_SIZE; i < TestRandomAccessBuffer.CHUNK_SIZE + 100; i++) {
      byteArray[i] = 1;
    }
    buffer.write(byteArray);
    ...
    TestCase.assertEquals(25, result);
    buffer.close();
  }
  ...
  public void testSeekBeyondEOF() throws IOException {
    ...
    TestCase.assertEquals(-1, buffer.read());
    // check EOF
    TestCase.assertTrue(buffer.isEOF());
    buffer.close();
  }

  public void testPDFBOX1490() throws Exception {
    // create a buffer filled with 1024 * "0"
    final byte[] byteArray = new byte[TestRandomAccessBuffer.CHUNK_SIZE - 1];
    final RandomAccessBuffer buffer = new RandomAccessBuffer();
    buffer.write(byteArray);
  ...
  public void testPDFBOX2969() throws Exception {
    ...
    final int chunkSize = (TestRandomAccessBuffer.CHUNK_SIZE << 4) + 3;
    final byte[] byteArray = new byte[chunkSize];

    final RandomAccessBuffer buffer = new RandomAccessBuffer(byteArray);
    ...
    // create clone
    final RandomAccessBuffer bufferClone = buffer.clone();

    // read all from both
    buffer.seek(0);
    final int bufRead = buffer.read(new byte[(int) buffer.length()]);

    bufferClone.seek(0);
    final int bufCloneRead = bufferClone.read(new byte[(int) bufferClone.length()]);

    TestCase.assertEquals(bufRead, bufCloneRead);
