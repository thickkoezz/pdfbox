...
public abstract class PDFStreamEngine {
  ...
  public void showForm(final PDFormXObject form) throws IOException {
    if (currentPage == null)
      throw new IllegalStateException(
          "No current page, call " + "#processChildStream(PDContentStream, PDPage) instead");
  ...
  protected void processTransparencyGroup(final PDTransparencyGroup group) throws IOException {
    ...
    final Stack<PDGraphicsState> savedStack = saveGraphicsStack();

    final Matrix parentMatrix = initialMatrix;

    // the stream's initial matrix includes the parent CTM, e.g. this allows a
    // scaled form
    ...
    // the current stroking and nonstroking alpha constants to 1.0, and the current
    // soft mask to None.
   ...
   * @throws IOException if there is an error reading or parsing the character
   *                     content stream.
   */
   ...
   * @throws IOException If there is an error reading or parsing the appearance
   *                     content stream.
   */
  protected void processAnnotation(final PDAnnotation annotation, final PDAppearanceStream appearance)
    ...
    if (rect != null && rect.getWidth() > 0 && rect.getHeight() > 0 && bbox != null) {
      ...
      // filled fields / annotations that have a matrix in the appearance stream, see
      // PDFBOX-3083
   ...
   * @param color         color to use, if this is an uncoloured pattern,
   *                      otherwise null.
   * @param colorSpace    color space to use, if this is an uncoloured pattern,
   *                      otherwise null.
   * @throws IOException if there is an error reading or parsing the tiling
  ...
  /**
   * Process the given tiling pattern. Allows the pattern matrix to be overridden
   ...
   * @param colorSpace    color space to use, if this is an uncoloured pattern,
   *                      otherwise null.
   * @param patternMatrix the pattern matrix, may be overridden for custom
   *                      rendering.
   * @throws IOException if there is an error reading or parsing the tiling
   *                     pattern content stream.
   */
  protected final void processTilingPattern(final PDTilingPattern tilingPattern, PDColor color,
      final PDColorSpace colorSpace, final Matrix patternMatrix) throws IOException {
    ...
    final PDRectangle rect = new PDRectangle((float) bbox.getX(), (float) bbox.getY(), (float) bbox.getWidth(),
  ...
  /**
   * Returns the appearance stream to process for the given annotation. May be
   * used to render a specific appearance such as "hover".
   *
  ...
  /**
   * Process a child stream of the given page. Cannot be used with
   * {@link #processPage(PDPage)}.
   *
   ...
   * @throws IOException if there is an error reading or parsing the content
   *                     stream.
   */
  private void processStreamOperators(final PDContentStream contentStream) throws IOException {
    ...
    try (InputStream is = contentStream.getContents()) {
      final PDFStreamParser parser = new PDFStreamParser(is);
  ...
  private PDResources pushResources(final PDContentStream contentStream) {
    // resource lookup: first look for stream resources, then fallback to the
    // current page
    ...
    if (streamResources != null) {
      ...
      // inherit directly from parent stream, this is not in the PDF spec, but the
      // file from
  ...
  /**
   * Transforms the given rectangle using the CTM and then intersects it with the
  ...
  /**
   * Called when the BT operator is encountered. This method is for overriding in
  ...
  /**
   * Called when the ET operator is encountered. This method is for overriding in
   * subclasses, the default implementation does nothing.
   *
   * @throws IOException if there was an error processing the text
   */
  ...
  /**
   * Applies a text position adjustment from the TJ operator. May be overridden in
   * subclasses.
   *
   * @param tx x-translation
   * @param ty y-translation
   */
  protected void applyTextAdjustment(final float tx, final float ty) throws IOException {
    // update the text matrix
    textMatrix.concatenate(Matrix.getTranslateInstance(tx, ty));
  }

  /**
   * Process text from the PDF Stream. You should override this method if you want
   * to perform an action when encoded text is being processed.
   *
   * @param string the encoded text
  ...
  protected void showText(final byte[] string) throws IOException {
    ...
    if (font == null) {
      PDFStreamEngine.LOG.warn("No current font, will use default");
    ...
    final Matrix parameters = new Matrix(fontSize * horizontalScaling, 0, // 0
        0, fontSize, // 0
    ...
    while (in.available() > 0) {
      ...
      final String unicode = font.toUnicode(code);
  ...
  /**
   * Called when a glyph is to be processed.This method is intended for overriding
   ...
   * @param unicode             the Unicode text for this glyph, or null if the
   *                            PDF does provide it
   * @param displacement        the displacement (i.e. advance) of the glyph in
   *                            text space
   * @throws IOException if the glyph cannot be processed
   */
  protected void showGlyph(final Matrix textRenderingMatrix, final PDFont font, final int code, final String unicode,
  ...
  protected void operatorException(final Operator operator, final List<COSBase> operands, final IOException e)
    ...
    if (e instanceof MissingOperandException || e instanceof MissingResourceException
      ...
      PDFStreamEngine.LOG.error(e.getMessage());
    } else if (e instanceof EmptyGraphicsStackException) {
      PDFStreamEngine.LOG.warn(e.getMessage());
  ...
  protected final Stack<PDGraphicsState> saveGraphicsStack() {
    final Stack<PDGraphicsState> savedStack = graphicsStack;
   ...
   */
  protected final void restoreGraphicsStack(final Stack<PDGraphicsState> snapshot) {
  ...
  public void setLineDashPattern(final COSArray array, int phase) {
    if (phase < 0) {
      PDFStreamEngine.LOG.warn("Dash phase has negative value " + phase + ", set to 0");
      phase = 0;
    }
  ...
  /**
   * @return the stream' resources. This is mainly to be used by the
   *         {@link OperatorProcessor} classes.
   */
