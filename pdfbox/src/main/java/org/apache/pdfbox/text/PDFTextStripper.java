...
public class PDFTextStripper extends LegacyPDFStreamEngine {
  ...
  static {
    ...
    try {
      ...
      PDFTextStripper.LOG.debug("Couldn't read system properties - using defaults", e);
    }
    if (strIndent != null && strIndent.length() > 0) {
      try {
        PDFTextStripper.defaultIndentThreshold = Float.parseFloat(strIndent);
    ...
    if (strDrop != null && strDrop.length() > 0) {
      try {
        PDFTextStripper.defaultDropThreshold = Float.parseFloat(strDrop);
      } catch (final NumberFormatException nfe) {
        // ignore and use default
      }
    }
  ...
  private float indentThreshold = PDFTextStripper.defaultIndentThreshold;
  private float dropThreshold = PDFTextStripper.defaultDropThreshold;
  ...
  protected void processPages(final PDPageTree pages) throws IOException {
    final PDPage startBookmarkPage = startBookmark == null ? null : startBookmark.findDestinationPage(document);
    ...
    final PDPage endBookmarkPage = endBookmark == null ? null : endBookmark.findDestinationPage(document);
  ...
  protected void writePage() throws IOException {
    float maxYForLine = PDFTextStripper.MAX_Y_FOR_LINE_RESET_VALUE;
    float minYTopForLine = PDFTextStripper.MIN_Y_TOP_FOR_LINE_RESET_VALUE;
    float endOfLastTextX = PDFTextStripper.END_OF_LAST_TEXT_X_RESET_VALUE;
    float lastWordSpacing = PDFTextStripper.LAST_WORD_SPACING_RESET_VALUE;
    float maxHeightForLine = PDFTextStripper.MAX_HEIGHT_FOR_LINE_RESET_VALUE;
    ...
    for (final List<TextPosition> textList : charactersByArticle) {
      ...
      while (textIter.hasNext()) {
        ...
        float expectedStartOfNextWordX = PDFTextStripper.EXPECTED_START_OF_NEXT_WORD_X_RESET_VALUE;
        if (Float.compare(endOfLastTextX, PDFTextStripper.END_OF_LAST_TEXT_X_RESET_VALUE) != 0) {
        ...
        if (lastPosition != null) {
          ...
          if (!overlap(positionY, positionHeight, maxYForLine, maxHeightForLine)) {
            ...
            lastLineStartPosition = handleLineSeparation(current, lastPosition, lastLineStartPosition,
                maxHeightForLine);
            expectedStartOfNextWordX = PDFTextStripper.EXPECTED_START_OF_NEXT_WORD_X_RESET_VALUE;
            maxYForLine = PDFTextStripper.MAX_Y_FOR_LINE_RESET_VALUE;
            maxHeightForLine = PDFTextStripper.MAX_HEIGHT_FOR_LINE_RESET_VALUE;
            minYTopForLine = PDFTextStripper.MIN_Y_TOP_FOR_LINE_RESET_VALUE;
          }
          // test if our TextPosition starts after a new word would be expected to start
          if (Float.compare(expectedStartOfNextWordX, PDFTextStripper.EXPECTED_START_OF_NEXT_WORD_X_RESET_VALUE) != 0
   ...
   */
  private PositionWrapper handleLineSeparation(final PositionWrapper current, final PositionWrapper lastPosition,
      PositionWrapper lastLineStartPosition, final float maxHeightForLine) throws IOException {
  ...
  private void isParagraphSeparation(final PositionWrapper position, final PositionWrapper lastPosition,
    ...
    if (lastLineStartPosition == null) {
      ...
      final float newXVal = multiplyFloat(getIndentThreshold(), position.getTextPosition().getWidthOfSpace());
  ...
  private Pattern matchListItemPattern(final PositionWrapper pw) {
    ...
    return PDFTextStripper.matchPattern(txt, getListItemPatterns());
  }
   ...
   * @throws IOException if something went wrong
   */
  private void writeLine(final List<WordWithTextPositions> line) throws IOException {
      ...
      if ((level & 1) != 0) {
        while (--end >= start) {
          ...
          if (Character.isMirrored(word.codePointAt(end))) {
            if (PDFTextStripper.MIRRORING_CHAR_MAP.containsKey(character)) {
              result.append(PDFTextStripper.MIRRORING_CHAR_MAP.get(character));
            } else {
  ...
  static {
    ...
    try (InputStream input = PDFTextStripper.class.getResourceAsStream(path)) {
      if (input != null) {
        ...
        PDFTextStripper.LOG.warn("Could not find '" + path + "', mirroring char map will be empty: ");
      }

    } catch (final IOException e) {
      PDFTextStripper.LOG.warn("Could not parse BidiMirroring.txt, mirroring char map will be empty: " + e.getMessage(),
          e);
    }
  }
  ...
  private static void parseBidiFile(final InputStream inputStream) throws IOException {
    ...
    do {
      ...
      if (fields.length == 2) {
        ...
        PDFTextStripper.MIRRORING_CHAR_MAP.put(fields[0], fields[1]);
      }
    } while (true);
  }
   ...
   */
  private StringBuilder normalizeAdd(final List<WordWithTextPositions> normalized, StringBuilder lineBuilder,
      final List<TextPosition> wordPositions, final LineItem item) {
