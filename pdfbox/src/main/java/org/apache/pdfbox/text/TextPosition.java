...
public final class TextPosition {
  ...
  // text matrix for the start of the text object, coordinates are in display
  // units
   ...
   * @param textMatrix      text rendering matrix for start of text (in display
   *                        units)
   ...
   * @param individualWidth The width of the given character/string. (in text
   *                        units)
   ...
   * @param charCodes       An array of the internal PDF character codes for the
   *                        glyphs in this text.
   * @param font            The current font for this text position.
   * @param fontSize        The new font size.
   * @param fontSizeInPt    The font size in pt units (see
   *                        {@link #getFontSizeInPt()} for details).
   */
  public TextPosition(final int pageRotation, final float pageWidth, final float pageHeight, final Matrix textMatrix,
      final float endX, final float endY, final float maxHeight, final float individualWidth, final float spaceWidth,
      final String unicode, final int[] charCodes, final PDFont font, final float fontSize, final int fontSizeInPt) {
    ...
    widths = new float[] { individualWidth };
  ...
  // mapped in the form
  // NFKC normalization method. Determined by going through the Combining
  // Diacritical Marks
  // section of the Unicode spec and identifying which characters are not mapped
  // to by the
  // normalization.
  ...
  /**
   * Return the string of characters stored in this object. The length can be
   * different than the CharacterCodes length e.g. if ligatures are used ("fi",
   * "fl", "ffl") where one glyph represents several unicode characters.
   *
  ...
  /**
   * The matrix containing the starting text position and scaling. Despite the
   * name, it is not the text matrix set by the "Tm" operator, it is really the
   * effective text rendering matrix (which is dependent on the current
   * transformation matrix (set by the "cm" operator), the text matrix (set by the
   * "Tm" operator), the font size (set by the "Tf" operator) and the page
   * cropbox).
   *
  ...
  /**
   * Return the direction/orientation of the string in this object based on its
   * text matrix.
   *
  ...
  /**
   * Return the X starting coordinate of the text, adjusted by the given rotation
   * amount. The rotation adjusts where the 0,0 location is relative to the text.
   *
   * @param rotation Rotation to apply (0, 90, 180, or 270). 0 will perform no
   *                 adjustments.
   * @return X coordinate
   */
  ...
  /**
   * This will get the page rotation adjusted x position of the character. This is
   * adjusted based on page rotation so that the upper left is 0,0.
   *
  ...
  /**
   * This will get the text direction adjusted x position of the character. This
   * is adjusted based on text direction so that the first character in that
   * direction is in the upper left at 0,0.
   *
  ...
  /**
   * This will get the y position of the character with 0,0 in lower left. This
   * will be adjusted by the given rotation.
   *
   * @param rotation Rotation to apply to text to adjust the 0,0 location
   *                 (0,90,180,270)
   ...
   * and it is adjusted based on the page rotation.
   *
  ...
  /**
   * This will get the y position of the text, adjusted so that 0,0 is upper left
   * and it is adjusted based on the text direction.
   *
   * @return The adjusted y coordinate of the character.
   */
  ...
  /**
   * This will get the width of the string when page rotation adjusted coordinates
   * are used.
   *
  ...
  /**
   * This will get the width of the string when text direction adjusted
  ...
  /**
   * This will get the font size that has been set with the "Tf" operator (Set
   * text font and size). When the text is rendered, it may appear bigger or
   * smaller depending on the current transformation matrix (set by the "cm"
  ...
  /**
   * This will get the font size in pt. To get this size we have to multiply the
   * font size from {@link #getFontSize() getFontSize()} with the text matrix (set
   * by the "Tm" operator) horizontal scaling factor and truncate the result to
   * integer. The actual rendering may appear bigger or smaller depending on the
   * current transformation matrix (set by the "cm" operator). To get the size in
   * rendering, use {@link #getXScale() getXScale()}.
   *
   ...
   * algorithms such as the text stripper, that need to know the width of a space
   * character.
   *
  ...
  /**
   * This will get the X scaling factor. This is dependent on the current
   * transformation matrix (set by the "cm" operator), the text matrix (set by the
   * "Tm" operator) and the font size (set by the "Tf" operator).
   *
  ...
  /**
   * This will get the Y scaling factor. This is dependent on the current
   * transformation matrix (set by the "cm" operator), the text matrix (set by the
   * "Tm" operator) and the font size (set by the "Tf" operator).
   *
  ...
  /**
   * Determine if this TextPosition logically contains another (i.e. they overlap
   * and should be rendered on top of each other).
   *
  ...
  public boolean contains(final TextPosition tp2) {
    ...
    // no Y overlap at all so return as soon as possible. Note: 0.0 is in the upper
    // left and
  ...
  /**
   * Merge a single character TextPosition into the current object. This is to be
   * used only for cases where we have a diacritic that overlaps an existing
   * TextPosition. In a graphical display, we could overlay them, but for text
   * extraction we need to merge them. Use the contains() method to test if two
   * objects overlap.
   *
  ...
  public void mergeDiacritic(final TextPosition diacritic) {
    ...
    for (int i = 0; i < strLen && !wasAdded; i++) {
      if (i >= widths.length) {
        TextPosition.LOG.info("diacritic " + diacritic.getUnicode() + " on ligature " + unicode
      ...
      // the
      // current character and the previous character. If no previous character, just
      // append
      ...
      // diacritic completely covers this character and therefore we assume that this
      // is the
   ...
   * updates the widths array to include the extra character width.
   *
  ...
  private void insertDiacritic(final int i, final TextPosition diacritic) {
    ...
    // Unicode combining diacritics always go after the base character, regardless
    // of whether
  ...
  /**
   * Combine the diacritic, for example, convert non-combining diacritic
   * characters to their combining counterparts.
   *
  ...
  private String combineDiacritic(final String str) {
    ...
    if (TextPosition.DIACRITICS.containsKey(codePoint))
      return TextPosition.DIACRITICS.get(codePoint);
    else
  ...
  /**
   * This will get the x coordinate of the end position. This is the unadjusted
  ...
  /**
   * This will get the y coordinate of the end position. This is the unadjusted
   * value passed into the constructor.
   *
  ...
  /**
   * This will get the rotation of the page that the text is located in. This is
   * the unadjusted value passed into the constructor.
   *
  ...
  /**
   * This will get the height of the page that the text is located in. This is the
   * unadjusted value passed into the constructor.
   *
  ...
  /**
   * This will get the width of the page that the text is located in. This is the
   * unadjusted value passed into the constructor.
   *
