...
/**
 * LEGACY text calculations which are known to be incorrect but are depended on
 * by PDFTextStripper.
 *
 * This class exists only so that we don't break the code of users who have
 * their own subclasses of PDFTextStripper. It replaces the good implementation
 * of showGlyph in PDFStreamEngine, with a bad implementation which is backwards
 * compatible.
 *
 * DO NOT USE THIS CODE UNLESS YOU ARE WORKING WITH PDFTextStripper. THIS CODE
 * IS DELIBERATELY INCORRECT, USE PDFStreamEngine INSTEAD.
 */
class LegacyPDFStreamEngine extends PDFStreamEngine {
   ...
   */
  @Override
  protected void showGlyph(final Matrix textRenderingMatrix, final PDFont font, final int code, String unicode,
    ...
    if (fontDescriptor != null) {
      ...
      if (ascent > 0 && descent < 0 && ((ascent - descent) / 2 < glyphHeight || Float.compare(glyphHeight, 0) == 0)) {
    ...
    final float dyDisplay = height * textRenderingMatrix.getScalingFactorY();

    //
    // start of the original method
    //

    // Note on variable names. There are three different units being used in this
    // code.
    // Character sizes are given in glyph units, text locations are initially given
    // in text
    // units, and we want to save the data in display units. The variable names
    // should end with
    // Text or Disp to represent if the values are in text or disp units (no glyph
    // units are
    // saved).
    ...
    try {
      // to avoid crash as described in PDFBOX-614, see what the space displacement
      // should be
      spaceWidthText = font.getSpaceWidth() * glyphSpaceToTextSpaceFactor;
    } catch (final Exception exception) {
      LegacyPDFStreamEngine.LOG.warn(exception, exception);
    }

    if (Float.compare(spaceWidthText, 0) == 0) {
      spaceWidthText = font.getAverageFontWidth() * glyphSpaceToTextSpaceFactor;
      // the average space width appears to be higher than necessary so make it
      // smaller
      spaceWidthText *= .80f;
    }
    if (Float.compare(spaceWidthText, 0) == 0) {
