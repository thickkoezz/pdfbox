...
public abstract class Filter {
  ...
  /**
   * Compression Level System Property. Set this to a value from 0 to 9 to change
   * the zlib deflate compression level used to compress /Flate streams. The
   * default value is -1 which is {@link Deflater#DEFAULT_COMPRESSION}. To set
   * maximum compression, use
   ...
   */
  public abstract DecodeResult decode(InputStream encoded, OutputStream decoded, COSDictionary parameters, int index)
      throws IOException;

  /**
   * Decodes data, with optional DecodeOptions. Not all filters support all
   * options, and so callers should check the options' <code>honored</code> flag
   * to test if they were applied.
   *
   * @param encoded    the encoded byte stream
   * @param decoded    the stream where decoded data will be written
   * @param parameters the parameters used for decoding
  ...
  protected COSDictionary getDecodeParams(final COSDictionary dictionary, final int index) {
    ...
    if (filter instanceof COSName && obj instanceof COSDictionary)
      // PDFBOX-3932: The PDF specification requires "If there is only one filter and
      // that
      ...
      Filter.LOG.error("Expected DecodeParams to be an Array or Dictionary but found " + obj.getClass().getName());
    }
   ...
   */
  protected static ImageReader findImageReader(final String formatName, final String errorCause)
  ...
  public static int getCompressionLevel() {
    ...
    try {
      ...
      Filter.LOG.warn(ex.getMessage(), ex);
    }
