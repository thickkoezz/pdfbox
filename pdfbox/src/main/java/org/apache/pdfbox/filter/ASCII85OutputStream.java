...
final class ASCII85OutputStream extends FilterOutputStream {
  ...
  /**
   * Function produces five ASCII printing characters from four bytes of binary
   * data.
   */
  private int maxline;
  ...
  public void setTerminator(final char term) {
    if (term < 118 || term > 126 || term == ASCII85OutputStream.Z)
  ...
  private void transformASCII85() {
    ...
    if (word == 0) {
      outdata[0] = (byte) ASCII85OutputStream.Z;
      outdata[1] = 0;
      return;
    }
    long x;
    x = word / (85L * 85L * 85L * 85L);
    outdata[0] = (byte) (x + ASCII85OutputStream.OFFSET);
    word -= x * 85L * 85L * 85L * 85L;

    x = word / (85L * 85L * 85L);
    outdata[1] = (byte) (x + ASCII85OutputStream.OFFSET);
    word -= x * 85L * 85L * 85L;

    x = word / (85L * 85L);
    outdata[2] = (byte) (x + ASCII85OutputStream.OFFSET);
    word -= x * 85L * 85L;

    x = word / 85L;
    outdata[3] = (byte) (x + ASCII85OutputStream.OFFSET);

    outdata[4] = (byte) (word % 85L + ASCII85OutputStream.OFFSET);
  }
  ...
  public void write(final int b) throws IOException {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      out.write(outdata[i]);
      if (--lineBreak == 0) {
        out.write(ASCII85OutputStream.NEWLINE);
        lineBreak = maxline;
      }
    }
    count = 0;
  ...
  public void flush() throws IOException {
    ...
    if (count > 0) {
      ...
      transformASCII85();
      if (outdata[0] == ASCII85OutputStream.Z) {
        for (int i = 0; i < 5; i++) // expand 'z',
        {
          outdata[i] = (byte) ASCII85OutputStream.OFFSET;
        }
      }
