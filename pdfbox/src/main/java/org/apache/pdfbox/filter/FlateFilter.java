...
final class FlateFilter extends Filter {
  ...
  // Use Inflater instead of InflateInputStream to avoid an EOFException due to a
  // probably
      ...
      while (true) {
        ...
        try {
          ...
          if (dataWritten) {
            ...
            FlateFilter.LOG.warn("FlateFilter: premature end of stream due to a DataFormatException");
            break;
          } else
  ...
  protected void encode(final InputStream input, final OutputStream encoded, final COSDictionary parameters)
      throws IOException {
    final int compressionLevel = Filter.getCompressionLevel();
    final Deflater deflater = new Deflater(compressionLevel);
    try (DeflaterOutputStream out = new DeflaterOutputStream(encoded, deflater)) {
      int amountRead;
      final int mayRead = input.available();
      if (mayRead > 0) {
        final byte[] buffer = new byte[Math.min(mayRead, FlateFilter.BUFFER_SIZE)];
        while ((amountRead = input.read(buffer, 0, Math.min(mayRead, FlateFilter.BUFFER_SIZE))) != -1) {
