...
/**
 * Options that may be passed to a Filter to request special handling when
 * decoding the stream. Filters may not honor some or all of the specified
 * options, and so callers should check the honored flag if further processing
 * relies on the options being used.
 */
public class DecodeOptions {
  /**
   * Default decode options. The honored flag for this instance is always true, as
   * it represents the default behavior.
   */
  ...
  /**
   * Constructs an instance specifying the region of the image that should be
   * decoded. The actual region will be clipped to the dimensions of the image.
   *
  ...
  /**
   * Constructs an instance specifying the image should be decoded using
   * subsampling. The subsampling will be the same for the X and Y axes.
   *
   * @param subsampling The number of rows and columns to advance in the source
   *                    for each pixel in the decoded image.
   */
  ...
  /**
   * When decoding an image, the part of the image that should be decoded, or null
   * if the entire image is needed.
   *
   * @return The source region to decode, or null if the entire image should be
   *         decoded
   */
  ...
  /**
   * Sets the region of the source image that should be decoded. The region will
   * be clipped to the dimensions of the source image. Setting this value to null
   * will result in the entire image being decoded.
   *
   * @param sourceRegion The source region to decode, or null if the entire image
   *                     should be decoded.
   */
  ...
  /**
   * When decoding an image, the number of columns to advance in the source for
   * every pixel decoded.
   *
  ...
  /**
   * When decoding an image, the number of rows to advance in the source for every
   * pixel decoded.
   *
  ...
  public void setSubsamplingOffsetY(final int ssOffsetY) {
    subsamplingOffsetY = ssOffsetY;
  }

  /**
   * Flag used by the filter to specify if it performed subsampling.
   *
   * Some filters may be unable or unwilling to apply subsampling, and so the
   * caller must check this flag <b>after</b> decoding.
   *
   * @return True if the filter applied the options specified by this instance,
   *         false otherwise.
   */
  ...
  /**
   * Used internally by filters to signal they have applied subsampling as
   * requested by this options instance.
   *
   * @param filterSubsampled Value specifying if the filter could meet the
   *                         requested options. Usually a filter will only call
   *                         this with the value <code>true</code>, as the default
   *                         value for the flag is <code>false</code>.
   */
