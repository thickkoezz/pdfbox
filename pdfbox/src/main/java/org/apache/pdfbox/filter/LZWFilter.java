...
public class LZWFilter extends Filter {
  ...
  private void doLZWDecode(final InputStream encoded, final OutputStream decoded, final int earlyChange)
      throws IOException {
    ...
    try {
      while ((nextCommand = in.readBits(chunk)) != LZWFilter.EOD) {
        if (nextCommand == LZWFilter.CLEAR_TABLE) {
          chunk = 9;
      ...
      LZWFilter.LOG.warn("Premature EOF in LZW stream, EOD code missing", ex);
    }
    decoded.flush();
  }
  ...
  private void checkIndexBounds(final List<byte[]> codeTable, final long index, final MemoryCacheImageInputStream in)
    ...
    if (index >= codeTable.size())
      throw new IOException(
          "array index overflow: " + index + " >= " + codeTable.size() + " near offset " + in.getStreamPosition());
  }
  ...
  protected void encode(final InputStream rawData, final OutputStream encoded, final COSDictionary parameters)
    ...
    try (MemoryCacheImageOutputStream out = new MemoryCacheImageOutputStream(encoded)) {
      out.writeBits(LZWFilter.CLEAR_TABLE, chunk);
      int foundCode = -1;
      int r;
      ...
      out.writeBits(LZWFilter.EOD, chunk);
