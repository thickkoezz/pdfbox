...
/**
 * Helper class to contain predictor decoding used by Flate and LZW filter. To
 * see the history, look at the FlateFilter class.
 */
public final class Predictor {
   ...
   * @param bitsPerComponent Number of bits per components, from decode
   *                         parameters.
   * @param columns          Number samples in a row, from decode parameters.
   * @param actline          Current (active) line to decode. Data will be decoded
   *                         in-place, i.e. - the contents of this buffer will be
   *                         modified.
   * @param lastline         The previous decoded line. When decoding the first
   *                         line, this parameter should be an empty byte array of
   *                         the same length as <code>actline</code>.
   */
  static void decodePredictorRow(final int predictor, final int colors, final int bitsPerComponent, final int columns,
      final byte[] actline, final byte[] lastline) {
    if (predictor == 1)
      // no prediction
      return;
    ...
    case 2:
      ...
      if (bitsPerComponent == 16) {
        for (int p = bytesPerPixel; p < rowlength; p += 2) {
          ...
          final int left = ((actline[p - bytesPerPixel] & 0xff) << 8) + (actline[p - bytesPerPixel + 1] & 0xff);
      ...
      for (int p = colors; p < elements; ++p) {
        ...
        final int sub = Predictor.getBitSeq(actline[bytePosSub], bitPosSub, bitsPerComponent);
        final int left = Predictor.getBitSeq(actline[bytePosLeft], bitPosLeft, bitsPerComponent);
        actline[bytePosSub] = (byte) Predictor.calcSetBitSeq(actline[bytePosSub], bitPosSub, bitsPerComponent,
            sub + left);
      }
      break;
    case 10:
  ...
  static void decodePredictor(final int predictor, final int colors, final int bitsPerComponent, final int columns,
    ...
    if (predictor == 1) {
      ...
      // calculate sizes
      final int rowlength = Predictor.calculateRowLength(colors, bitsPerComponent, columns);
      ...
      while (in.available() > 0) {
        // test for PNG predictor; each value >= 10 (not only 15) indicates usage of PNG
        // predictor
        if (predictor >= 10) {
        ...
        Predictor.decodePredictorRow(linepredictor, colors, bitsPerComponent, columns, actline, lastline);
  ...
  /**
   * Wraps and <code>OutputStream</code> in a predictor decoding stream as
   * necessary. If no predictor is specified by the parameters, the original
   * stream is returned as is.
   *
   * @param out          The stream to which decoded data should be written
   * @param decodeParams Decode parameters for the stream
   * @return An <code>OutputStream</code> is returned, which will write decoded
   *         data into the given stream. If no predictor is specified, the
   *         original stream is returned.
   */
  ...
  /**
   * Output stream that implements predictor decoding. Data is buffered until a
   * complete row is available, which is then decoded and written to the
   * underlying stream. The previous row is retained for decoding the next row.
   */
  private static final class PredictorOutputStream extends FilterOutputStream {
    ...
    PredictorOutputStream(final OutputStream out, final int predictor, final int colors, final int bitsPerComponent,
        final int columns) {
      super(out);
      ...
      rowLength = Predictor.calculateRowLength(colors, bitsPerComponent, columns);
    ...
    private void decodeAndWriteRow() throws IOException {
      Predictor.decodePredictorRow(predictor, colors, bitsPerComponent, columns, currentRow, lastRow);
