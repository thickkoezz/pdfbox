 ...
 *
 * Requires the Java Advanced Imaging (JAI) Image I/O Tools to be installed from
 * java.net, see <a href=
 * "http://download.java.net/media/jai-imageio/builds/release/1.1/">jai-imageio</a>.
 ...
 * obtain the required jar files. The .so file can be safely ignored.
 *
 * @author John Hewson
 * @author Timo Boehme
 */
public final class JPXFilter extends Filter {
  ...
  private BufferedImage readJPX(final InputStream input, final DecodeOptions options, final DecodeResult result)
      throws IOException {
    final ImageReader reader = Filter.findImageReader("JPEG2000",
    ...
    try (ImageInputStream iis = new MemoryCacheImageInputStream(input)) {
      ...
      // "Decode shall be ignored, except in the case where the image is treated as a
      // mask"
      ...
      if (!parameters.containsKey(COSName.COLORSPACE)) {
        if (image.getSampleModel() instanceof MultiPixelPackedSampleModel && image.getColorModel().getPixelSize() == 1
            && image.getRaster().getNumBands() == 1 && image.getColorModel() instanceof IndexColorModel) {
          // PDFBOX-4326:
