...
final class DCTFilter extends Filter {
  ...
  public DecodeResult decode(final InputStream encoded, final OutputStream decoded, final COSDictionary parameters,
    ...
    final ImageReader reader = Filter.findImageReader("JPEG", "a suitable JAI I/O image filter is not installed");
    try (ImageInputStream iis = ImageIO.createImageInputStream(encoded)) {
      ...
      if ("3".equals(numChannels) || numChannels.isEmpty()) {
        try {
          // I'd like to use ImageReader#readRaster but it is buggy and can't read RGB
          // correctly
          ...
          DCTFilter.LOG.debug("Couldn't read use read() for RGB image - using readRaster() as fallback", e);
      ...
      if (raster.getNumBands() == 4) {
        ...
        try {
          ...
          DCTFilter.LOG
              .debug("Couldn't read usíng getAdobeTransform() - using getAdobeTransformByBruteForce() as fallback", e);
  ...
  // com.twelvemonkeys.imageio.plugins.jpeg.AdobeDCT class for structure of APP14
  // segment
  private int getAdobeTransformByBruteForce(final ImageInputStream iis) throws IOException {
    ...
    int by;
    while ((by = iis.read()) != -1) {
      if (DCTFilter.ADOBE.charAt(a) == by) {
        ++a;
        if (a != DCTFilter.ADOBE.length()) {
          continue;
        }
        // match
        a = 0;
        ...
        if (len >= DCTFilter.POS_TRANSFORM + 1) {
          final byte[] app14 = new byte[Math.max(len, DCTFilter.POS_TRANSFORM + 1)];
          if (iis.read(app14) >= DCTFilter.POS_TRANSFORM + 1)
            return app14[DCTFilter.POS_TRANSFORM];
        }
      } else {
        a = 0;
      }
  ...
  // returns the number of channels as a string, or an empty string if there is an
  // error getting the meta data
  private String getNumChannels(final ImageReader reader) {
    try {
      ...
      DCTFilter.LOG.debug("Couldn't read metadata - returning empty string", e);
      return "";
    }
  }
