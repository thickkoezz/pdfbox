...
final class CCITTFaxFilter extends Filter {
  ...
  public DecodeResult decode(final InputStream encoded, final OutputStream decoded, final COSDictionary parameters,
    ...
    if (rows > 0 && height > 0) {
      // PDFBOX-771, PDFBOX-3727: rows in DecodeParms sometimes contains an incorrect
      // value
      rows = height;
    } else {
  ...
  void readFromDecoderStream(final CCITTFaxDecoderStream decoderStream, final byte[] result) throws IOException {
    int pos = 0;
    int read;
  ...
  protected void encode(final InputStream input, final OutputStream encoded, final COSDictionary parameters)
    ...
    final CCITTFaxEncoderStream ccittFaxEncoderStream = new CCITTFaxEncoderStream(encoded, cols, rows,
