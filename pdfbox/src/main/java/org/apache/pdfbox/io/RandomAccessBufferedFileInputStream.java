...
public class RandomAccessBufferedFileInputStream extends InputStream implements RandomAccessRead {
  ...
  private final Map<Long, byte[]> pageCache = new LinkedHashMap<>(maxCachedPages, 0.75f, true) {
  ...
  /**
   * Create a random access input stream instance for the file with the given
   * name.
   *
   * @param filename the filename of the file to be read.
   * @throws IOException if something went wrong while accessing the given file.
   */
  public RandomAccessBufferedFileInputStream(final String filename) throws IOException {
   ...
   * @throws IOException if something went wrong while accessing the given file.
   */
  public RandomAccessBufferedFileInputStream(final File file) throws IOException {
  ...
  /**
   * Create a random access input stream for the given input stream by copying the
   * data to a temporary file.
   *
   * @param input the input stream to be read. It will be closed by this method.
   * @throws IOException if something went wrong while creating the temporary
   *                     file.
   */
  public RandomAccessBufferedFileInputStream(final InputStream input) throws IOException {
  ...
  private File createTmpFile(final InputStream input) throws IOException {
    final File tmpFile = File.createTempFile(RandomAccessBufferedFileInputStream.TMP_FILE_PREFIX, ".pdf");
  ...
  private byte[] readPage() throws IOException {
    ...
    while (readBytes < pageSize) {
      final int curBytesRead = raFile.read(page, readBytes, pageSize - readBytes);
      if (curBytesRead < 0) {
        // EOF
        break;
      }
