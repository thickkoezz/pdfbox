 ...
 * {@link ScratchFileBuffer}). A buffer is created calling
 * {@link #createBuffer()}.
 *
 * <p>
 * Pages can be stored in main memory or in a temporary file. A mixed mode is
 * supported storing a certain amount of pages in memory and only the additional
 ...
 * temporary file) it is deleted when {@link ScratchFile#close()} is called.
 * </p>
 *
 * <p>
 ...
 * otherwise would get an {@link OutOfMemoryError} in case of using
 * {@link RandomAccessBuffer}.
 * </p>
 *
 * <p>
 * This base class for providing pages is thread safe (the buffer
...
public class ScratchFile implements Closeable {
  ...
  /**
   * holds pointers to in-memory page content; will be initialized once in case of
   * restricted main memory, otherwise it is enlarged as needed and first
   * initialized to a size of {@link #INIT_UNRESTRICTED_MAINMEM_PAGECOUNT}
   */
   ...
   *                             or <code>null</code> to created it in the default
   *                             temporary directory.
   *
   ...
   * (memorySize/{@link #PAGE_SIZE}) will be stored in-memory and only additional
   * pages will be written to/read from scratch file.
   * </p>
   *
   * @param memUsageSetting set how memory/temporary files are used for buffering
   *                        streams etc.
   *
   * @throws IOException If scratch file directory was given but don't exist.
   */
  public ScratchFile(final MemoryUsageSetting memUsageSetting) throws IOException {
    ...
    if (scratchFileDirectory != null && !scratchFileDirectory.isDirectory())
      throw new IOException("Scratch file directory does not exist: " + scratchFileDirectory);

    maxPageCount = memUsageSetting.isStorageRestricted()
        ? (int) Math.min(Integer.MAX_VALUE, memUsageSetting.getMaxStorageBytes() / ScratchFile.PAGE_SIZE)
        : Integer.MAX_VALUE;

    inMemoryMaxPageCount = memUsageSetting.useMainMemory() ? memUsageSetting.isMainMemoryRestricted()
        ? (int) Math.min(Integer.MAX_VALUE, memUsageSetting.getMaxMainMemoryBytes() / ScratchFile.PAGE_SIZE)
        : Integer.MAX_VALUE : 0;
    inMemoryPages = new byte[maxMainMemoryIsRestricted ? inMemoryMaxPageCount
        : ScratchFile.INIT_UNRESTRICTED_MAINMEM_PAGECOUNT][];
   ...
   * (same as
   * <code>new ScratchFile(MemoryUsageSetting.setupMainMemoryOnly())</code>).
   *
  ...
  public static ScratchFile getMainMemoryOnlyInstance() {
    try {
      ...
      ScratchFile.LOG
          .error("Unexpected exception occurred creating main memory scratch file instance: " + ioe.getMessage(), ioe);
      return null;
    }
  }
   ...
   *                           <code>-1</code> for no restriction; <code>0</code>
  ...
  /**
   * Returns a new free page, either from free page pool or by enlarging scratch
   * file (may be created).
   *
  ...
  /**
   * This will provide new free pages by either enlarging the scratch file by a
   * number of pages defined by {@link #ENLARGE_PAGE_COUNT} - in case scratch file
   * usage is allowed - or increase the {@link #inMemoryPages} array in case main
   * memory was not restricted. If neither of both is allowed/the case than free
   * pages count won't be changed. The same is true if no new pages could be added
   * because we reached the maximum of {@link Integer#MAX_VALUE} pages.
   *
   * <p>
   * If scratch file uage is allowed and scratch file does not exist already it
   * will be created.
   * </p>
   *
   * <p>
      ...
      if (useScratchFile) {
        ...
        if (raf == null) {
          ...
          try {
            ...
            if (!file.delete()) {
              ScratchFile.LOG.warn("Error deleting scratch file: " + file.getAbsolutePath());
            }
            throw e;
          }
        }
        ...
        final long expectedFileLen = ((long) pageCount - inMemoryMaxPageCount) * ScratchFile.PAGE_SIZE;
        ...
        if (pageCount + ScratchFile.ENLARGE_PAGE_COUNT > pageCount) {
          fileLen += ScratchFile.ENLARGE_PAGE_COUNT * ScratchFile.PAGE_SIZE;

          raf.setLength(fileLen);

          freePages.set(pageCount, pageCount + ScratchFile.ENLARGE_PAGE_COUNT);
        }
   ...
   *
   * @return byte array of size {@link #PAGE_SIZE} filled with page data read from
   *         file
   *
   * @throws IOException
   */
  byte[] readPage(final int pageIdx) throws IOException {
    ...
    synchronized (ioLock) {
      ...
      final byte[] page = new byte[ScratchFile.PAGE_SIZE];
      raf.seek(((long) pageIdx - inMemoryMaxPageCount) * ScratchFile.PAGE_SIZE);
      raf.readFully(page);
  ...
  /**
   * Writes updated page. Page is either kept in-memory if pageIdx &lt;
   * {@link #inMemoryMaxPageCount} or is written to scratch file.
   *
   * <p>
   * Provided page byte array must not be re-used for other pages since we store
   * it as is in case of in-memory handling.
   * </p>
   *
   * @param pageIdx index of page to write
   * @param page    page to write (length has to be {@value #PAGE_SIZE})
   *
   * @throws IOException in case page index is out of range or page has wrong
   *                     length or writing to file failed
   */
  void writePage(final int pageIdx, final byte[] page) throws IOException {
    ...
    if (page.length != ScratchFile.PAGE_SIZE)
      throw new IOException("Wrong page size to write: " + page.length + ". Expected: " + ScratchFile.PAGE_SIZE);
  ...
  /**
   * Checks if this page handler has already been closed. If so, an
  ...
  /**
   * Creates a new buffer using this page handler and initializes it with the data
   * read from provided input stream (input stream is copied to buffer). The
   * buffer data pointer is reset to point to first byte.
   *
  ...
  /**
   * Closes and deletes the temporary file. No further interaction with the
   * scratch file or associated buffers can happen after this method is called. It
   * also releases in-memory pages.
   *
   * @throws IOException If there was a problem closing or deleting the temporary
   *                     file.
   */
  @Override
