...
public final class MemoryUsageSetting {
  ...
  /**
   * maximum number of main-memory bytes allowed to be used; <code>-1</code> means
   * 'unrestricted'
   */
  ...
  /**
   * Private constructor for setup buffering memory usage called by one of the
   * setup methods.
   *
   * @param useMainMemory      if <code>true</code> main memory usage is enabled;
   *                           in case of <code>false</code> and
   *                           <code>useTempFile</code> is <code>false</code> too
   *                           we set this to <code>true</code>
   * @param useTempFile        if <code>true</code> using of temporary file(s) is
   *                           enabled
   * @param maxMainMemoryBytes maximum number of main-memory to be used; if
   *                           <code>-1</code> means 'unrestricted'; if
   *                           <code>0</code> we only use temporary file if
   *                           <code>useTempFile</code> is <code>true</code>
   *                           otherwise main-memory usage will have restriction
   *                           defined by maxStorageBytes
   * @param maxStorageBytes    maximum size the main-memory and temporary file(s)
   *                           may have all together; <code>0</code> or less will
   *                           be ignored; if it is less than maxMainMemoryBytes
   *                           we use maxMainMemoryBytes value instead
   */
  private MemoryUsageSetting(final boolean useMainMemory, final boolean useTempFile, final long maxMainMemoryBytes,
  ...
  /**
   * Setups buffering memory usage to only use main-memory with the defined
   * maximum.
   *
  ...
  public static MemoryUsageSetting setupTempFileOnly() {
    return MemoryUsageSetting.setupTempFileOnly(-1);
  }
   ...
   *                        together; <code>-1</code> for no restriction;
   *                        <code>0</code> will also be interpreted here as no
   *                        restriction
   */
   ...
   * additionally temporary file(s) in case the specified portion is exceeded.
   *
   * @param maxMainMemoryBytes maximum number of main-memory to be used; if
   *                           <code>-1</code> this is the same as
   *                           {@link #setupMainMemoryOnly()}; if <code>0</code>
   *                           this is the same as {@link #setupTempFileOnly()}
   */
   ...
   *
   * @param parallelUseCount specifies the number of parallel usages for the
   *                         setting to be returned
   *
   * @return a copy from this instance with the maximum memory/storage
   *         restrictions adjusted to the multiple usage
   */
  public MemoryUsageSetting getPartitionedCopy(final int parallelUseCount) {
    ...
    final long newMaxStorageBytes = maxStorageBytes <= 0 ? maxStorageBytes : maxStorageBytes / parallelUseCount;

    final MemoryUsageSetting copy = new MemoryUsageSetting(useMainMemory, useTempFile, newMaxMainMemoryBytes,
        newMaxStorageBytes);
   ...
   *
   * <p>
   * If this returns <code>false</code> it is ensured {@link #useTempFile()}
   ...
   *
   * <p>
   * If this returns <code>false</code> it is ensured {@link #useMainMemory}
   * returns <code>true</code>.
   * </p>
   */
  ...
  /**
   * Returns <code>true</code> if maximum amount of storage is restricted to a
  ...
  /**
   * Returns maximum size of storage bytes to be used (main-memory in temporary
   * files all together).
   */
  ...
  /**
   * Returns directory to be used for temporary files or <code>null</code> if it
   * was not set.
   */
