...
/**
 * An implementation of the RandomAccess interface to store data in memory. The
 * data will be stored in chunks organized in an ArrayList.
 */
public class RandomAccessBuffer implements RandomAccess, Cloneable {
  ...
  private int chunkSize = RandomAccessBuffer.DEFAULT_CHUNK_SIZE;
  ...
  public RandomAccessBuffer() {
    this(RandomAccessBuffer.DEFAULT_CHUNK_SIZE);
  }

  /**
   * Default constructor.
   */
  private RandomAccessBuffer(final int definedChunkSize) {
   ...
   */
  public RandomAccessBuffer(final byte[] input) {
   ...
   */
  public RandomAccessBuffer(final InputStream input) throws IOException {
    this();
  ...
  public RandomAccessBuffer clone() {
    final RandomAccessBuffer copy = new RandomAccessBuffer(chunkSize);
  ...
  private int readRemainingBytes(final byte[] b, final int offset, final int length) throws IOException {
    if (pointer >= size)
      return 0;
  ...
  public void write(final byte[] b, final int offset, final int length) throws IOException {
    checkClosed();
    final long newSize = pointer + length;
    final int remainingBytes = chunkSize - currentBufferPointer;
    if (length >= remainingBytes) {
      ...
      final int numberOfNewArrays = (int) remainingBytes2Write / chunkSize;
