...
/**
 * Implementation of {@link RandomAccess} as sequence of multiple fixed size
 * pages handled by {@link ScratchFile}.
 */
class ScratchFileBuffer implements RandomAccess {
  ...
  private long size = 0;
  /**
   * Index of current page in {@link #pageIndexes} (the nth page within this
   * buffer).
   */
  ...
  /**
   * The current position (for next read/write) of the buffer as an offset in the
   * current page.
   */
  ...
  /**
   * contains ordered list of pages with the index the page is known by page
   * handler ({@link ScratchFile})
   */
   ...
   *
   * @param pageHandler The {@link ScratchFile} managing the pages to be used by
   *                    this buffer.
   *
  ...
  /**
   * Checks if this buffer, or the underlying {@link ScratchFile} have been
   * closed, throwing {@link IOException} if so.
   *
   * @throws IOException If either this buffer, or the underlying
   *                     {@link ScratchFile} have been closed.
   */
  ...
  /**
   * Ensures the current page has at least one byte left ({@link #positionInPage}
   * in &lt; {@link #pageSize}).
   *
   * <p>
   * If this is not the case we go to next page (writing current one if changed).
   * If current buffer has no more pages we add a new one.
   * </p>
   *
   * @param addNewPageIfNeeded if <code>true</code> it is allowed to add a new
   *                           page in case we are currently at end of last buffer
   *                           page
   *
   * @return <code>true</code> if we were successful positioning pointer before
   *         end of page; we might return <code>false</code> if it is not allowed
   *         to add another page and current pointer points at end of last page
   *
   * @throws IOException
   */
  ...
  public void seek(final long seekToPosition) throws IOException {
    ...
    /*
     * for now we won't allow to seek past end of buffer; this can be changed by
    ...
    if (seekToPosition >= currentPageOffset && seekToPosition <= currentPageOffset + pageSize) {
      ...
      final int newPagePosition = (int) (seekToPosition / pageSize);
  ...
  public byte[] readFully(final int len) throws IOException {
    ...
    int n = 0;
    do {
      final int count = read(b, n, len - n);
      if (count < 0)
        throw new EOFException();
      n += count;
    } while (n < len);
  ...
  /**
   * While calling finalize is normally discouraged we will have to use it here as
   * long as closing a scratch file buffer is not done in every case. Currently
   * {@link COSStream} creates new buffers without closing the old one - which
   * might still be used.
   *
   * <p>
   * Enabling debugging one will see if there are still cases where the buffer is
   * not closed.
   * </p>
   */
  @Override
  protected void finalize() throws Throwable {
    try {
      if (pageHandler != null && ScratchFileBuffer.LOG.isDebugEnabled()) {
        ScratchFileBuffer.LOG.debug("ScratchFileBuffer not closed!");
      }
      close();
    } finally {
      super.finalize();
