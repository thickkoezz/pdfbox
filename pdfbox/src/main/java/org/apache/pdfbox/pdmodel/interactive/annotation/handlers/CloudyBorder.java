 ...
 * <p>
 * Dashed stroke styles are not recommended with cloudy borders. The result
 * would not look good because some parts of the arcs are traced twice by the
 * stroked path. Actually Acrobat Reader's line style dialog does not allow to
 * choose a dashed and a cloudy style at the same time.
 */
...
class CloudyBorder {
  ...
  /**
   * Creates a new <code>CloudyBorder</code> that writes to the specified content
   * stream.
   *
   * @param stream    content stream
   * @param intensity intensity of cloudy effect (entry <code>I</code>); typically
   *                  1.0 or 2.0
   * @param lineWidth line width for annotation border (entry <code>W</code>)
   * @param rect      annotation rectangle (entry <code>Rect</code>)
   */
  CloudyBorder(final PDAppearanceContentStream stream, final double intensity, final double lineWidth,
   ...
   * specified by the <code>RD</code> entry and the <code>Rect</code> entry that
   * was passed in to the constructor.
   * <p>
   * This can be used for Square and FreeText annotations. However, this does not
   * produce the text and the callout line for FreeTexts.
   *
  ...
  /**
   * Creates a cloudy border for a Circle annotation. The ellipse is specified by
   * the <code>RD</code> entry and the <code>Rect</code> entry that was passed in
   * to the constructor.
   *
   * @param rd entry <code>RD</code>, or null if the entry does not exist
   * @throws IOException If there is an error writing to the stream.
  ...
  /**
   * Returns the <code>BBox</code> entry (bounding box) for the appearance stream
   * form XObject.
   *
  ...
  /**
   * Returns the updated <code>Rect</code> entry for the annotation. The rectangle
   * completely contains the cloudy border.
   *
   ...
   * polygon.
   */
  private void cloudyRectangleImpl(final double left, final double bottom, final double right, final double top,
      final boolean isEllipse) throws IOException {
    ...
    if (w < 1.0) {
      polygon = new Point2D.Double[] { new Point2D.Double(left, bottom), new Point2D.Double(left, top),
          new Point2D.Double(left, bottom) };
    } else if (h < 1.0) {
      polygon = new Point2D.Double[] { new Point2D.Double(left, bottom), new Point2D.Double(right, bottom),
          new Point2D.Double(left, bottom) };
    } else {
      polygon = new Point2D.Double[] { new Point2D.Double(left, bottom), new Point2D.Double(right, bottom),
          new Point2D.Double(right, top), new Point2D.Double(left, top), new Point2D.Double(left, bottom) };
    }
   ...
   */
  private void cloudyPolygonImpl(final Point2D.Double[] vertices, final boolean isEllipse) throws IOException {
    ...
    final double k = Math.cos(CloudyBorder.ANGLE_34_DEG);
    ...
    double alphaPrev = n0 == 0 ? array[0] : CloudyBorder.ANGLE_34_DEG;
    ...
    for (int j = 0; j + 1 < numPoints; j++) {
      ...
      if (Double.compare(length, 0.0) == 0) {
        alphaPrev = CloudyBorder.ANGLE_34_DEG;
        continue;
      }

      // n is the number of intermediate curls in the current polygon segment.
      final int n = computeParamsPolygon(advIntermDefault, advCornerDefault, k, cloudRadius, length, array);
      if (n < 0) {
        if (!outputStarted) {
          moveTo(pt);
        }
      ...
      final double cos = CloudyBorder.cosine(ptNext.x - pt.x, length);
      final double sin = CloudyBorder.sine(ptNext.y - pt.y, length);
      double x = pt.x;
      double y = pt.y;
      ...
      anglePrev = angleCur;
      alphaPrev = n == 0 ? alpha : CloudyBorder.ANGLE_34_DEG;
    }
  }
   ...
   */
  private int computeParamsPolygon(final double advInterm, final double advCorner, final double k, final double r,
      final double length, final double[] array) {
    if (Double.compare(length, 0.0) == 0) {
      array[0] = CloudyBorder.ANGLE_34_DEG;
      array[1] = 0;
      return -1;
    }
   ...
   */
  private void addCornerCurl(final double anglePrev, final double angleCur, final double radius, final double cx,
      final double cy, final double alpha, final double alphaPrev, final boolean addMoveTo) throws IOException {
    double a = anglePrev + CloudyBorder.ANGLE_180_DEG + alphaPrev;
    double b = anglePrev + CloudyBorder.ANGLE_180_DEG + alphaPrev - Math.toRadians(22);
   ...
   */
  private void addFirstIntermediateCurl(final double angleCur, final double r, final double alpha, final double cx,
      final double cy) throws IOException {
    final double a = angleCur + CloudyBorder.ANGLE_180_DEG;

    getArcSegment(a + alpha, a + alpha - CloudyBorder.ANGLE_30_DEG, cx, cy, r, r, null, false);
    getArcSegment(a + alpha - CloudyBorder.ANGLE_30_DEG, a + CloudyBorder.ANGLE_90_DEG, cx, cy, r, r, null, false);
    getArcSegment(a + CloudyBorder.ANGLE_90_DEG, a + CloudyBorder.ANGLE_180_DEG - CloudyBorder.ANGLE_34_DEG, cx, cy, r,
        r, null, false);
  }
   ...
   */
  private Point2D.Double[] getIntermediateCurlTemplate(final double angleCur, final double r) throws IOException {
    final ArrayList<Point2D.Double> points = new ArrayList<>();
    final double a = angleCur + CloudyBorder.ANGLE_180_DEG;

    getArcSegment(a + CloudyBorder.ANGLE_34_DEG, a + CloudyBorder.ANGLE_12_DEG, 0, 0, r, r, points, false);
    getArcSegment(a + CloudyBorder.ANGLE_12_DEG, a + CloudyBorder.ANGLE_90_DEG, 0, 0, r, r, points, false);
    getArcSegment(a + CloudyBorder.ANGLE_90_DEG, a + CloudyBorder.ANGLE_180_DEG - CloudyBorder.ANGLE_34_DEG, 0, 0, r, r,
        points, false);
   ...
   */
  private void outputCurlTemplate(final Point2D.Double[] template, final double x, final double y) throws IOException {
  ...
  /**
   * Makes a polygon whose direction is the same as the positive angle direction
   * in the coordinate system. The polygon must not intersect itself.
   */
  ...
  /**
   * Returns the direction of the specified polygon. A positive value indicates
   * that the polygon's direction is the same as the direction of positive angles
   * in the coordinate system. A negative value indicates the opposite direction.
   *
  ...
  /**
   * Creates one or more Bézier curves that represent an elliptical arc. Angles
   * are in radians. The arc will always proceed in the positive angle direction.
   * If the argument `out` is null, this writes the results to the instance
   * variable `output`.
   */
  private void getArc(final double startAng, final double endAng, final double rx, final double ry, final double cx,
      final double cy, final ArrayList<Point2D.Double> out, final boolean addMoveTo) throws IOException {
    ...
    while (angleTodo > angleIncr) {
      getArcSegment(startAng + angleDone, startAng + angleDone + angleIncr, cx, cy, rx, ry, out, false);
      angleDone += angleIncr;
      angleTodo -= angleIncr;
    }
   ...
   * The sweep angle of the section must not be larger than 90 degrees. If
   * argument `out` is null, this writes the results to the instance variable
   * `output`.
   */
  private void getArcSegment(final double startAng, final double endAng, final double cx, final double cy,
      final double rx, final double ry, final ArrayList<Point2D.Double> out, final boolean addMoveTo)
      throws IOException {
   ...
   */
  private static Point2D.Double[] flattenEllipse(final double left, final double bottom, final double right,
      final double top) {
   ...
   */
  private void cloudyEllipseImpl(final double leftOrig, final double bottomOrig, final double rightOrig,
      final double topOrig) throws IOException {
    if (intensity <= 0.0) {
    ...
    final double radiusAdj = Math.sin(CloudyBorder.ANGLE_12_DEG) * cloudRadius - 1.50;
    ...
    final Point2D.Double[] flatPolygon = CloudyBorder.flattenEllipse(left, bottom, right, top);
    ...
    // Construct centerPoints array, in which each point is the center point of a
    // curl.
    ...
    for (int i = 0; i + 1 < numPoints; i++) {
      ...
      if (lengthTodo >= curlAdvance - comparisonToler || i == numPoints - 2) {
        final double cos = CloudyBorder.cosine(dx, length);
        final double sin = CloudyBorder.sine(dy, length);
   ...
   */
  private double computeParamsEllipse(final Point2D.Double pt, final Point2D.Double ptNext, final double r,
      final double curlAdv) {
    final double length = pt.distance(ptNext);
    if (Double.compare(length, 0.0) == 0)
      return CloudyBorder.ANGLE_34_DEG;
  ...
  private void curveTo(final double ax, final double ay, final double bx, final double by, final double cx,
      final double cy) throws IOException {
    updateBBox(ax, ay);
    updateBBox(bx, by);
    updateBBox(cx, cy);
