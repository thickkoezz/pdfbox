...
public abstract class PDSimpleFont extends PDFont {
   ...
   */
  PDSimpleFont(final COSDictionary fontDictionary) throws IOException {
    super(fontDictionary);
  }

  /**
   * Reads the Encoding from the Font dictionary or the embedded or substituted
   * font file. Must be called at the end of any subclass constructors.
   *
    ...
    if (encoding != null) {
      if (encoding instanceof COSName) {
        ...
        if (this.encoding == null) {
          PDSimpleFont.LOG.warn("Unknown encoding: " + encodingName.getName());
          this.encoding = readEncodingFromFont(); // fallback
        }
      } else if (encoding instanceof COSDictionary) {
        final COSDictionary encodingDict = (COSDictionary) encoding;
        Encoding builtIn = null;
        Boolean symbolic = getSymbolicFlag();
        final boolean isFlaggedAsSymbolic = symbolic != null && symbolic;

        final COSName baseEncoding = encodingDict.getCOSName(COSName.BASE_ENCODING);

        final boolean hasValidBaseEncoding = baseEncoding != null && Encoding.getInstance(baseEncoding) != null;
  ...
  /**
   * Called by readEncoding() if the encoding needs to be extracted from the font
   * file.
   *
   * @throws IOException if the font file could not be read.
   */
  ...
  /**
   * Internal implementation of isSymbolic, allowing for the fact that the result
  ...
  /**
   * Returns the value of the symbolic flag, allowing for the fact that the result
   * may be indeterminate.
   */
  protected final Boolean getSymbolicFlag() {
    if (getFontDescriptor() != null)
      // fixme: isSymbolic() defaults to false if the flag is missing so we can't
      // trust this
  ...
  public String toUnicode(final int code, final GlyphList customGlyphList) throws IOException {
    ...
    // we only do this when the font's glyph list is the AGL, to avoid breaking Zapf
    // Dingbats
    GlyphList unicodeGlyphList;
    if (glyphList == GlyphList.getAdobeGlyphList()) {
      unicodeGlyphList = customGlyphList;
    } else {
      unicodeGlyphList = glyphList;
    }
    ...
    // character.
    if (PDSimpleFont.LOG.isWarnEnabled() && !noUnicode.contains(code)) {
      // we keep track of which warnings have been issued, so we don't log multiple
      // times
      noUnicode.add(code);
      if (name != null) {
        PDSimpleFont.LOG.warn("No Unicode mapping for " + name + " (" + code + ") in font " + getName());
      } else {
        PDSimpleFont.LOG.warn("No Unicode mapping for character code " + code + " in font " + getName());
      }
    }
  ...
  protected final float getStandard14Width(final int code) {
    if (getStandard14AFM() != null) {
      final String nameInAFM = getEncoding().getName(code);
  ...
  public boolean isStandard14() {
    ...
    if (getEncoding() instanceof DictionaryEncoding) {
      ...
      if (dictionary.getDifferences().size() > 0) {
        // we also require that the differences are actually different, see PDFBOX-1900
        // with
  ...
  protected boolean isNonZeroBoundingBox(final PDRectangle bbox) {
    return bbox != null && (Float.compare(bbox.getLowerLeftX(), 0) != 0 || Float.compare(bbox.getLowerLeftY(), 0) != 0
        || Float.compare(bbox.getUpperRightX(), 0) != 0 || Float.compare(bbox.getUpperRightY(), 0) != 0);
  }

  /**
   * Returns the path for the character with the given name. For some fonts, GIDs
   * may be used instead of names when calling this method.
   *
   * @return glyph path
