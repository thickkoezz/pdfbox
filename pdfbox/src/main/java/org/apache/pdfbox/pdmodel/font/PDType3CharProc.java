...
public final class PDType3CharProc implements COSObjectable, PDContentStream {
  ...
  /**
   * Calculate the bounding box of this glyph. This will work only if the first
   * operator in the stream is d1.
   *
   * @return the bounding box of this glyph, or null if the first operator is not
   *         d1.
   * @throws IOException If an io error occurs while parsing the stream.
   */
  public PDRectangle getGlyphBBox() throws IOException {
    final List<COSBase> arguments = new ArrayList<>();
    final PDFStreamParser parser = new PDFStreamParser(getContents());
   ...
   * @throws IOException if the stream could not be read, or did not have d0 or d1
   *                     as first operator, or if their first argument was not a
   *                     number.
   */
