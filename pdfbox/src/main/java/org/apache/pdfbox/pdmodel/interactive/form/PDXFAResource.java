...
public final class PDXFAResource implements COSObjectable {
   ...
   *
   * The XFA is either a stream containing the entire XFA resource or an array
   * specifying individual packets that together make up the XFA resource.
   *
   * A packet is a pair of a string and stream. The string contains the name of
   * the XML element and the stream contains the complete text of this XML
   * element. Each packet represents a complete XML element, with the exception of
   * the first and last packet, which specify begin and end tags for the xdp:xdp
   * element. [IS0 32000-1:2008: 12.7.8]
   *
  ...
  public byte[] getBytes() throws IOException {
    ...
    if (getCOSObject() instanceof COSArray)
      return PDXFAResource.getBytesFromPacket((COSArray) getCOSObject());
    else if (xfa.getCOSObject() instanceof COSStream)
      return PDXFAResource.getBytesFromStream((COSStream) getCOSObject());
    return new byte[0];
  }
  ...
  public Document getDocument() throws IOException {
    return org.apache.pdfbox.util.XMLUtil //
        .parse(new ByteArrayInputStream(getBytes()), true);
  }
}
