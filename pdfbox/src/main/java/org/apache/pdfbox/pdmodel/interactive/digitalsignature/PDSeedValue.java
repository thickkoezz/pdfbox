...
public class PDSeedValue implements COSObjectable {
  ...
  public boolean isFilterRequired() {
    return getCOSObject().getFlag(COSName.FF, PDSeedValue.FLAG_FILTER);
  }
  ...
  public void setFilterRequired(final boolean flag) {
    getCOSObject().setFlag(COSName.FF, PDSeedValue.FLAG_FILTER, flag);
  }
  ...
  public boolean isSubFilterRequired() {
    return getCOSObject().getFlag(COSName.FF, PDSeedValue.FLAG_SUBFILTER);
  }

  /**
   * set true if the subfilter shall be required.
   *
   * @param flag if true, the first supported SubFilter in the array shall be used
   *             when signing.
   */
  public void setSubFilterRequired(final boolean flag) {
    getCOSObject().setFlag(COSName.FF, PDSeedValue.FLAG_SUBFILTER, flag);
  }
  ...
  public boolean isDigestMethodRequired() {
    return getCOSObject().getFlag(COSName.FF, PDSeedValue.FLAG_DIGEST_METHOD);
  }
  ...
  public void setDigestMethodRequired(final boolean flag) {
    getCOSObject().setFlag(COSName.FF, PDSeedValue.FLAG_DIGEST_METHOD, flag);
  }
  ...
  public boolean isVRequired() {
    return getCOSObject().getFlag(COSName.FF, PDSeedValue.FLAG_V);
  }
  ...
  public void setVRequired(final boolean flag) {
    getCOSObject().setFlag(COSName.FF, PDSeedValue.FLAG_V, flag);
  }
  ...
  public boolean isReasonRequired() {
    return getCOSObject().getFlag(COSName.FF, PDSeedValue.FLAG_REASON);
  }
  ...
  public void setReasonRequired(final boolean flag) {
    getCOSObject().setFlag(COSName.FF, PDSeedValue.FLAG_REASON, flag);
  }
  ...
  public boolean isLegalAttestationRequired() {
    return getCOSObject().getFlag(COSName.FF, PDSeedValue.FLAG_LEGAL_ATTESTATION);
  }
  ...
  public void setLegalAttestationRequired(final boolean flag) {
    getCOSObject().setFlag(COSName.FF, PDSeedValue.FLAG_LEGAL_ATTESTATION, flag);
  }
  ...
  public boolean isAddRevInfoRequired() {
    return getCOSObject().getFlag(COSName.FF, PDSeedValue.FLAG_ADD_REV_INFO);
  }
  ...
  public void setAddRevInfoRequired(final boolean flag) {
    getCOSObject().setFlag(COSName.FF, PDSeedValue.FLAG_ADD_REV_INFO, flag);
  }

  /**
   * If <b>Filter</b> is not null and the {@link #isFilterRequired()} indicates
   ...
   * place. If {@link #isFilterRequired()} indicates that this is an optional
   * constraint, this handler may be used if it is available. If it is not
  ...
  /**
   * If <b>SubFilter</b> is not null and the {@link #isSubFilterRequired()}
   * indicates this entry is a required constraint, then the first matching
   * encodings shall be used when signing; otherwise, signing shall not take
   * place. If {@link #isSubFilterRequired()} indicates that this is an optional
   * constraint, then the first matching encoding shall be used if it is
   * available. If it is not available, a different encoding may be used instead.
   *
   * @return the subfilter that shall be used by the signature handler
   */
  public List<String> getSubFilter() {
    List<String> retval = null;
    final COSArray fields = (COSArray) dictionary.getDictionaryObject(COSName.SUB_FILTER);

    if (fields != null) {
      final List<String> actuals = new ArrayList<>();
      for (int i = 0; i < fields.size(); i++) {
        final String element = fields.getName(i);
        if (element != null) {
          actuals.add(element);
        }
      }
   ...
   * handler shall be the encoding that is actually used for signing.
   *
   * @param subfilter is the name that shall be used for encoding
   */
  public void setSubFilter(final List<COSName> subfilter) {
   ...
   * <b>SHA512</b>, <b>RIPEMD160</b>. The default value is
  ...
  public List<String> getDigestMethod() {
    List<String> retval = null;
    final COSArray fields = (COSArray) dictionary.getDictionaryObject(COSName.DIGEST_METHOD);
   ...
   * to use when signing. The value shall be one of <b>SHA1</b>, <b>SHA256</b>,
   * <b>SHA384</b>, <b>SHA512</b>, <b>RIPEMD160</b>. The default value is
   * implementation-specific.
   * </p>
   *
   * <p>
   * This property is only applicable if the digital credential signing contains
   * RSA public/privat keys
   * </p>
   *
   * @param digestMethod is a list of possible names of the digests, that should
   *                     be used for signing.
   */
  public void setDigestMethod(final List<COSName> digestMethod) {
    // integrity check
    for (final COSName cosName : digestMethod) {
      if (!(cosName.equals(COSName.DIGEST_SHA1) || cosName.equals(COSName.DIGEST_SHA256)
          || cosName.equals(COSName.DIGEST_SHA384) || cosName.equals(COSName.DIGEST_SHA512)
  ...
  /**
   * The minimum required capability of the signature field seed value dictionary
   * parser. A value of 1 specifies that the parser shall be able to recognize all
   * seed value dictionary entries in a PDF 1.5 file. A value of 2 specifies that
   * it shall be able to recognize all seed value dictionary entries specified.
   *
   ...
   * specifies that it shall be able to recognize all seed value dictionary
   * entries specified.
   *
  ...
  /**
   * If the Reasons array is provided and {@link #isReasonRequired()} indicates
   * that Reasons is a required constraint, one of the reasons in the array shall
   * be used for the signature dictionary; otherwise signing shall not take place.
   * If the {@link #isReasonRequired()} indicates Reasons is an optional
   * constraint, one of the reasons in the array may be chose or a custom reason
   * can be provided.
   *
  ...
  public List<String> getReasons() {
    List<String> retval = null;
    final COSArray fields = (COSArray) dictionary.getDictionaryObject(COSName.REASONS);

    if (fields != null) {
      final List<String> actuals = new ArrayList<>();
      for (int i = 0; i < fields.size(); i++) {
        final String element = fields.getString(i);
        if (element != null) {
          actuals.add(element);
        }
      }
   ...
   * those used by conforming products.
   *
   * @param reasons is a list of possible text string that specifying possible
   *                reasons
   */
   ...
   * whose value is an integer between 0 and 3. A value of 0 defines the
   * signatures as an author signature. The value 1 through 3 shall be used for
   * certification signatures and correspond to the value of P in a DocMDP
  ...
  public PDSeedValueMDP getMDP() {
    final COSDictionary dict = (COSDictionary) dictionary.getDictionaryObject(COSName.MDP);
  ...
  /**
   * (Optional) A certificate seed value dictionary containing information about
   ...
   * is a ASCII string specifying the URL to a rfc3161 conform timestamp server
   * and Ff to indicate if a timestamp is required or optional.
   * </p>
   *
   * @return the timestamp dictionary as PDSeedValueTimeStamp
   */
  public PDSeedValueTimeStamp getTimeStamp() {
    final COSDictionary dict = (COSDictionary) dictionary.getDictionaryObject(COSName.TIME_STAMP);
  ...
  public List<String> getLegalAttestation() {
    List<String> retval = null;
    final COSArray fields = (COSArray) dictionary.getDictionaryObject(COSName.LEGAL_ATTESTATION);
   ...
   * attestations.
   *
   * @param legalAttestation is a list of possible text string that specifying
