...
/**
 * This is the class that represents a line annotation. Introduced in PDF 1.3
 * specification
 *
 * @author Paul King
 */
public class PDAnnotationLine extends PDAnnotationMarkup {
  ...
  /*
   * The various values for line ending styles, see the PDF 1.6 reference Table
   * 8.23
   */
  ...
  public PDAnnotationLine() {
    getCOSObject().setName(COSName.SUBTYPE, PDAnnotationLine.SUB_TYPE);
   ...
   * entry is set).
   *
   * @param l array of 4 floats [x1, y1, x2, y2] line start and end points in
   *          default user space.
   */
  ...
  /**
   * This will retrieve the start and end coordinates of the line (or leader line
   * if LL entry is set).
   *
   * @return array of floats [x1, y1, x2, y2] line start and end points in default
   *         user space.
   */
  ...
  /**
   * This will retrieve the interior color of the line endings defined in the LE
  ...
  /**
   * This will retrieve whether the text specified by the /Contents or /RC entries
   * shall be shown as a caption in the appearance of the line.
   *
  ...
  public float getLeaderLineLength() {
    return getCOSObject().getFloat(COSName.LL, 0);
  }
  ...
  public float getLeaderLineExtensionLength() {
    return getCOSObject().getFloat(COSName.LLE, 0);
  }
  ...
  public float getLeaderLineOffsetLength() {
    return getCOSObject().getFloat(COSName.LLO, 0);
  }
  ...
  public float getCaptionHorizontalOffset() {
    float retval = 0.f;
    final COSArray array = (COSArray) getCOSObject().getDictionaryObject(COSName.CO);
    if (array != null) {
  ...
  public void constructAppearances() {
    if (customAppearanceHandler == null) {
      final PDLineAppearanceHandler appearanceHandler = new PDLineAppearanceHandler(this);
