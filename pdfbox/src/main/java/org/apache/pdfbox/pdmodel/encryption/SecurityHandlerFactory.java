...
/**
 * Manages security handlers for the application. It follows the singleton
 * pattern. To be usable, security managers must be registered in it. Security
 * managers are retrieved by the application when necessary.
 *
...
public final class SecurityHandlerFactory {
   ...
   *
   * If the security handler was already registered an exception is thrown. If
   * another handler was previously registered for the same filter name or for the
   * same policy name, an exception is thrown
   *
   * @param name             the name of the filter
   * @param securityHandler  security handler class to register
   * @param protectionPolicy protection policy class to register
   */
  public void registerHandler(final String name, final Class<? extends SecurityHandler> securityHandler,
   ...
   */
  public SecurityHandler newSecurityHandlerForPolicy(final ProtectionPolicy policy) {
   ...
   * @return a new SecurityHandler instance, or null if none is available
   */
  public SecurityHandler newSecurityHandlerForFilter(final String name) {
  ...
  /*
   * Returns a new security handler for the given parameters, or null none is
   * available.
   *
   ...
   * @return a new SecurityHandler instance, or null if none is available.
   */
  private SecurityHandler newSecurityHandler(final Class<? extends SecurityHandler> handlerClass,
      final Class<?>[] argsClasses, final Object[] args) {
    try {
      final Constructor<? extends SecurityHandler> ctor = handlerClass.getDeclaredConstructor(argsClasses);
