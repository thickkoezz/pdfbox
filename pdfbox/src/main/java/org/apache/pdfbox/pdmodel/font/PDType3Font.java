...
public class PDType3Font extends PDSimpleFont {
  ...
  protected final void readEncoding() throws IOException {
    ...
    if (encodingBase instanceof COSName) {
      ...
      if (encoding == null) {
        PDType3Font.LOG.warn("Unknown encoding: " + encodingName.getName());
      }
  ...
  public float getWidth(final int code) throws IOException {
    ...
    if (!getWidths().isEmpty() && code >= firstChar && code <= lastChar) {
      final Float w = getWidths().get(code - firstChar);
      return w == null ? 0 : w;
    } else {
  ...
  public float getHeight(final int code) throws IOException {
    ...
    if (desc != null) {
      ...
      // values. Maybe we'll find another way to get those value for every single
      // glyph
  ...
  public PDResources getResources() {
    if (resources == null) {
      ...
      if (base instanceof COSDictionary) {
        resources = new PDResources((COSDictionary) base, resourceCache);
      }
    }
    return resources;
  }
      ...
      for (final COSName name : cp.keySet()) {
        ...
        if (base instanceof COSStream) {
          ...
          try {
            ...
            // ignore
            PDType3Font.LOG.debug("error getting the glyph bounding box - font bounding box will be used", ex);
          }
        }
      }
    }
  ...
  /**
   * Returns the dictionary containing all streams to be used to render the
   * glyphs.
   *
