...
final class SampledImageReader {
  ...
  /**
   * Returns an ARGB image filled with the given paint and using the given image
   * as a mask.
   *
  ...
  public static BufferedImage getStencilImage(final PDImage pdImage, final Paint paint) throws IOException {
    ...
    try (InputStream iis = pdImage.createInputStream()) {
      final float[] decode = SampledImageReader.getDecodeArray(pdImage);
      ...
      for (int y = 0; y < height; y++) {
        int x = 0;
        final int readLen = iis.read(buff);
        ...
        if (readLen != rowLen) {
          SampledImageReader.LOG.warn("premature EOF, image will be incomplete");
          break;
        }
      }
    }
   ...
   * color space. If a color key mask is provided then an ARGB image is returned
   * instead. This method never returns null.
   *
   ...
   *                    pixel, a value of 1 meaning every pixel will be read. It
   *                    must not be larger than the image width or height.
   * @param colorKey    an optional color key mask
  ...
  public static BufferedImage getRGBImage(final PDImage pdImage, final Rectangle region, final int subsampling,
    ...
    final Rectangle clipped = SampledImageReader.clipRegion(pdImage, region);
    ...
    try {
      ...
      // in depth to 8bpc as they will be drawn to TYPE_INT_RGB images anyway. All
      // code
      // in PDColorSpace#toRGBImage expects an 8-bit range, i.e. 0-255.
      // Interleaved raster allows chunk-copying for 8-bit images.
      final WritableRaster raster = Raster.createInterleavedRaster(DataBuffer.TYPE_BYTE, width, height, numComponents,
          new Point(0, 0));
  ...
  private static BufferedImage from1Bit(final PDImage pdImage, final Rectangle clipped, final int subsampling,
    ...
    try (InputStream iis = pdImage.createInputStream(options)) {
      ...
      if (options.isFilterSubsampled()) {
        // Decode options were honored, and so there is no need for additional clipping
        // or subsampling
        inputWidth = width;
        startx = 0;
        starty = 0;
  ...
  private static BufferedImage from8bit(final PDImage pdImage, final WritableRaster raster, final Rectangle clipped,
      final int subsampling, final int width, final int height) throws IOException {
    int currentSubsampling = subsampling;
    final DecodeOptions options = new DecodeOptions(currentSubsampling);
    options.setSourceRegion(clipped);
    try (InputStream input = pdImage.createInputStream(options)) {
      ...
      if (startx == 0 && starty == 0 && scanWidth == width && scanHeight == height && currentSubsampling == 1) {
        ...
        final long inputResult = input.read(bank);
        if (Long.compare(inputResult, width * height * (long) numComponents) != 0) {
          SampledImageReader.LOG.debug("Tried reading " + width * height * (long) numComponents + " bytes but only "
      ...
      int i = 0;
      for (int y = 0; y < starty + scanHeight; ++y) {
        final long inputResult = input.read(tempBytes);

        if (Long.compare(inputResult, tempBytes.length) != 0) {
          SampledImageReader.LOG
              .debug("Tried reading " + tempBytes.length + " bytes but only " + inputResult + " bytes read");
        }
        ...
        if (currentSubsampling == 1) {
          ...
          System.arraycopy(tempBytes, startx * numComponents, bank, y * inputWidth * numComponents,
  ...
  private static BufferedImage fromAny(final PDImage pdImage, final WritableRaster raster, final COSArray colorKey,
      final Rectangle clipped, final int subsampling, final int width, final int height) throws IOException {
    int currentSubsampling = subsampling;
    final PDColorSpace colorSpace = pdImage.getColorSpace();
    final int numComponents = colorSpace.getNumberOfComponents();
    final int bitsPerComponent = pdImage.getBitsPerComponent();
    ...
    try (ImageInputStream iis = new MemoryCacheImageInputStream(pdImage.createInputStream(options))) {
        ...
        for (int x = 0; x < startx + scanWidth; x++) {
          ...
          for (int c = 0; c < numComponents; c++) {
            ...
            if (isIndexed) {
              ...
              final int outputByte = Math.round((output - Math.min(dMin, dMax)) / Math.abs(dMax - dMin) * 255f);
          ...
          if (x >= startx && y >= starty && x % currentSubsampling == 0 && y % currentSubsampling == 0) {
            raster.setDataElements((x - startx) / currentSubsampling, (y - starty) / currentSubsampling,
                srcColorValues);

            // set alpha channel in color key mask, if any
            if (colorKeyMask != null) {
              alpha[0] = (byte) (isMasked ? 255 : 0);
              colorKeyMask.getRaster().setDataElements((x - startx) / currentSubsampling,
      ...
      if (colorKeyMask != null)
        return SampledImageReader.applyColorKeyMask(rgbImage, colorKeyMask);
      else
        return rgbImage;
    }
  }
  ...
  private static float[] getDecodeArray(final PDImage pdImage) throws IOException {
    ...
    if (cosDecode != null) {
      ...
      if (cosDecode.size() != numberOfComponents * 2) {
        if (pdImage.isStencil() && cosDecode.size() >= 2 && cosDecode.get(0) instanceof COSNumber
          ...
          if (decode0 >= 0 && decode0 <= 1 && decode1 >= 0 && decode1 <= 1) {
            SampledImageReader.LOG
                .warn("decode array " + cosDecode + " not compatible with color space, using the first two entries");
            return new float[] { decode0, decode1 };
          }
        }
        SampledImageReader.LOG.error("decode array " + cosDecode + " not compatible with color space, using default");
      } else {
