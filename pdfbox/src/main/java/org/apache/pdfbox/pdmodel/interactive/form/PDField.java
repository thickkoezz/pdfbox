...
public abstract class PDField implements COSObjectable {
  ...
  /**
   * Creates a COSField subclass from the given COS field. This is for reading
   * fields from PDFs.
   *
   ...
   * on the actual type. The field type is an inheritable attribute.
   *
   ...
   *
   * @return a List of {@link PDAnnotationWidget} annotations. Be aware that this
   *         list is <i>not</i> backed by the actual widget collection of the
   *         field, so adding or deleting has no effect on the PDF document. For
   *         {@link PDTerminalField} you'd have to call
   *         {@link PDTerminalField#setWidgets(java.util.List) setWidgets()} with
   *         the modified list.
   */
  ...
  public void setReadOnly(final boolean readonly) {
    dictionary.setFlag(COSName.FF, PDField.FLAG_READ_ONLY, readonly);
  }
  ...
  public boolean isReadOnly() {
    return dictionary.getFlag(COSName.FF, PDField.FLAG_READ_ONLY);
  }

  /**
   * sets the flag whether the field is to be required to have a value at the time
   * it is exported by a submit-form action.
   *
   * @param required The new flag for required.
   */
  public void setRequired(final boolean required) {
    dictionary.setFlag(COSName.FF, PDField.FLAG_REQUIRED, required);
  }

  /**
   * @return true if the field is required to have a value at the time it is
   *         exported by a submit-form action.
   */
  public boolean isRequired() {
    return dictionary.getFlag(COSName.FF, PDField.FLAG_REQUIRED);
  }
  ...
  public void setNoExport(final boolean noExport) {
    dictionary.setFlag(COSName.FF, PDField.FLAG_NO_EXPORT, noExport);
  }
  ...
  public boolean isNoExport() {
    return dictionary.getFlag(COSName.FF, PDField.FLAG_NO_EXPORT);
  }
  ...
  /**
   * Get the additional actions for this field. This will return null if there are
   * no additional actions for this field.
   *
   ...
   * of the name to search down the tree of names. The nameIndex is where to start
   * in that array. This method is called recursively until it finds the end point
   * based on the name array.
   *
  ...
  /**
   * Gets the alternate name of the field ("shall be used in place of the actual
   * field name wherever the field shall be identified in the user interface (such
   * as in error or status messages referring to the field)").
   *
  ...
  /**
   * This will set the alternate name of the field ("shall be used in place of the
   * actual field name wherever the field shall be identified in the user
   * interface (such as in error or status messages referring to the field)"). The
   * text appears as a tool tip in Adobe Reader. Because of the usage for error or
   * status messages, it should be different for each field.
   *
