...
abstract class TriangleBasedShadingContext extends ShadingContext implements PaintContext {
   ...
   *                content stream
   * @throws IOException if there is an error getting the color space or doing
  ...
  TriangleBasedShadingContext(final PDShading shading, final ColorModel cm, final AffineTransform xform,
    ...
    TriangleBasedShadingContext.LOG.debug("bitsPerCoordinate: " + (Math.pow(2, bitsPerCoordinate) - 1));
    bitsPerColorComponent = triangleBasedShadingType.getBitsPerComponent();
    TriangleBasedShadingContext.LOG.debug("bitsPerColorComponent: " + bitsPerColorComponent);
    numberOfColorComponents = hasFunction ? 1 : getShadingColorSpace().getNumberOfComponents();
    TriangleBasedShadingContext.LOG.debug("numberOfColorComponents: " + numberOfColorComponents);
  }
   ...
   *
   * @return a Hash table which contains all the points' positions and colors of
   *         one image
   */
  abstract Map<Point, Integer> calcPixelTable(Rectangle deviceBounds) throws IOException;

  /**
   * Get the points from the triangles, calculate their color and add point-color
   * mappings.
   */
  protected void calcPixelTable(final List<ShadedTriangle> triangleList, final Map<Point, Integer> map,
    ...
    for (final ShadedTriangle tri : triangleList) {
      ...
      if (degree == 2) {
        ...
        final Point p0 = new IntPoint((int) Math.round(tri.corner[0].getX()), (int) Math.round(tri.corner[0].getY()));
        final Point p1 = new IntPoint((int) Math.round(tri.corner[1].getX()), (int) Math.round(tri.corner[1].getY()));
        final Point p2 = new IntPoint((int) Math.round(tri.corner[2].getX()), (int) Math.round(tri.corner[2].getY()));
  ...
  /**
   * Convert color to RGB color value, using function if required, then convert
   * from the shading color space to an RGB value, which is encoded into an
   * integer.
   */
