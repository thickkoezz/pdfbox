  ...
  public PDType1CFont(final COSDictionary fontDictionary) throws IOException {
    ...
    if (fd != null) {
      ...
      if (ff3Stream != null) {
        ...
        if (bytes.length == 0) {
          PDType1CFont.LOG.error("Invalid data for embedded Type1C font " + getName());
          bytes = null;
        }
      }
    }
    ...
    try {
      ...
      PDType1CFont.LOG.error("Can't read the embedded Type1C font " + getName(), e);
      fontIsDamaged = true;
    }
    ...
    if (cffFont != null) {
      ...
      if (mapping.isFallback()) {
        PDType1CFont.LOG.warn("Using fallback font " + genericFont.getName() + " for " + getBaseFont());
      }
      isEmbedded = false;
    }
    readEncoding();
  ...
  public final Matrix getFontMatrix() {
    if (fontMatrix == null) {
      ...
      try {
        ...
        PDType1CFont.LOG.debug("Couldn't get font matrix - returning default value", e);
  ...
  private String getNameInFont(final String name) throws IOException {
    if (isEmbedded() || genericFont.hasGlyph(name))
      ...
      if (unicodes != null && unicodes.length() == 1) {
        final String uniName = UniUtil.getUniNameOfCodePoint(unicodes.codePointAt(0));
