...
public abstract class PDStandardAttributeObject extends PDAttributeObject {
   ...
   * @return <code>true</code> if the attribute is specified, <code>false</code>
   *         otherwise
   */
  ...
  protected void setString(final String name, final String value) {
    final COSBase oldBase = getCOSObject().getDictionaryObject(name);
    getCOSObject().setString(name, value);
    final COSBase newBase = getCOSObject().getDictionaryObject(name);
  ...
  protected String[] getArrayOfString(final String name) {
    final COSBase v = getCOSObject().getDictionaryObject(name);
  ...
  protected void setArrayOfString(final String name, final String[] values) {
    ...
    getCOSObject().setItem(name, array);
    final COSBase newBase = getCOSObject().getDictionaryObject(name);
    potentiallyNotifyChanged(oldBase, newBase);
  }
  ...
  protected Object getNumberOrArrayOfNumber(final String name, final float defaultValue) {
    ...
    if (Float.compare(defaultValue, PDStandardAttributeObject.UNSPECIFIED) == 0)
      return null;
    return defaultValue;
  }
  ...
  protected PDGamma getColor(final String name) {
    final COSArray c = (COSArray) getCOSObject().getDictionaryObject(name);
    if (c != null)
  ...
  protected Object getColorOrFourColors(final String name) {
    final COSArray array = (COSArray) getCOSObject().getDictionaryObject(name);
    if (array == null)
      return null;
    if (array.size() == 3)
      // only one colour
