...
public class PDTrueTypeFont extends PDSimpleFont implements PDVectorFont {
  ...
  static {
    final Map<Integer, String> codeToName = MacOSRomanEncoding.INSTANCE.getCodeToNameMap();
    for (final Map.Entry<Integer, String> entry : codeToName.entrySet()) {
      if (!PDTrueTypeFont.INVERTED_MACOS_ROMAN.containsKey(entry.getValue())) {
        PDTrueTypeFont.INVERTED_MACOS_ROMAN.put(entry.getValue(), entry.getKey());
      }
    }
  }
  ...
  public PDTrueTypeFont(final COSDictionary fontDictionary) throws IOException {
    ...
    if (getFontDescriptor() != null) {
      ...
      if (ff2Stream != null) {
        try {
          ...
          PDTrueTypeFont.LOG.warn("Could not read embedded TTF for font " + getBaseFont(), e);
          fontIsDamaged = true;
        }
      }
    }
    ...
    // substitute
    if (ttfFont == null) {
      final FontMapping<TrueTypeFont> mapping = FontMappers.instance().getTrueTypeFont(getBaseFont(),
          getFontDescriptor());
      ttfFont = mapping.getFont();

      if (mapping.isFallback()) {
        PDTrueTypeFont.LOG.warn("Using fallback font '" + ttfFont + "' for '" + getBaseFont() + "'");
      }
    }
    ttf = ttfFont;
    readEncoding();
  ...
  private PDTrueTypeFont(final PDDocument document, final TrueTypeFont ttf, final Encoding encoding,
      final boolean closeTTF) throws IOException {
    final PDTrueTypeFontEmbedder embedder = new PDTrueTypeFontEmbedder(document, dict, ttf, encoding);
   ...
   */
  public static PDTrueTypeFont load(final PDDocument doc, final File file, final Encoding encoding) throws IOException {
  ...
  protected Encoding readEncodingFromFont() throws IOException {
    if (!isEmbedded() && getStandard14AFM() != null)
      ...
      // non-symbolic fonts don't have a built-in encoding per se, but there encoding
      // is
      // assumed to be StandardEncoding by the PDF spec unless an explicit Encoding is
      // present
    ...
    if (encoding != null) {
      if (!encoding.contains(getGlyphList().codePointToName(unicode)))
        throw new IllegalArgumentException(
            String.format("U+%04X is not available in this font's encoding: %s", unicode, encoding.getEncodingName()));
      ...
      if (!ttf.hasGlyph(name)) {
        // try unicode name
        final String uniName = UniUtil.getUniNameOfCodePoint(unicode);
  ...
  /**
   * Inverts the font's code -&gt; GID mapping. Any duplicate (GID -&gt; code)
    ...
    if (!isSymbolic()) // non-symbolic
    {
      ...
      if (".notdef".equals(name))
        ...
        if (gid == 0 && cmapMacRoman != null) {
          final Integer macCode = PDTrueTypeFont.INVERTED_MACOS_ROMAN.get(name);
          if (macCode != null) {
    ...
    } else // symbolic
    {
      ...
      if (cmapWinSymbol != null) {
        ...
        if (code >= 0 && code <= 0xFF) {
          ...
          if (gid == 0) {
            // F000 - F0FF
            gid = cmapWinSymbol.getGlyphId(code + PDTrueTypeFont.START_RANGE_F000);
          }
          if (gid == 0) {
            // F100 - F1FF
            gid = cmapWinSymbol.getGlyphId(code + PDTrueTypeFont.START_RANGE_F100);
          }
          if (gid == 0) {
            // F200 - F2FF
            gid = cmapWinSymbol.getGlyphId(code + PDTrueTypeFont.START_RANGE_F200);
          }
        }
      }
