...
/**
 * Contains additional information about the components of colour space. Instead
 * of using the alternate color space and tint transform, conforming readers may
 * use custom blending algorithms, along with other information provided in the
...
public final class PDDeviceNAttributes {
  ...
  /**
   * Creates a new DeviceN colour space attributes dictionary from the given
   * dictionary.
   *
  ...
  public PDDeviceNProcess getProcess() {
    final COSDictionary process = (COSDictionary) dictionary.getDictionaryObject(COSName.PROCESS);
    if (process == null)
      return null;
  ...
  public String toString() {
    ...
    try {
      ...
      PDDeviceNAttributes.LOG.debug("Couldn't get the colorants information - returning 'ERROR' instead'", e);
      sb.append("ERROR");
    }
    sb.append('}');
    return sb.toString();
