...
public class FDFDictionary implements COSObjectable {
      ...
      if (node instanceof Element) {
        ...
        case "ids":
          ...
          try {
            ...
            FDFDictionary.LOG
                .warn("Error parsing ID entry for attribute 'original' [" + original + "]. ID entry ignored.", e);
          }
          try {
            ids.add(COSString.parseHex(modified));
          } catch (final IOException e) {
            FDFDictionary.LOG
                .warn("Error parsing ID entry for attribute 'modified' [" + modified + "]. ID entry ignored.", e);
          }
          setID(ids);
          break;
        case "fields":
          ...
          for (int f = 0; f < fields.getLength(); f++) {
            ...
            if (currentNode instanceof Element && ((Element) currentNode).getTagName().equals("field")) {
              try {
                ...
                FDFDictionary.LOG.warn("Error parsing field entry [" + currentNode.getNodeValue() + "]. Field ignored.",
                    e);
              }
            }
          }
            ...
            if (annotNode instanceof Element) {
              ...
              try {
                ...
                case "underline":
                  ...
                  break;
                default:
                  FDFDictionary.LOG.warn("Unknown or unsupported annotation type '" + annotationName + "'");
                  break;
                }
              } catch (final IOException e) {
                FDFDictionary.LOG
                    .warn("Error parsing annotation information [" + annot.getNodeValue() + "]. Annotation ignored", e);
              }
            }
          }
  ...
  public void writeXML(final Writer output) throws IOException {
    final PDFileSpecification fs = getFile();
    if (fs != null) {
      output.write("<f href=\"" + fs.getFile() + "\" />\n");
    }
    final COSArray ids = getID();
    if (ids != null) {
  ...
  /**
   * The source file or target file: the PDF document file that this FDF file was
   * exported from or is intended to be imported into.
   *
  ...
  /**
   * This will get the list of FDF Pages. This will return a list of FDFPage
   * objects.
   *
  ...
  /**
   * The encoding to be used for a FDF field. The default is PDFDocEncoding and
  ...
  /**
   * This will get the list of FDF Annotations. This will return a list of
   * FDFAnnotation objects or null if the entry is not set.
   *
  ...
  /**
   * This will set the list of annotations. This should be a list of FDFAnnotation
   * objects.
   *
   *
