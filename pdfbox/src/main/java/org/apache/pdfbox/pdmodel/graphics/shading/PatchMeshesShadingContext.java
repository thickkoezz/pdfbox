...
abstract class PatchMeshesShadingContext extends TriangleBasedShadingContext {
   ...
   * @param controlPoints number of control points, 12 for type 6 shading and 16
   *                      for type 7 shading
   * @throws IOException if something went wrong
   */
  protected PatchMeshesShadingContext(final PDShadingType6 shading, final ColorModel colorModel,
      final AffineTransform xform, final Matrix matrix, final Rectangle deviceBounds, final int controlPoints)
      throws IOException {
  ...
  /**
   * Create a patch list from a data stream, the returned list contains all the
   * patches contained in the data stream.
   *
  ...
  final List<Patch> collectPatches(final PDShadingType6 shadingType, final AffineTransform xform, final Matrix matrix,
    ...
    try (ImageInputStream mciis = new MemoryCacheImageInputStream(cosStream.createInputStream())) {
      ...
      try {
        ...
        PatchMeshesShadingContext.LOG.error(ex);
      }
      ...
      while (!eof) {
        try {
          ...
          final Patch current = readPatch(mciis, isFree, implicitEdge, implicitCornerColor, maxSrcCoord, maxSrcColor,
              rangeX, rangeY, colRange, matrix, xform, controlPoints);
          if (current == null) {
            break;
          }
          list.add(current);
          ...
          case 3:
            ...
            break;
          default:
            PatchMeshesShadingContext.LOG.warn("bad flag: " + flag);
            break;
          }
        } catch (final EOFException ex) {
          eof = true;
  ...
  /**
   * Read a single patch from a data stream, a patch contains information of its
   * coordinates and color parameters.
   *
   ...
   * @param maxSrcCoord         the maximum coordinate value calculated from
   *                            source data
   * @param maxSrcColor         the maximum color value calculated from source
   *                            data
   * @param rangeX              range for coordinate x
   * @param rangeY              range for coordinate y
   * @param colRange            range for color
   * @param matrix              the pattern matrix concatenated with that of the
  ...
  protected Patch readPatch(final ImageInputStream input, final boolean isFree, final Point2D[] implicitEdge,
      final float[][] implicitCornerColor, final long maxSrcCoord, final long maxSrcColor, final PDRange rangeX,
      final PDRange rangeY, final PDRange[] colRange, final Matrix matrix, final AffineTransform xform,
      final int controlPoints) throws IOException {
    ...
    try {
      ...
      PatchMeshesShadingContext.LOG.debug("EOF", ex);
      return null;
    }
