...
/**
 * Represents a default appearance string, as found in the /DA entry of free
 * text annotations.
 *
 * <p>
 * The default appearance string (DA) contains any graphics state or text state
 * operators needed to establish the graphics state parameters, such as text
 * size and colour, for displaying the field’s variable text. Only operators
...
class PDDefaultAppearanceString {
  ...
  private PDFont font;
  private float fontSize = PDDefaultAppearanceString.DEFAULT_FONT_SIZE;
   ...
   * @throws IOException in case there are missing operators or the font is not
  ...
  private void processSetFont(final List<COSBase> operands) throws IOException {
    ...
    // todo: handle cases where font == null with special mapping logic (see
    // PDFBOX-2661)
    if (font == null)
  ...
  private void processSetFontColor(final List<COSBase> operands) throws IOException {
    ...
    default:
      throw new IOException(
          "Missing operands for set non stroking color operator " + Arrays.toString(operands.toArray()));
    }
  ...
  /**
   * Write font name, font size and color from the /DA string to the given content
   * stream.
   *
   * @param contents     The content stream.
   * @param zeroFontSize The calculated font size to use if the /DA string has a
   *                     size 0 (autosize). Otherwise the size from the /DA string
   *                     is used.
   */
  ...
  /**
   * Copies any needed resources from the document’s DR dictionary into the
   * stream’s Resources dictionary. Resources with the same name shall be left
   * intact.
   */
