...
class CubicBezierCurve {
   ...
   *                  into 2^0 = 1 segments, if l = n, one cubic Bezier curve is
   *                  divided into 2^n segments
   */
  ...
  // calculate sampled points on the cubic Bezier curve defined by the 4 given
  // control points
  private Point2D[] getPoints(int l) {
    ...
    for (int i = 0; i < sz; i++) {
      ...
      final double tmpX = (1 - t) * (1 - t) * (1 - t) * controlPoints[0].getX()
          + 3 * t * (1 - t) * (1 - t) * controlPoints[1].getX() + 3 * t * t * (1 - t) * controlPoints[2].getX()
          + t * t * t * controlPoints[3].getX();
      final double tmpY = (1 - t) * (1 - t) * (1 - t) * controlPoints[0].getY()
          + 3 * t * (1 - t) * (1 - t) * controlPoints[1].getY() + 3 * t * t * (1 - t) * controlPoints[2].getY()
