...
/**
 * This class represents a certificate seed value dictionary that is in the seed
 * value which puts constraints on certificates when signing documents.
 *
 * @author Hossam Hazem
 */
public class PDSeedValueCertificate implements COSObjectable {
  ...
  public boolean isSubjectRequired() {
    return getCOSObject().getFlag(COSName.FF, PDSeedValueCertificate.FLAG_SUBJECT);
  }
  ...
  public void setSubjectRequired(final boolean flag) {
    getCOSObject().setFlag(COSName.FF, PDSeedValueCertificate.FLAG_SUBJECT, flag);
  }
  ...
  public boolean isIssuerRequired() {
    return getCOSObject().getFlag(COSName.FF, PDSeedValueCertificate.FLAG_ISSUER);
  }
  ...
  public void setIssuerRequired(final boolean flag) {
    getCOSObject().setFlag(COSName.FF, PDSeedValueCertificate.FLAG_ISSUER, flag);
  }
  ...
  public boolean isOIDRequired() {
    return getCOSObject().getFlag(COSName.FF, PDSeedValueCertificate.FLAG_OID);
  }
  ...
  public void setOIDRequired(final boolean flag) {
    getCOSObject().setFlag(COSName.FF, PDSeedValueCertificate.FLAG_OID, flag);
  }
  ...
  public boolean isSubjectDNRequired() {
    return getCOSObject().getFlag(COSName.FF, PDSeedValueCertificate.FLAG_SUBJECT_DN);
  }

  /**
   * set true if subject DN shall be required as a constraint on signature.
   *
   * @param flag if true, the specified Subject DN shall be enforced as a
   *             constraint.
   */
  public void setSubjectDNRequired(final boolean flag) {
    getCOSObject().setFlag(COSName.FF, PDSeedValueCertificate.FLAG_SUBJECT_DN, flag);
  }
  ...
  public boolean isKeyUsageRequired() {
    return getCOSObject().getFlag(COSName.FF, PDSeedValueCertificate.FLAG_KEY_USAGE);
  }

  /**
   * set true if KeyUsage shall be required as a constraint on signature.
   *
   * @param flag if true, the specified KeyUsage shall be enforced as a
   *             constraint.
   */
  public void setKeyUsageRequired(final boolean flag) {
    getCOSObject().setFlag(COSName.FF, PDSeedValueCertificate.FLAG_KEY_USAGE, flag);
  }
  ...
  public boolean isURLRequired() {
    return getCOSObject().getFlag(COSName.FF, PDSeedValueCertificate.FLAG_URL);
  }
  ...
  public void setURLRequired(final boolean flag) {
    getCOSObject().setFlag(COSName.FF, PDSeedValueCertificate.FLAG_URL, flag);
  }
  ...
  public List<byte[]> getSubject() {
    final COSBase base = dictionary.getDictionaryObject(COSName.SUBJECT);
    if (base instanceof COSArray) {
      final COSArray array = (COSArray) base;
      return PDSeedValueCertificate.getListOfByteArraysFromCOSArray(array);
    }
    return null;
  }

  /**
   * (Optional) A list of byte arrays containing DER-encoded X.509v3 certificates
   * that are acceptable for signing. if <b>Subject</b> is not null and
   * {@link #isSubjectRequired()} is true then the subject constraint is enforced
   * on the subjects in this array subjects.
   *
   * @param subjects list of byte arrays containing DER-encoded X.509v3
  ...
  /**
   * (Optional) byte array containing DER-encoded X.509v3 certificate that is
   * acceptable for signing. works like {@link #setSubject(List)} but one byte
   * array
   *
  ...
  public void addSubject(final byte[] subject) {
    ...
    final COSString string = new COSString(subject);
    array.add(string);
    dictionary.setItem(COSName.SUBJECT, array);
  }
  ...
  /**
   * Returns list of maps that contains subject distinguished names like [(cn:
   * John Doe, o: Doe), (cn: John Smith)] both keys are typically of the form
   * 'cn', 'o', 'email', '2.5.4.43'; and values are text strings.
   */
  public List<Map<String, String>> getSubjectDN() {
    final COSBase base = dictionary.getDictionaryObject(COSName.SUBJECT_DN);
    if (base instanceof COSArray) {
      final COSArray cosArray = (COSArray) base;
      final List subjectDNList = cosArray.toList();
      final List<Map<String, String>> result = new LinkedList<>();
      for (final Object subjectDNItem : subjectDNList) {
  ...
  /**
   * (Optional; PDF 1.7) A list of maps, where each map contains key value pairs,
   * that specify the Subject Distinguished Name (DN) that must be present within
   * the certificate for it to be acceptable for signing. The certificate must at
   * a minimum contain all the attributes specified in one of the maps entered.
   *
  ...
  public void setSubjectDN(final List<Map<String, String>> subjectDN) {
    ...
    for (final Map<String, String> subjectDNItem : subjectDN) {
      ...
      for (final Map.Entry<String, String> entry : subjectDNItem.entrySet()) {
        dict.setItem(entry.getKey(), new COSString(entry.getValue()));
      }
      subjectDNDict.add(dict);
    }
    dictionary.setItem(COSName.SUBJECT_DN, COSArrayList.converterToCOSArray(subjectDNDict));
  }

  /**
   * Returns list of key usages of certificate strings where each string is 9
   * characters long and each character is one of these values {0, 1, X} 0 for
   * must not set, 1 for must set, X for don't care. each index in the string
  ...
  public List<String> getKeyUsage() {
    final COSBase base = dictionary.getDictionaryObject(COSName.KEY_USAGE);
    if (base instanceof COSArray) {
      final COSArray array = (COSArray) base;
  ...
  /**
   * (Optional; PDF 1.7) A List of ASCII strings, where each string specifies an
   * acceptable key-usage extension that must be present in the signing
   * certificate. Multiple strings specify a range of acceptable key-usage
   * extensions; where each string 9 characters long and each character is one of
   * these values {0, 1, X} 0 for must not set, 1 for must set, X for don't care.
   * each index in the string represents a key usage:
   * <ol>
   * <li>digitalSignature</li>
   * <li>non-Repudiation</li>
   * <li>keyEncipherment</li>
   ...
   * </ol>
   *
   * @param keyUsageExtensions list of ASCII strings that consists only of {0, 1,
   *                           X}
   */
  public void setKeyUsage(final List<String> keyUsageExtensions) {
    dictionary.setItem(COSName.KEY_USAGE, COSArrayList.converterToCOSArray(keyUsageExtensions));
  }

  /**
   * (Optional; PDF 1.7) specifies an acceptable key-usage extension that must be
   * presennt in the signing certificate for works like {@link #setKeyUsage(List)}
   * but takes only one string
   *
  ...
  public void addKeyUsage(final String keyUsageExtension) {
    ...
    final COSString string = new COSString(keyUsageExtension);
    array.add(string);
  ...
  public List<byte[]> getIssuer() {
    final COSBase base = dictionary.getDictionaryObject(COSName.ISSUER);
    if (base instanceof COSArray) {
      final COSArray array = (COSArray) base;
      return PDSeedValueCertificate.getListOfByteArraysFromCOSArray(array);
    }
  ...
  /**
   * (Optional) A list of array of bytes containing DER-encoded X.509v3
   * certificates of acceptable issuers. If the signer’s certificate chains up to
   * any of the specified issuers (either directly or indirectly), the certificate
   * is considered acceptable for signing.
   *
   * @param issuers A list of byte array containing DER-encoded X.509v3
   *                certificates
   */
  public void setIssuer(final List<byte[]> issuers) {
    final COSArray array = new COSArray();
    for (final byte[] issuer : issuers) {
  ...
  /**
   * array of bytes containing DER-encoded X.509v3 certificates of acceptable
   * issuers. If the signer’s certificate chains up to any of the specified
   * issuers (either directly or indirectly), the certificate is considered
  ...
  public void addIssuer(final byte[] issuer) {
    ...
    final COSString string = new COSString(issuer);
    array.add(string);
    dictionary.setItem(COSName.ISSUER, array);
  }
  ...
  /**
   * Returns A list of array of bytes that contain Object Identifiers (OIDs) of
   * the certificate policies that must be present in the signing certificate
   */
  public List<byte[]> getOID() {
    final COSBase base = dictionary.getDictionaryObject(COSName.OID);
    if (base instanceof COSArray) {
      final COSArray array = (COSArray) base;
      return PDSeedValueCertificate.getListOfByteArraysFromCOSArray(array);
    }
  ...
  /**
   * (Optional) A list of byte arrays that contain Object Identifiers (OIDs) of
   * the certificate policies that must be present in the signing certificate.
   * This field is only applicable if the value of Issuer is not empty.
   *
   * @param oidByteStrings list of byte arrays that contain OIDs
   */
  public void setOID(final List<byte[]> oidByteStrings) {
    final COSArray array = new COSArray();
    for (final byte[] oid : oidByteStrings) {
  ...
  public void addOID(final byte[] oid) {
    ...
    final COSString string = new COSString(oid);
    array.add(string);
    dictionary.setItem(COSName.OID, array);
  }
  ...
  /**
   * A name indicating the usage of the URL entry. There are standard uses and
   * there can be implementation-specific use for this URL. The following value
   * specifies a valid standard usage:
   * <ul>
   * <li>Browser, The URL references content that should be displayed in a web
   * browser to allow enrolling for a new credential if a matching credential is
   * not found. The Ff attribute’s URL bit is ignored for this usage.</li>
   * <li>ASSP, The URL references a signature web service that can be used for
   * server-based signing. If the Ff attribute’s URL bit indicates that this is a
   * required constraint, this implies that the credential used when signing must
   * come from this server.</li>
   * </ul>
   *
   ...
   * come from this server.</li>
   * </ul>
   * Third parties can extend the use of this attribute with their own attribute
   * values, which must conform to the guidelines specified in <a href=
   * "http://www.adobe.com/content/dam/acom/en/devnet/pdf/PDF32000_2008.pdf#page=681">PDF
   * Spec 1.7 Appendix E (PDF Name Registry)</a> if urlType is not set the default
   * is Browser for URL
   *
