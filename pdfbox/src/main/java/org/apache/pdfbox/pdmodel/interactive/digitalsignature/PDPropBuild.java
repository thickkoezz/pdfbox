...
/**
 * A signature build dictionary as specified in the PDF Signature Build
 * Dictionary Specification.
 *
 * The signature build properties dictionary provides signature properties for
 * the software application that was used to create the signature.
 *
 * @see <a href=
...
public class PDPropBuild implements COSObjectable {
  ...
  /**
   * A build data dictionary for the signature handler that was used to create the
   * parent signature.
   *
  ...
  public PDPropBuildDataDict getFilter() {
    ...
    final COSDictionary filterDic = (COSDictionary) dictionary.getDictionaryObject(COSName.FILTER);
    if (filterDic != null) {
  ...
  /**
   * Set the build data dictionary for the signature handler. This entry is
   * optional but is highly recommended for the signatures.
   *
  ...
  /**
   * A build data dictionary for the PubSec software module that was used to
  ...
  public PDPropBuildDataDict getPubSec() {
    ...
    final COSDictionary pubSecDic = (COSDictionary) dictionary.getDictionaryObject(COSName.PUB_SEC);
    if (pubSecDic != null) {
  ...
  /**
   * A build data dictionary for the viewing application software module that was
  ...
  public PDPropBuildDataDict getApp() {
    ...
    final COSDictionary appDic = (COSDictionary) dictionary.getDictionaryObject(COSName.APP);
    if (appDic != null) {
