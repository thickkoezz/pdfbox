...
public class PDExtendedGraphicsState implements COSObjectable {
    ...
    for (final COSName key : dict.keySet()) {
      if (key.equals(COSName.LW)) {
        ...
        if (softmask != null) {
          ...
          // https://bugs.ghostscript.com/show_bug.cgi?id=691157#c7 for a good
          // explanation.
        ...
        if (dict.containsKey(COSName.TR2)) {
          // "If both TR and TR2 are present in the same graphics state parameter
          // dictionary,
  ...
  /**
   * Returns the provided default value in case 'standard' valu is
   * <code>null</code>. To be used in cases unboxing may lead to a NPE.
   *
   ...
   */
  private float defaultIfNull(final Float _value, final float _default) {
  ...
  /**
   * This will get the alpha source flag (“alpha is shape”), that specifies
   ...
   *
   * @return The transfer function. According to the PDF specification, this is
   *         either a single function (which applies to all process colorants) or
   *         an array of four functions (which apply to the process colorants
   *         individually). The name Identity may be used to represent the
   *         identity function.
   */
   ...
   *
   * @param transfer The transfer function. According to the PDF specification,
   *                 this is either a single function (which applies to all
   *                 process colorants) or an array of four functions (which apply
   *                 to the process colorants individually). The name Identity may
   *                 be used to represent the identity function.
   */
   ...
   *         an array of four functions (which apply to the process colorants
   *         individually). The name Identity may be used to represent the
   *         identity function, and the name Default denotes the transfer function
   ...
   *
   * @param transfer2 The transfer function. According to the PDF specification,
   *                  this is either a single function (which applies to all
   *                  process colorants) or an array of four functions (which
   *                  apply to the process colorants individually). The name
   *                  Identity may be used to represent the identity function, and
   *                  the name Default denotes the transfer function that was in
   *                  effect at the start of the page.
   */
