...
public abstract class PDButton extends PDTerminalField {
  ...
  /**
   * A Ff flag. If set, radio buttons individual fields, using the same value for
   * the on state will turn on and off in unison.
   */
   ...
   */
  public PDButton(final PDAcroForm acroForm) {
    super(acroForm);
  ...
  public boolean isPushButton() {
    return getCOSObject().getFlag(COSName.FF, PDButton.FLAG_PUSHBUTTON);
  }
   ...
   */
  public void setPushButton(final boolean pushbutton) {
    getCOSObject().setFlag(COSName.FF, PDButton.FLAG_PUSHBUTTON, pushbutton);
  }
  ...
  public boolean isRadioButton() {
    return getCOSObject().getFlag(COSName.FF, PDButton.FLAG_RADIO);
  }

  /**
   * Set the radio button bit.
   *
   * @param radiobutton if true the button field is treated as a radio button
   *                    field.
   */
  public void setRadioButton(final boolean radiobutton) {
    getCOSObject().setFlag(COSName.FF, PDButton.FLAG_RADIO, radiobutton);
  }
   ...
   *
   * <p>
   * Off is the default value which will also be returned if the value hasn't been
   * set at all.
   *
  ...
  /**
   * Set the selected option given its name, and try to update the visual
   * appearance.
   *
  ...
  public void setValue(final String value) throws IOException {
    ...
    final boolean hasExportValues = getExportValues().size() > 0;
   ...
   * <ul>
   * <li>hold values in non-Latin writing systems as name objects, which represent
   * the field value, are limited to PDFDocEncoding</li>
   * <li>allow radio buttons having the same export value to be handled
   * independently</li>
   * </ul>
   *
   * @return List containing all possible export values. If there is no /Opt entry
   ...
   *
   * @see #getExportValues()
   * @param values List containing all possible export values. Supplying null or
   *               an empty list will remove the Opt entry.
   */
   ...
   *
   * <p>
   * The On value could be an arbitrary string as long as it is within the
   * limitations of a PDF name object. The Off value shall always be 'Off'. If not
   * set or not part of the normal appearance keys 'Off' is the default
   * </p>
   *
   * @return the potential values setting the check box to the On state. If an
   *         empty Set is returned there is no appearance definition.
   */
   ...
   * @throws IllegalArgumentException if the value is not a valid option.
   */
  void checkValue(final String value) throws IllegalArgumentException {
    final Set<String> onValues = getOnValues();
    if (COSName.Off.getName().compareTo(value) != 0 && !onValues.contains(value))
      throw new IllegalArgumentException("value '" + value + "' is not a valid option for the field "
          + getFullyQualifiedName() + ", valid values are: " + onValues + " and " + COSName.Off.getName());
  }
