...
public class PDCIDFontType0 extends PDCIDFont {
  ...
  public PDCIDFontType0(final COSDictionary fontDictionary, final PDType0Font parent) throws IOException {
    ...
    if (bytes != null && bytes.length > 0 && (bytes[0] & 0xff) == '%') {
      ...
      PDCIDFontType0.LOG.warn("Found PFB but expected embedded CFF font " + fd.getFontName());
      fontIsDamaged = true;
    } else if (bytes != null) {
      ...
      try {
        ...
        PDCIDFontType0.LOG.error("Can't read the embedded CFF font " + fd.getFontName(), e);
        fontIsDamaged = true;
      }
    }
    ...
    if (cffFont != null) {
      ...
      if (mapping.isFallback()) {
        PDCIDFontType0.LOG.warn("Using fallback " + font.getName() + " for CID-keyed font " + getBaseFont());
      }
      isEmbedded = false;
    ...
    if (fontMatrix == null) {
      ...
      if (cidFont != null) {
        ...
        try {
          ...
          PDCIDFontType0.LOG.debug("Couldn't get font matrix - returning default value", e);
  ...
  private BoundingBox generateBoundingBox() {
    ...
    if (cidFont != null)
      ...
      try {
        ...
        PDCIDFontType0.LOG.debug("Couldn't get font bounding box - returning default value", e);
  ...
  /**
   * Returns the embedded CFF CIDFont, or null if the substitute is not a CFF
   * font.
   */
  ...
  /**
   * Returns the Type 2 charstring for the given CID, or null if the substituted
   * font does not contain Type 2 charstrings.
   *
   * @param cid CID
  ...
  /**
   * Returns the name of the glyph with the given character code. This is done by
   * looking up the code in the parent font's ToUnicode map and generating a glyph
   * name from that.
   */
  private String getGlyphName(final int code) throws IOException {
  ...
  /**
   * Returns the CID for the given character code. If not found then CID 0 is
   * returned.
   *
  ...
  private class ByteSource implements CFFParser.ByteSource {
    ...
    public byte[] getBytes() throws IOException {
      final PDStream ff3Stream = getFontDescriptor().getFontFile3();
