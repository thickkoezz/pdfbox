...
public final class PDImageXObject extends PDXObject implements PDImage {
  ...
  // initialize to MAX_VALUE as we prefer lower subsampling when keeping/replacing
  // cache.
   ...
   * internal PDFBox use and is not for PDF generation. Users who want to create
   ...
   * stream. This constructor is for internal PDFBox use and is not for PDF
   * generation. Users who want to create images should look at
   * {@link #createFromFileByExtension(File, PDDocument) }.
   *
   * @param document         the current document
   ...
   */
  public PDImageXObject(final PDDocument document, final InputStream encodedStream, final COSBase cosFilter,
      final int width, final int height, final int bitsPerComponent, final PDColorSpace initColorSpace)
      throws IOException {
  ...
  /**
   * Creates an Image XObject with the given stream as its contents and current
   * color spaces. This constructor is for internal PDFBox use and is not for PDF
   * generation. Users who want to create images should look at
   * {@link #createFromFileByExtension(File, PDDocument) }.
   *
   ...
   */
  private static COSStream createRawStream(final PDDocument document, final InputStream rawInput) throws IOException {
   ...
   * {@link #createFromFileByExtension(File, PDDocument)} for more details.
   *
   * @param imagePath the image file path.
   * @param doc       the document that shall use this PDImageXObject.
   * @return a PDImageXObject.
   * @throws IOException if there is an error when reading the file or creating
   *                     the PDImageXObject, or if the image type is not
   *                     supported.
   */
  ...
  /**
   * Create a PDImageXObject from an image file. The file format is determined by
   * the file name suffix. The following suffixes are supported: jpg, jpeg, tif,
   * tiff, gif, bmp and png. This is a convenience method that calls
   * {@link JPEGFactory#createFromStream}, {@link CCITTFactory#createFromFile} or
   * {@link ImageIO#read} combined with {@link LosslessFactory#createFromImage}.
   * (The later can also be used to create a PDImageXObject from a BufferedImage).
   *
   * @param file the image file.
   * @param doc  the document that shall use this PDImageXObject.
   * @return a PDImageXObject.
   * @throws IOException              if there is an error when reading the file
   *                                  or creating the PDImageXObject.
  ...
  /**
   * Create a PDImageXObject from an image file. The file format is determined by
   * the file content. The following file types are supported: jpg, jpeg, tif,
   * tiff, gif, bmp and png. This is a convenience method that calls
   * {@link JPEGFactory#createFromStream}, {@link CCITTFactory#createFromFile} or
   * {@link ImageIO#read} combined with {@link LosslessFactory#createFromImage}.
   * (The later can also be used to create a PDImageXObject from a BufferedImage).
  ...
  public static PDImageXObject createFromFileByContent(final File file, final PDDocument doc) throws IOException {
    ...
    if (fileType.equals(FileType.TIFF)) {
      try {
        ...
        PDImageXObject.LOG.debug("Reading as TIFF failed, setting fileType to PNG", ex);
  ...
  /**
   * Create a PDImageXObject from bytes of an image file. The file format is
   * determined by the file content. The following file types are supported: jpg,
   * jpeg, tif, tiff, gif, bmp and png. This is a convenience method that calls
   * {@link JPEGFactory#createFromByteArray}, {@link CCITTFactory#createFromFile}
   * or {@link ImageIO#read} combined with
   * {@link LosslessFactory#createFromImage}. (The later can also be used to
   * create a PDImageXObject from a BufferedImage).
   *
   ...
   * @throws IllegalArgumentException if the image type is not supported.
   */
  public static PDImageXObject createFromByteArray(final PDDocument document, final byte[] byteArray, final String name)
      throws IOException {
    FileType fileType;
    try {
   ...
   *
   * @return this object's key the structural parent tree or -1 if there isn't
   *         any.
   */
  ...
  public BufferedImage getImage(final Rectangle region, final int subsampling) throws IOException {
    ...
    if (region == null && subsampling <= cachedImageSubsampling) {
      // only cache full-image renders, and prefer lower subsampling frequency, as
      // lower
  ...
  /**
   * Returns an RGB buffered image containing the opaque image stream without any
   * masks applied. If this Image XObject is a mask then the buffered image will
  ...
  private BufferedImage applyMask(BufferedImage image, BufferedImage mask, final boolean isSoft, final float[] matte)
      throws IOException {
    if (mask == null)
      return image;
  ...
  /**
   * Returns the Mask Image XObject associated with this image, or null if there
   * is none.
   *
  ...
  /**
   * Returns the color key mask array associated with this image, or null if there
   * is none.
   *
   * @return Mask Image XObject
   */
  ...
  /**
   * Returns the Soft Mask Image XObject associated with this image, or null if
   * there is none.
   *
  ...
  public String getSuffix() {
    ...
    if (filters == null)
      ...
      PDImageXObject.LOG.warn("getSuffix() returns null, filters: " + filters);
      return null;
    }
  }
}
