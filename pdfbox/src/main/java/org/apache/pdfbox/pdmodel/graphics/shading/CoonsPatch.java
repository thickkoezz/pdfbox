...
class CoonsPatch extends Patch {
  ...
  // adjust the 12 control points to 4 groups, each group defines one edge of a
  // patch
  ...
  /*
   * dividing a patch into a grid, return a matrix of the coordinate and color at
   * the crossing points of the grid, the rule to calculate the coordinate is
   * defined in page 195 of PDF32000_2008.pdf, the rule to calculate the
   * cooresponding color is bilinear interpolation
   */
  private CoordinateColorPair[][] getPatchCoordinatesColor(final CubicBezierCurve c1, final CubicBezierCurve c2,
      final CubicBezierCurve d1, final CubicBezierCurve d2) {
