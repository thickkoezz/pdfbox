...
public final class PDPageContentStream extends PDAbstractContentStream implements Closeable {
  ...
  /**
   * Create a new PDPage content stream. This constructor overwrites all existing
   * content streams of this page.
   *
   ...
   * @param document      The document the page is part of.
   * @param sourcePage    The page to write the contents to.
   * @param appendContent Indicates whether content will be overwritten, appended
   *                      or prepended.
   ...
   * @param compress      Tell if the content stream should compress the page
   *                      contents.
   * @param resetContext  Tell if the graphic context should be reset. This is
   *                      only relevant when the appendContent parameter is set to
   *                      {@link AppendMode#APPEND}. You should use this when
   *                      appending to an existing stream, because the existing
   *                      stream may have changed graphic properties (e.g.
   *                      scaling, rotation).
   * @throws IOException If there is an error writing to the page contents.
   */
  public PDPageContentStream(final PDDocument document, final PDPage sourcePage, final AppendMode appendContent,
  ...
  private PDPageContentStream(final PDDocument document, final PDPage sourcePage, final AppendMode appendContent,
      final boolean compress, final boolean resetContext, final PDStream stream, final PDResources resources)
      throws IOException {
    ...
    if (!appendContent.isOverwrite() && sourcePage.hasContents()) {
      ...
      if (resetContext) {
        ...
        final OutputStream prefixOut = prefixStream.createOutputStream();
   ...
   */
  public PDPageContentStream(final PDDocument doc, final PDAppearanceStream appearance,
  ...
  /**
   * The Td operator. A current text matrix will be replaced with a new one (1 0 0
   * 1 x y).
   *
   ...
   */
  @Deprecated
  public void setTextMatrix(final double a, final double b, final double c, final double d, final double e,
   ...
   *
   * @throws IOException If there is an error writing to the stream.
   * @deprecated Use {@link #drawImage(PDInlineImage, float, float, float, float)}
   *             instead.
   */
  @Deprecated
  public void drawInlineImage(final PDInlineImage inlineImage, final float x, final float y, final float width,
      final float height) throws IOException {
    drawImage(inlineImage, x, y, width, height);
  }

  /**
   * Draw an xobject(form or image) at the x,y coordinates and a certain width and
   * height.
   *
   ...
   */
  @Deprecated
  public void drawXObject(final PDXObject xobject, final float x, final float y, final float width, final float height)
      throws IOException {
   ...
   * @deprecated Use {@link #drawImage(PDImageXObject, Matrix)
   *             drawImage(PDImageXObject, Matrix)} or
   *             {@link #drawForm(PDFormXObject) drawForm(PDFormXObject)} with
   ...
   */
  @Deprecated
  public void concatenate2CTM(final double a, final double b, final double c, final double d, final double e,
      final double f) throws IOException {
   ...
   */
  @Deprecated
  public void addBezier312(final float x1, final float y1, final float x2, final float y2, final float x3,
  ...
  /**
   * Draw a line on the page using the current stroking color and the current line
   * width.
   *
   * @param xStart The start x coordinate.
   * @param yStart The start y coordinate.
   ...
   * @throws IllegalStateException If the method was called within a text block.
   * @deprecated Use {@link #moveTo moveto(xStart,yStart)} followed by
   *             {@link #lineTo lineTo(xEnd,yEnd)} followed by {@link #stroke
   *             stroke()}.
   */
  @Deprecated
   ...
   * @throws IllegalArgumentException If the parameter is not a valid winding
   *                                  rule.
