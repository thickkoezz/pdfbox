...
public abstract class BlendMode {
  ...
  public static final SeparableBlendMode COMPATIBLE = BlendMode.NORMAL;
  ...
  // these maps *must* come after the BlendMode.* constant declarations, otherwise
  // their values would be null
  private static final Map<COSName, BlendMode> BLEND_MODES = BlendMode.createBlendModeMap();
  private static final Map<BlendMode, COSName> BLEND_MODE_NAMES = BlendMode.createBlendModeNamesMap();
  ...
  public static BlendMode getInstance(final COSBase cosBlendMode) {
    ...
    if (cosBlendMode instanceof COSName) {
      result = BlendMode.BLEND_MODES.get(cosBlendMode);
      ...
      for (int i = 0; i < cosBlendModeArray.size(); i++) {
        result = BlendMode.BLEND_MODES.get(cosBlendModeArray.getObject(i));
        if (result != null) {
          break;
        }
      }
  ...
  public static COSName getCOSName(final BlendMode bm) {
    return BlendMode.BLEND_MODE_NAMES.get(bm);
  }
  ...
  private static void getSaturationRGB(final float[] srcValues, final float[] dstValues, final float[] result) {
    ...
    final int rd = BlendMode.get255Value(dstValues[0]);
    final int gd = BlendMode.get255Value(dstValues[1]);
    final int bd = BlendMode.get255Value(dstValues[2]);
    final int rs = BlendMode.get255Value(srcValues[0]);
    final int gs = BlendMode.get255Value(srcValues[1]);
    final int bs = BlendMode.get255Value(srcValues[2]);
