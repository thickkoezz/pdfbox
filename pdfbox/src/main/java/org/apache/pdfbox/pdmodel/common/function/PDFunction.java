...
public abstract class PDFunction implements COSObjectable {
   ...
   * function 4 - PostScript calculator function
   *
  ...
  /**
   * This will get the number of output parameters that have a range specified. A
   * range for output parameters is optional so this may return zero for a
   * function that does have output parameters, this will simply return the number
   * that have the range specified.
   *
   ...
   *
   * @param input The array of input values for the function. In many cases will
   *              be an array of a single value, but not always.
   *
   * @return The of outputs the function returns based on those inputs. In many
   *         cases will be an array of a single value, but not always.
   *
   * @throws IOException an IOExcpetion is thrown if something went wrong
   *                     processing the function.
   */
  public abstract float[] eval(float[] input) throws IOException;

  /**
   * Returns all ranges for the output values as COSArray . Required for type 0
   * and type 4 functions
   *
  ...
  /**
   * Returns all domains for the input values as COSArray. Required for all
   * function types.
   *
  ...
  /**
   * For a given value of x, interpolate calculates the y value on the line
   * defined by the two points (xRangeMin , xRangeMax ) and (yRangeMin , yRangeMax
   * ).
   *
   ...
   */
  protected float interpolate(final float x, final float xRangeMin, final float xRangeMax, final float yRangeMin,
      final float yRangeMax) {
