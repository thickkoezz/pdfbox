...
public class PDLineAppearanceHandler extends PDAbstractAppearanceHandler {
  ...
  public void generateNormalAppearance() {
    ...
    rect.setUpperRightX(
        Math.max(maxX + Math.max(lineEndingSize * 10, Math.abs(llo + ll + lle)), rect.getUpperRightX()));
    rect.setUpperRightY(
        Math.max(maxY + Math.max(lineEndingSize * 10, Math.abs(llo + ll + lle)), rect.getUpperRightY()));
    ...
    try (PDAppearanceContentStream cs = getNormalAppearanceAsContentStream()) {
      ...
      // empty color array results in an invisible line ("n" operator) but the rest is
      // visible
      ...
      // the endpoints of the leader lines rather than the endpoints of the line
      // itself.
      ...
      if (annotation.hasCaption() && !contents.isEmpty()) {
        ...
        // when the text is so long that it would cross arrows, but we ignore this for
        // now
        ...
        try {
          contentLength = font.getStringWidth(annotation.getContents()) / 1000 * PDLineAppearanceHandler.FONT_SIZE;
          ...
          PDLineAppearanceHandler.LOG.error("line text '" + annotation.getContents() + "' can't be shown", ex);
        }
        ...
        // draw the line horizontally, using the rotation CTM to get to correct final
        // position
        // that's the easiest way to calculate the positions for the line before and
        // after inline caption
        ...
        if (PDAbstractAppearanceHandler.SHORT_STYLES.contains(annotation.getEndPointEndingStyle())) {
        ...
        if (contentLength > 0) {
          cs.beginText();
          cs.setFont(font, PDLineAppearanceHandler.FONT_SIZE);
      ...
      // paint the styles here and not before showing the text, or the text would
      // appear
      ...
      if (!PDAnnotationLine.LE_NONE.equals(annotation.getStartPointEndingStyle())) {
        ...
        if (PDAbstractAppearanceHandler.ANGLED_STYLES.contains(annotation.getStartPointEndingStyle())) {
          ...
          // because the lines do not always go from (x1,y1) to (x2,y2) due to the leader
          // lines
          // when the "y" value above is not 0.
          // We use the angle we already know and the distance y to translate to the new
          // coordinate.
          final float xx1 = x1 - (float) (y * Math.sin(angle));
          final float yy1 = y1 + (float) (y * Math.cos(angle));
          drawStyle(annotation.getStartPointEndingStyle(), cs, xx1, yy1, lineEndingSize, hasStroke, hasBackground,
              false);
        }
        cs.restoreGraphicsState();
      }
      ...
      PDLineAppearanceHandler.LOG.error(ex);
    }
  }
