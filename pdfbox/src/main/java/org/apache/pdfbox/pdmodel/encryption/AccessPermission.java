...
public class AccessPermission {
  ...
  private int bytes = AccessPermission.DEFAULT_PERMISSIONS;
  ...
  public AccessPermission() {
    bytes = AccessPermission.DEFAULT_PERMISSIONS;
  }
  ...
  public boolean isOwnerPermission() {
    return canAssembleDocument() && canExtractContent() && canExtractForAccessibility() && canFillInForm()
        && canModify() && canModifyAnnotations() && canPrint() && canPrintDegraded();
  }
  ...
  /**
   * This returns an integer representing the access permissions. This integer can
   * be used for public key encryption. This format is not documented in the PDF
   * specifications but is necessary for compatibility with Adobe Acrobat and
   * Adobe Reader.
   *
  ...
  /**
   * The returns an integer representing the access permissions. This integer can
   * be used for standard PDF encryption as specified in the PDF specifications.
   *
  ...
  public boolean canPrint() {
    return isPermissionBitOn(AccessPermission.PRINT_BIT);
  }
  ...
  public void setCanPrint(final boolean allowPrinting) {
    if (!readOnly) {
      setPermissionBit(AccessPermission.PRINT_BIT, allowPrinting);
    }
  }
  ...
  public boolean canModify() {
    return isPermissionBitOn(AccessPermission.MODIFICATION_BIT);
  }
   ...
   * This method will have no effect if the object is in read only mode.
   *
   * @param allowModifications A boolean determining if the user can modify the
   *                           document.
   */
  public void setCanModify(final boolean allowModifications) {
    if (!readOnly) {
      setPermissionBit(AccessPermission.MODIFICATION_BIT, allowModifications);
    }
  }
  ...
  public boolean canExtractContent() {
    return isPermissionBitOn(AccessPermission.EXTRACT_BIT);
  }
  ...
  public void setCanExtractContent(final boolean allowExtraction) {
    if (!readOnly) {
      setPermissionBit(AccessPermission.EXTRACT_BIT, allowExtraction);
    }
  }
   ...
   * Note that if {@link #canFillInForm() canFillInForm()} returns true, it is
   * still possible to fill in interactive forms (including signature fields) even
   * if this method here returns false.
   *
   * @return true If supplied with the user password they are allowed to modify
   *         annotations.
   */
  public boolean canModifyAnnotations() {
    return isPermissionBitOn(AccessPermission.MODIFY_ANNOTATIONS_BIT);
  }

  /**
   * Set if the user can add or modify text annotations and fill in interactive
   * forms fields and, if {@link #canModify() canModify()} returns true, create or
   * modify interactive form fields (including signature fields). Note that if
   * {@link #canFillInForm() canFillInForm()} returns true, it is still possible
  ...
  public void setCanModifyAnnotations(final boolean allowAnnotationModification) {
    if (!readOnly) {
      setPermissionBit(AccessPermission.MODIFY_ANNOTATIONS_BIT, allowAnnotationModification);
    }
  }
  ...
  public boolean canFillInForm() {
    return isPermissionBitOn(AccessPermission.FILL_IN_FORM_BIT);
  }
   ...
   * This method will have no effect if the object is in read only mode.
   *
   * @param allowFillingInForm A boolean determining if the user can fill in
   *                           interactive forms.
   */
  public void setCanFillInForm(final boolean allowFillingInForm) {
    if (!readOnly) {
      setPermissionBit(AccessPermission.FILL_IN_FORM_BIT, allowFillingInForm);
    }
  }
  ...
  public boolean canExtractForAccessibility() {
    return isPermissionBitOn(AccessPermission.EXTRACT_FOR_ACCESSIBILITY_BIT);
  }
  ...
  public void setCanExtractForAccessibility(final boolean allowExtraction) {
    if (!readOnly) {
      setPermissionBit(AccessPermission.EXTRACT_FOR_ACCESSIBILITY_BIT, allowExtraction);
    }
  }
  ...
  public boolean canAssembleDocument() {
    return isPermissionBitOn(AccessPermission.ASSEMBLE_DOCUMENT_BIT);
  }
   ...
   * This method will have no effect if the object is in read only mode.
   *
   * @param allowAssembly A boolean determining if the user can assemble the
   *                      document.
   */
  public void setCanAssembleDocument(final boolean allowAssembly) {
    if (!readOnly) {
      setPermissionBit(AccessPermission.ASSEMBLE_DOCUMENT_BIT, allowAssembly);
    }
  }
  ...
  public boolean canPrintDegraded() {
    return isPermissionBitOn(AccessPermission.DEGRADED_PRINT_BIT);
  }
  ...
  public void setCanPrintDegraded(final boolean canPrintDegraded) {
    if (!readOnly) {
      setPermissionBit(AccessPermission.DEGRADED_PRINT_BIT, canPrintDegraded);
    }
  }

  /**
   * Locks the access permission read only (ie, the setters will have no effects).
   * After that, the object cannot be unlocked. This method is used for the
   * currentAccessPermssion of a document to avoid users to change access
   * permission.
   */
