 ...
 * A block of text.
 * <p>
 * A block of text can contain multiple paragraphs which will be treated
 * individually within the block placement.
 * </p>
 *
 */
class PlainText {
   ...
   *
   * Constructs the text block from a single value splitting into individual
   * {@link Paragraph} when a new line character is encountered.
   *
   ...
   *
   * Constructs the text block from a list of values treating each entry as an
   ...
   *
   * This is similar to {@link java.awt.font.TextAttribute} but handled
   * individually as to avoid a dependency on awt.
   *
   */
  ...
  static class Paragraph {
    ...
    List<Line> getLines(final PDFont font, final float fontSize, final float width) throws IOException {
      ...
      final float scale = fontSize / PlainText.FONTSCALE;
