...
public class PDOptionalContentProperties implements COSObjectable {
  ...
  public PDOptionalContentProperties() {
    ...
    dict.setItem(COSName.OCGS, new COSArray());
  ...
  private COSArray getOCGs() {
    ...
    if (ocgs == null) {
      ocgs = new COSArray();
      dict.setItem(COSName.OCGS, ocgs); // OCGs is required
    }
    return ocgs;
  }

  private COSDictionary getD() {
    final COSBase base = dict.getDictionaryObject(COSName.D);
  ...
  public void addGroup(final PDOptionalContentGroup ocg) {
    ...
    // By default, add new group to the "Order" entry so it appears in the user
    // interface
  ...
  public String[] getGroupNames() {
    final COSArray ocgs = (COSArray) dict.getDictionaryObject(COSName.OCGS);
  ...
  /**
   * Indicates whether a particular optional content group is found in the PDF
   * file.
   *
  ...
  /**
   * Indicates whether <em>at least one</em> optional content group with this name
   * is enabled. There may be disabled optional content groups with this name even
   ...
   * @return true if at least one group with this name already had an on or off
