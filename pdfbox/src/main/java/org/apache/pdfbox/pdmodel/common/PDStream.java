...
public class PDStream implements COSObjectable {
   ...
   * document. This method closes the InputStream.
   *
  ...
  /**
   * Constructor. Reads all data from the input stream and embeds it into the
   * document with the given filter applied. This method closes the InputStream.
   *
   * @param doc    The document that will hold the stream.
   * @param input  The stream parameter.
  ...
  /**
   * Constructor. Reads all data from the input stream and embeds it into the
   * document with the given filters applied. This method closes the InputStream.
   *
   * @param doc     The document that will hold the stream.
   * @param input   The stream parameter.
  ...
  /**
   * Constructor. Reads all data from the input stream and embeds it into the
   * document with the given filters applied, if any. This method closes the
   * InputStream.
   */
  ...
  /**
   * If there are not compression filters on the current stream then this will add
   * a compression filter, flate compression for example.
   *
   * @deprecated This method is inefficient. To copying an existing InputStream,
   *             use {@link #PDStream(PDDocument, InputStream, COSName)} instead,
   *             with COSName.FLATE_DECODE as the final argument.
   *
   *             Otherwise, to write new compressed data, use
   *             {@link #createOutputStream(COSName)}, with COSName.FLATE_DECODE
   *             as the argument.
   */
  @Deprecated
  public void addCompression() {
    ...
    if (filters == null) {
      if (stream.getLength() > 0) {
        ...
        try {
          final byte[] bytes = IOUtils.toByteArray(stream.createInputStream());
   ...
   * useful when doing images, ie filters = [flate,dct], we want to remove flate
   * but leave dct
   *
  ...
  /**
   * This will get the length of the filtered/compressed stream. This is readonly
   * in the PD Model and will be managed by this class.
   *
  ...
  public void setFilters(final List<COSName> filters) {
    final COSBase obj = COSArrayList.converterToCOSArray(filters);
    stream.setItem(COSName.FILTER, obj);
  }

  /**
   * Get the list of decode parameters. Each entry in the list will refer to an
  ...
  /**
   * This will get the file specification for this stream. This is only required
   * for external files.
   *
   ...
   * @throws IllegalStateException if the value of the metadata entry is different
   *                               from a stream or null
   */
  ...
  public int getDecodedStreamLength() {
    return stream.getInt(COSName.DL);
  }
  ...
  public void setDecodedStreamLength(final int decodedStreamLength) {
    stream.setInt(COSName.DL, decodedStreamLength);
  }
}
