...
public class PDDocumentCatalog implements COSObjectable {
  ...
  /**
   * Constructor. Internal PDFBox use only! If you need to get the document
   * catalog, call {@link PDDocument#getDocumentCatalog()}.
   *
  ...
  /**
   * Get the documents AcroForm. This will return null if no AcroForm is part of
   * the document.
   *
  ...
  /**
   * Get the viewer preferences associated with this document or null if they do
   * not exist.
   *
  ...
  /**
   * @return The named destinations dictionary for this document or null if none
   *         exists.
   */
   ...
   * @throws IOException if there is an error creating the PDPageDestination
   *                     object.
   */
  public PDPageDestination findNamedDestinationPage(final PDNamedDestination namedDest) throws IOException {
  ...
  /**
   * Get info about doc's usage of tagged features. This will return null if there
   * is no information.
   *
  ...
  /**
   * Add an OutputIntent to the list. If there is not OutputIntent, the list is
  ...
  public PageMode getPageMode() {
    ...
    if (mode != null) {
      try {
        ...
        PDDocumentCatalog.LOG.debug("Invalid PageMode used '" + mode + "' - setting to PageMode.USE_NONE", e);
  ...
  /**
   * Sets the optional content properties dictionary. The document version is
   * incremented to 1.5 if lower.
   *
