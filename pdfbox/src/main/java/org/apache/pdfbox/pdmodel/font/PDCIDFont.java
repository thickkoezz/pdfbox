...
/**
 * A CIDFont. A CIDFont is a PDF object that contains information about a
 * CIDFont program. Although its Type value is Font, a CIDFont is not actually a
 * font.
 *
 * <p>
 * It is not usually necessary to use this class directly, prefer
 * {@link PDType0Font}.
 *
...
public abstract class PDCIDFont implements COSObjectable, PDFontLike, PDVectorFont {
   ...
   */
  PDCIDFont(final COSDictionary fontDictionary, final PDType0Font parent) throws IOException {
    dict = fontDictionary;
    this.parent = parent;
    readWidths();
  ...
  private void readWidths() {
    ...
    if (wBase instanceof COSArray) {
      ...
      int counter = 0;
      while (counter < size) {
        final COSNumber firstCode = (COSNumber) wArray.getObject(counter++);
        ...
        if (next instanceof COSArray) {
          ...
          for (int i = 0; i < arraySize; i++) {
            final COSNumber width = (COSNumber) array.getObject(i);
            widths.put(startRange + i, width.floatValue());
          }
        } else {
          final COSNumber secondCode = (COSNumber) next;
          final COSNumber rangeWidth = (COSNumber) wArray.getObject(counter++);
          final int startRange = firstCode.intValue();
  ...
  /**
   * This will get the default width. The default value for the default width is
   * 1000.
   *
  ...
  public float getWidth(final int code) throws IOException {
    // these widths are supposed to be consistent with the actual widths given in
    // the CIDFont
    // program, but PDFBOX-563 shows that when they are not, Acrobat overrides the
    // embedded
  ...
  /**
   * Returns the CIDSystemInfo, or null if it is missing (which isn't allowed but
   * could happen).
   */
