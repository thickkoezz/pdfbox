...
public class PDFreeTextAppearanceHandler extends PDAbstractAppearanceHandler {
  ...
  public void generateNormalAppearance() {
    ...
    try (PDAppearanceContentStream cs = getNormalAppearanceAsContentStream(true)) {
      ...
      for (int i = 0; i < pathsArray.length / 2; ++i) {
        ...
        if (i == 0) {
          if (PDAbstractAppearanceHandler.SHORT_STYLES.contains(annotation.getLineEndingStyle())) {
      ...
      // paint the styles here and after line(s) draw, to avoid line crossing a filled
      // shape
      if (PDAnnotationFreeText.IT_FREE_TEXT_CALLOUT.equals(annotation.getIntent())
          // check only needed to avoid q cm Q if LE_NONE
          && !PDAnnotationLine.LE_NONE.equals(annotation.getLineEndingStyle()) && pathsArray.length >= 4) {
        final float x2 = pathsArray[2];
        final float y2 = pathsArray[3];
        final float x1 = pathsArray[0];
        final float y1 = pathsArray[1];
        cs.saveGraphicsState();
        if (PDAbstractAppearanceHandler.ANGLED_STYLES.contains(annotation.getLineEndingStyle())) {
      ...
      if (borderEffect != null && borderEffect.getStyle().equals(PDBorderEffectDictionary.STYLE_CLOUDY)) {
        ...
        final CloudyBorder cloudyBorder = new CloudyBorder(cs, borderEffect.getIntensity(), ab.width, getRectangle());
        ...
        cs.addRect(paddedRectangle.getLowerLeftX(), paddedRectangle.getLowerLeftY(), paddedRectangle.getWidth(),
            paddedRectangle.getHeight());
      }
      ...
      final float clipHeight = rotation == 90 || rotation == 270 ? borderBox.getWidth() - ab.width * 4
          : borderBox.getHeight() - ab.width * 4;
      final float fontSize = extractFontSize(annotation);
      ...
      final PlainTextFormatter formatter = new PlainTextFormatter.Builder(cs).style(appearanceStyle)
          .text(new PlainText(annotation.getContents())).width(width - ab.width * 4).wrapLines(true)
      ...
      if (pathsArray.length > 0) {
        ...
        // TODO when callout is used, /RD should be so that the result is the writable
        // part
      }
    } catch (final IOException ex) {
      PDFreeTextAppearanceHandler.LOG.error(ex);
    }
  }
  ...
  private PDColor extractNonStrokingColor(final PDAnnotationFreeText annotation) {
    // It could also work with a regular expression, but that should be written so
    // that
    // "/LucidaConsole 13.94766 Tf .392 .585 .93 rg" does not produce "2 .585 .93
    // rg" as result
    // Another alternative might be to create a PDDocument and a PDPage with /DA
    // content as /Content,
    ...
    try {
      // not sure if charset is correct, but we only need numbers and simple
      // characters
      final PDFStreamParser parser = new PDFStreamParser(defaultAppearance.getBytes(Charsets.US_ASCII));
      ...
      PDFreeTextAppearanceHandler.LOG.warn("Problem parsing /DA, will use default black", ex);
    }
    return strokingColor;
  }
  ...
  private float extractFontSize(final PDAnnotationFreeText annotation) {
    final String defaultAppearance = annotation.getDefaultAppearance();
    if (defaultAppearance == null)
      return 10;
    ...
    try {
      ...
      if (fontArguments.size() >= 2) {
        final COSBase base = fontArguments.get(1);
        if (base instanceof COSNumber)
          return ((COSNumber) base).floatValue();
      }
    } catch (final IOException ex) {
      PDFreeTextAppearanceHandler.LOG.warn("Problem parsing /DA, will use default 10", ex);
    }
    return 10;
  }
