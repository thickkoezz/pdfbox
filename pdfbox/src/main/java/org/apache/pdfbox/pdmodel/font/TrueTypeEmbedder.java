...
abstract class TrueTypeEmbedder implements Subsetter {
  ...
  TrueTypeEmbedder(final PDDocument document, final COSDictionary dict, final TrueTypeFont ttf,
    ...
    if (!embedSubset) {
      // full embedding
      final PDStream stream = new PDStream(document, ttf.getOriginalData(), COSName.FLATE_DECODE);
  ...
  public final void buildFontFile2(final InputStream ttfStream) throws IOException {
    ...
    // as the stream was closed within the PDStream constructor, we have to recreate
    // it
  ...
  private boolean isEmbeddingPermitted(final TrueTypeFont ttf) throws IOException {
    if (ttf.getOS2Windows() != null) {
      ...
      final int exclusive = fsType & 0x8; // bits 0-3 are a set of exclusive bits

      if ((exclusive & OS2WindowsMetricsTable.FSTYPE_RESTRICTED) == OS2WindowsMetricsTable.FSTYPE_RESTRICTED)
        // restricted License embedding
        return false;
      else if ((exclusive & OS2WindowsMetricsTable.FSTYPE_BITMAP_ONLY) == OS2WindowsMetricsTable.FSTYPE_BITMAP_ONLY)
  ...
  private boolean isSubsettingPermitted(final TrueTypeFont ttf) throws IOException {
    if (ttf.getOS2Windows() != null) {
      final int fsType = ttf.getOS2Windows().getFsType();
      if ((fsType & OS2WindowsMetricsTable.FSTYPE_NO_SUBSETTING) == OS2WindowsMetricsTable.FSTYPE_NO_SUBSETTING)
        return false;
    }
    return true;
  }
  ...
  private PDFontDescriptor createFontDescriptor(final TrueTypeFont ttf) throws IOException {
    ...
    fd.setItalic((fsSelection & (TrueTypeEmbedder.ITALIC | TrueTypeEmbedder.OBLIQUE)) != 0);
   ...
   */
  protected abstract void buildSubset(InputStream ttfSubset, String tag, Map<Integer, Integer> gidToCid)
      throws IOException;
  ...
  public String getTag(final Map<Integer, Integer> gidToCid) {
    ...
    do {
      ...
      sb.append(TrueTypeEmbedder.BASE25.charAt(mod));
      num = div;
