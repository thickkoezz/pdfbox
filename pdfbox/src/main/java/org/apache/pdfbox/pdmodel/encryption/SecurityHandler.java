...
/**
 * A security handler as described in the PDF specifications. A security handler
 * is responsible of documents protection.
 *
...
public abstract class SecurityHandler {
  ...
  protected int keyLength = SecurityHandler.DEFAULT_KEY_LENGTH;
  ...
  // PDFBOX-4453, PDFBOX-4477: Originally this was just a Set. This failed in rare
  // cases
  ...
  private final Set<COSBase> objects = Collections.newSetFromMap(new IdentityHashMap<COSBase, Boolean>());
  ...
  private void encryptData(final long objectNumber, final long genNumber, final InputStream data,
    ...
    // Determine whether we're using Algorithm 1 (for RC4 and AES-128), or 1.A (for
    // AES-256)
   ...
   * @throws IOException If there is an error reading the data.
   */
  protected void encryptDataRC4(final byte[] finalKey, final byte[] input, final OutputStream output)
      throws IOException {
    rc4.setKey(finalKey);
    rc4.write(input, output);
  }
   ...
   * @throws IOException If there is an error reading the data.
   */
  private void encryptDataAESother(final byte[] finalKey, final InputStream data, final OutputStream output,
      final boolean decrypt) throws IOException {
   ...
   * @throws IOException If there is an error reading the data.
   */
  private void encryptDataAES256(final InputStream data, final OutputStream output, final boolean decrypt)
      throws IOException {
    final byte[] iv = new byte[16];
    ...
    try {
      ...
      final SecretKeySpec keySpec = new SecretKeySpec(encryptionKey, "AES");
    ...
    try (CipherInputStream cis = new CipherInputStream(data, cipher)) {
      ...
      if (!(exception.getCause() instanceof GeneralSecurityException))
        throw exception;
      SecurityHandler.LOG.debug("A GeneralSecurityException occured when decrypting some stream data", exception);
    }
  }

  private boolean prepareAESInitializationVector(final boolean decrypt, final byte[] iv, final InputStream data,
      final OutputStream output) throws IOException {
    if (decrypt) {
      // read IV from stream
      final int ivSize = data.read(iv);
      if (ivSize == -1)
        return false;
      if (ivSize != iv.length)
        throw new IOException(
            "AES initialization vector not fully read: only " + ivSize + " bytes read instead of " + iv.length);
    } else {
  ...
  public void decrypt(final COSBase obj, final long objNum, final long genNum) throws IOException {
    if (!(obj instanceof COSString || obj instanceof COSDictionary || obj instanceof COSArray))
      return;
    // PDFBOX-4477: only cache strings and streams, this improves speed and memory
    // footprint
  ...
  public void decryptStream(final COSStream stream, final long objNum, final long genNum) throws IOException {
    ...
    if (COSName.METADATA.equals(type)) {
      ...
      try (InputStream is = stream.createRawInputStream()) {
        buf = new byte[10];
        final long isResult = is.read(buf);

        if (Long.compare(isResult, buf.length) != 0) {
          SecurityHandler.LOG.debug("Tried reading " + buf.length + " bytes but only " + isResult + " bytes read");
        }
      }
      if (Arrays.equals(buf, "<?xpacket ".getBytes(Charsets.ISO_8859_1))) {
        SecurityHandler.LOG.warn("Metadata is not encrypted, but was expected to be");
        SecurityHandler.LOG.warn("Read PDF specification about EncryptMetadata (default value: true)");
        return;
      }
    }
   ...
   * encrypted by visitFromString() in COSWriter and we don't want to encrypt it
   * twice.
   *
   * @param stream The stream to decrypt.
   * @param objNum The object number.
   ...
   */
  private void decryptDictionary(final COSDictionary dictionary, final long objNum, final long genNum)
      throws IOException {
  ...
  private void decryptString(final COSString string, final long objNum, final long genNum) throws IOException {
    ...
    try {
      ...
      SecurityHandler.LOG.error("Failed to decrypt COSString of length " + string.getBytes().length + " in object "
          + objNum + ": " + ex.getMessage(), ex);
    }
  }
