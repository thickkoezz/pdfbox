...
/**
 * A field in an interactive form. Fields may be one of four types: button,
 * text, choice, or signature.
 *
 * @author sug
 */
public abstract class PDTerminalField extends PDField {
   ...
   *         backed by the actual widget collection of the field, so adding or
   *         deleting has no effect on the PDF document until you call
   *         {@link #setWidgets(java.util.List) setWidgets()} with the modified
   *         list.
   */
  @Override
   ...
   * are multiple sub widgets associated with this field, in which case you want
   * to use getWidgets(). If the kids entry is specified, then the first entry in
   * that list will be returned.
   *
   * @return The widget that is associated with this field.
   * @deprecated Fields may have more than one widget, call {@link #getWidgets()}
   *             instead.
   */
  @Deprecated
  ...
  /**
   * Applies a value change to the field. Generates appearances if required and
   * raises events.
   *
  ...
  /**
   * Constructs appearance streams and appearance dictionaries for all widget
   * annotations. Subclasses should not call this method directly but via
   * {@link #applyChange()}.
   *
   * @throws IOException if the appearance couldn't be generated
   */
