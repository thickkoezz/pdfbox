...
class TensorPatch extends Patch {
  ...
  /*
   * order the 16 1d points to a square matrix which is as the one described in
   * p.199 of PDF3200_2008.pdf rotated 90 degrees clockwise
   */
  ...
  private int[] calcLevel() {
    ...
    // if two opposite edges are both lines, there is a possibility to reduce the
    // dividing level
    if (isEdgeALine(ctlC1) && isEdgeALine(ctlC2)) {
      /*
       * if any of the 4 inner control points is out of the patch formed by the 4
       * edges, keep the high dividing level, otherwise, determine the dividing level
       * by the lengths of edges
       */
      if (isOnSameSideCC(controlPoints[1][1]) || isOnSameSideCC(controlPoints[1][2])
        ...
        final double lc1 = getLen(ctlC1[0], ctlC1[3]), lc2 = getLen(ctlC2[0], ctlC2[3]);
        if (lc1 > 800 || lc2 > 800) {
          // keeps init value 4
  ...
  /*
   * dividing a patch into a grid according to level, then calculate the
   * coordinate and color of each crossing point in the grid, the rule to
   * calculate the coordinate is tensor-product which is defined in page 119 of
   * PDF32000_2008.pdf, the method to calculate the cooresponding color is
  ...
  private CoordinateColorPair[][] getPatchCoordinatesColor() {
    ...
    for (int k = 0; k < szV; k++) {
      ...
      for (int l = 0; l < szU; l++) {
        double tmpx = 0.0;
        double tmpy = 0.0;
        // these two "for" loops are for the equation to define the patch surface
        // (coordinates)
        for (int i = 0; i < 4; i++) {
          for (int j = 0; j < 4; j++) {
