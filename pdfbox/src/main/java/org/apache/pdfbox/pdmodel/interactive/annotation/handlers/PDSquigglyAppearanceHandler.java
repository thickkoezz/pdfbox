  ...
  public void generateNormalAppearance() {
    ...
    try (PDAppearanceContentStream cs = getNormalAppearanceAsContentStream()) {
      ...
      for (int i = 0; i < pathsArray.length / 8; ++i) {
        // Adobe uses a fixed pattern that assumes a height of 40, and it transforms to
        // that height
        ...
        // (almost)
        final PDFormXObject form = new PDFormXObject(new COSStream());
        form.setBBox(
            new PDRectangle(-0.5f, -0.5f, (pathsArray[i * 8 + 2] - pathsArray[i * 8]) / height * 40f + 0.5f, 13));
      ...
      PDSquigglyAppearanceHandler.LOG.error(ex);
    }
  }
