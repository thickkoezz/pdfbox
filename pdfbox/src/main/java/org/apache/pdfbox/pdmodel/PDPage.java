...
public class PDPage implements COSObjectable, PDContentStream {
  ...
  /**
   * Creates a new PDPage instance for embedding, with a size of U.S. Letter (8.5
   * x 11 inches).
   */
  public PDPage() {
   ...
   *
   * @return An InputStream, never null. Multiple content streams are concatenated
   *         and separated with a newline. An empty stream is returned if the page
   *         doesn't have any content stream.
   ...
   *
   * @return the integer key of the page's entry in the structural parent tree or
  ...
  public PDRectangle getMediaBox() {
    ...
    if (mediaBox == null) {
      PDPage.LOG.debug("Can't find MediaBox, will use U.S. Letter");
   ...
   * contents are to be clipped (cropped) to this rectangle.
   */
  ...
  /**
   * A rectangle, expressed in default user space units, defining the region to
   * which the contents of the page should be clipped when output in a production
   * environment. The default is the CropBox.
   *
  ...
  /**
   * A rectangle, expressed in default user space units, defining the intended
   * dimensions of the finished page after trimming. The default is the CropBox.
   *
  ...
  /**
   * A rectangle, expressed in default user space units, defining the extent of
   * the page's meaningful content (including potential white space) as intended
   * by the page's creator The default is the CropBox.
   *
   ...
   * clockwise when displayed or printed. Valid values in a PDF must be a multiple
   * of 90.
   *
   * @return The rotation angle in degrees in normalized form (0, 90, 180 or 270)
   *         or 0 if invalid or not set at this level.
   */
  ...
  public void setContents(final List<PDStream> contents) {
    ...
    for (final PDStream stream : contents) {
      array.add(stream);
    }
  ...
  /**
   * This will get a list of PDThreadBead objects, which are article threads in
   * the document. This will return an empty list if there are no thread beads.
   *
   * @return A list of article threads on this page, never null. The returned list
   *         is backed by the beads COSArray, so any adding or deleting in this
   *         list will change the document too.
   */
  ...
  /**
   * Get the metadata that is part of the document catalog. This will return null
   * if there is no meta data for this object.
   *
  ...
  /**
   * @return The page transition associated with this page or null if no
   ...
   *
   * @param transition The new transition to set on this page.
   * @param duration   The maximum length of time, in seconds, that the page shall
   *                   be displayed during presentations before the viewer
   ...
   *
   * @return List of the PDAnnotation objects, never null. The returned list is
   *         backed by the annotations COSArray, so any adding or deleting in this
   *         list will change the document too.
   *
   ...
   * This will return a list of the annotations for this page.
   *
   * @param annotationFilter the annotation filter provided allowing to filter out
  ...
  /**
   * Returns the resource cache associated with this page, or null if there is
   * none.
   */
  ...
  public List<PDViewportDictionary> getViewports() {
    ...
    for (int i = 0; i < array.size(); ++i) {
      ...
      if (base2 instanceof COSDictionary) {
        ...
        PDPage.LOG.warn("Array element " + base2 + " is skipped, must be a (viewport) dictionary");
      }
    }
    return viewports;
  }
  ...
  public void setViewports(final List<PDViewportDictionary> viewports) {
    ...
    final COSArray array = new COSArray();
    for (final PDViewportDictionary viewport : viewports) {
      array.add(viewport);
    }
