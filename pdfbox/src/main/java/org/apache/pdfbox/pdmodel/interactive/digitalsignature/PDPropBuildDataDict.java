...
public class PDPropBuildDataDict implements COSObjectable {
  ...
  /**
   * The build date of the software module. This string is normally produced by
   * the compiler that is used to compile the software, for example using the Date
   * and Time preprocessor flags. As such, this not likely to be in PDF Date
   * format.
   *
  ...
  /**
   * A text string indicating the version of the application implementation, as
   * described by the <code>Name</code> attribute in this dictionary. When set by
   * Adobe Acrobat, this entry is in the format: major.minor.micro (for example
   * 7.0.7).
   * <p>
   * NOTE: Version value is specific for build data dictionary when used as the
   * <code>App</code> dictionary in a build properties dictionary.
   * </p>
   *
  ...
  /**
   * Indicates the operating system. The string format isn't specified yet. In its
   * PDF Signature Build Dictionary Specifications Adobe differently specifies the
   * value type to store operating system string:
   * <ul>
   * <li>Specification for PDF v1.5 specifies type as string;</li>
   * <li>Specification for PDF v1.7 specifies type as array and provided example
   * for <code>/PropBuild</code> dictionary indicate it as array of names.</li>
   * </ul>
  ...
  /**
   * Indicates the operating system. The string format isn't specified yet. Value
   * will be stored as first item of the array, as specified in PDF Signature
   * Build Dictionary Specification for PDF v1.7.
   *
  ...
  /**
   * If there is a LegalPDF dictionary in the catalog of the PDF file and the
   * NonEmbeddedFonts attribute in this dictionary has a non zero value, and the
   * viewing application has a preference set to suppress the display of this
   * warning then the value of this attribute will be set to true.
   *
   ...
   *
   * Documentation says: (Optional; PDF 1.5) If there is a LegalPDF dictionary in
   * the catalog of the PDF file and the NonEmbeddedFonts attribute in this
   * dictionary has a non zero value, and the viewing application has a preference
   * set to suppress the display of this warning then the value of this attribute
   * will be set to true.
   */
   ...
   *
   * @return true if the application was in trusted mode while signing. default:
   *         false
   */
