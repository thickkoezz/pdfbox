...
public class FDFIconFit implements COSObjectable {
  ...
  /**
   * This will get the scale option. See the SCALE_OPTION_XXX constants. This is
   * guaranteed to never return null. Default: Always
   *
  ...
  public String getScaleOption() {
    ...
    if (retval == null) {
      retval = FDFIconFit.SCALE_OPTION_ALWAYS;
    }
    return retval;
  }

  /**
   * This will set the scale option for the icon. Set the SCALE_OPTION_XXX
   * constants.
   *
  ...
  public String getScaleType() {
    ...
    if (retval == null) {
      retval = FDFIconFit.SCALE_TYPE_PROPORTIONAL;
    }
    return retval;
  }
   ...
   *
   * To quote the PDF Spec "An array of two numbers between 0.0 and 1.0 indicating
   * the fraction of leftover space to allocate at the left and bottom of the
   * icon. A value of [0.0 0.0] positions the icon at the bottom-left corner of
   * the annotation rectangle; a value of [0.5 0.5] centers it within the
   * rectangle. This entry is used only if the icon is scaled proportionally.
