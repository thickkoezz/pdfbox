...
public final class PDFontFactory {
   ...
   *
   * @param dictionary    a font dictionary
   * @param resourceCache resource cache, only useful for type 3 fonts, can be
   *                      null
   * @return a PDFont instance, based on the SubType entry of the dictionary
   * @throws IOException if something goes wrong
   */
  public static PDFont createFont(final COSDictionary dictionary, final ResourceCache resourceCache)
      throws IOException {
    final COSName type = dictionary.getCOSName(COSName.TYPE, COSName.FONT);
    if (!COSName.FONT.equals(type)) {
      PDFontFactory.LOG.error("Expected 'Font' dictionary but found '" + type.getName() + "'");
    }
    ...
    if (COSName.TYPE1.equals(subType)) {
      ...
      PDFontFactory.LOG.warn("Invalid font subtype '" + subType + "'");
      return new PDType1Font(dictionary);
    }
  }
   ...
   * @throws IOException if something goes wrong
   */
  static PDCIDFont createDescendantFont(final COSDictionary dictionary, final PDType0Font parent) throws IOException {
    final COSName type = dictionary.getCOSName(COSName.TYPE, COSName.FONT);
    if (!COSName.FONT.equals(type))
