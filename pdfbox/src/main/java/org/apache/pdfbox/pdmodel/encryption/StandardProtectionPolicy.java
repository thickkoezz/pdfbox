 ...
 *
 * The following example shows how to protect a PDF document with password. In
 * this example, the document will be protected so that someone opening the
 * document with the user password <code>user_pwd</code> will not be able to
 * modify the document.
 *
 * <pre>
...
public final class StandardProtectionPolicy extends ProtectionPolicy {
  ...
  /**
   * Tell whether AES encryption is preferred when several encryption methods are
   * available for the chosen key length. The default is false. This setting is
   * only relevant if the key length is 128 bits.
   *
