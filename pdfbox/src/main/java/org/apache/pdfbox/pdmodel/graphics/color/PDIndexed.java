...
/**
 * An Indexed colour space specifies that an area is to be painted using a
 * colour table of arbitrary colours from another color space.
 *
 * @author John Hewson
...
public final class PDIndexed extends PDSpecialColorSpace {
   ...
   *
   * @param indexedArray the array containing the indexed parameters
   * @param resources    the resources, can be null. Allows to use its cache for
   *                     the colorspace.
   * @throws java.io.IOException
   */
  ...
  private void initRgbColorTable() throws IOException {
    ...
    try {
      baseRaster = Raster.createBandedRaster(DataBuffer.TYPE_BYTE, actualMaxIndex + 1, 1, numBaseComponents,
          new Point(0, 0));
  ...
  private void readColorTable() throws IOException {
    final byte[] lookupData = getLookupData();
  ...
  public String toString() {
    return "Indexed{base:" + baseColorSpace + " " + "hival:" + getHival() + " " + "lookup:(" + colorTable.length
        + " entries)}";
  }
}
