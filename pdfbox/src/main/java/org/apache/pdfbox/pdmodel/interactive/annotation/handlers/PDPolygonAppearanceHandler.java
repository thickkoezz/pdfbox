...
public class PDPolygonAppearanceHandler extends PDAbstractAppearanceHandler {
  ...
  public void generateNormalAppearance() {
    ...
    if (pathArray == null)
      return;
    for (final float[] element : pathArray) {
      for (int j = 0; j < element.length / 2; ++j) {
        final float x = element[j * 2];
        final float y = element[j * 2 + 1];
    ...
    try (PDAppearanceContentStream contentStream = getNormalAppearanceAsContentStream()) {
      ...
      final boolean hasBackground = contentStream.setNonStrokingColorOnDemand(annotation.getInteriorColor());
      ...
      if (borderEffect != null && borderEffect.getStyle().equals(PDBorderEffectDictionary.STYLE_CLOUDY)) {
        final CloudyBorder cloudyBorder = new CloudyBorder(contentStream, borderEffect.getIntensity(), lineWidth,
            getRectangle());
        ...
        for (int i = 0; i < pathArray.length; i++) {
          ...
          if (i == 0 && pointsArray.length == 2) {
            ...
            if (pointsArray.length == 2) {
              ...
              contentStream.curveTo(pointsArray[0], pointsArray[1], pointsArray[2], pointsArray[3], pointsArray[4],
                  pointsArray[5]);
            }
          }
        }
      ...
      PDPolygonAppearanceHandler.LOG.error(e);
    }
  }
