 ...
 * DeviceN represents a colour space containing multiple components that
 * correspond to colorants of some target device. As with Separation colour
 * spaces, readers are able to approximate the colorants if they are not
...
public class PDDeviceN extends PDSpecialColorSpace {
  ...
  public PDDeviceN(final COSArray deviceN) throws IOException {
    array = deviceN;
    alternateColorSpace = PDColorSpace.create(array.getObject(PDDeviceN.ALTERNATE_CS));
    tintTransform = PDFunction.create(array.getObject(PDDeviceN.TINT_TRANSFORM));

    if (array.size() > PDDeviceN.DEVICEN_ATTRIBUTES) {
      attributes = new PDDeviceNAttributes((COSDictionary) array.getObject(PDDeviceN.DEVICEN_ATTRIBUTES));
    }
  ...
  private BufferedImage toRGBWithAttributes(final WritableRaster raster) throws IOException {
    ...
    for (int c = 0; c < numColorants; c++) {
      ...
      final WritableRaster componentRaster = Raster.createBandedRaster(DataBuffer.TYPE_BYTE, width, height,
          componentColorSpace.getNumberOfComponents(), new Point(0, 0));
  ...
  public List<String> getColorantNames() {
    final COSArray names = (COSArray) array.getObject(PDDeviceN.COLORANT_NAMES);
  ...
  public void setColorantNames(final List<String> names) {
    ...
    array.set(PDDeviceN.COLORANT_NAMES, namesArray);
  }

  /**
   * Sets the color space attributes. If null is passed in then all attribute will
   * be removed.
   *
  ...
  public void setAttributes(final PDDeviceNAttributes attributes) {
    ...
    if (attributes == null) {
      array.remove(PDDeviceN.DEVICEN_ATTRIBUTES);
    } else {
      // make sure array is large enough
      while (array.size() <= PDDeviceN.DEVICEN_ATTRIBUTES) {
        array.add(COSNull.NULL);
      }
      array.set(PDDeviceN.DEVICEN_ATTRIBUTES, attributes.getCOSDictionary());
    }
  }
  ...
  public void setAlternateColorSpace(final PDColorSpace cs) {
    ...
    array.set(PDDeviceN.ALTERNATE_CS, space);
  }
  ...
  public void setTintTransform(final PDFunction tint) {
    tintTransform = tint;
    array.set(PDDeviceN.TINT_TRANSFORM, tint);
  }
