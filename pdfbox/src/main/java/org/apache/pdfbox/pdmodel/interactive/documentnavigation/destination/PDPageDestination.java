...
/**
 * This represents a destination to a page, see subclasses for specific
 * parameters.
 *
...
public abstract class PDPageDestination extends PDDestination {
  ...
  /**
   * This will get the page for this destination. A page destination can either
   * reference a page (for a local destination) or a page number (when doing a
   * remote destination to another PDF). If this object is referencing by page
   * number then this method will return null and {@link #getPageNumber()} should
   * be used.
   *
   ...
   * {@link #setPageNumber(int) setPageNumber(int pageNumber)}.
   *
  ...
  /**
   * This will get the page number for this destination. A page destination can
   * either reference a page (for a local destination) or a page number (when
   * doing a remote destination to another PDF). If this object is referencing by
   * page number then this method will return that number, otherwise -1 will be
   * returned.
   *
  ...
  /**
   * Returns the page number for this destination, regardless of whether this is a
   * page number or a reference to a page.
   *
  ...
  // climb up the page tree up to the top to be able to call PageTree.indexOf for
  // a page dictionary
  ...
  /**
   * Set the page number for a remote destination. For an internal destination,
   * call {@link #setPage(PDPage) setPage(PDPage page)}.
   *
