...
class AppearanceGeneratorHelper {
  ...
  AppearanceGeneratorHelper(final PDVariableText field) throws IOException {
    ...
    defaultAppearance = field.getDefaultAppearanceString();
  }
        ...
        for (final COSName fontResourceName : widgetResources.getFontNames()) {
          try {
            if (acroFormResources.getFont(fontResourceName) == null) {
              AppearanceGeneratorHelper.LOG
                  .debug("Adding font resource " + fontResourceName + " from widget to AcroForm");
              acroFormResources.put(fontResourceName, widgetResources.getFont(fontResourceName));
            }
          } catch (final IOException e) {
            AppearanceGeneratorHelper.LOG.warn("Unable to match field level font with AcroForm font", e);
          }
        }
      }
    }
  ...
  public void setAppearanceValue(final String apValue) throws IOException {
    ...
    for (final PDAnnotationWidget widget : field.getWidgets()) {
      ...
      if (rect == null) {
        ...
        AppearanceGeneratorHelper.LOG.warn(
            "widget of field " + field.getFullyQualifiedName() + " has no rectangle, no appearance stream created");
        continue;
      }
  ...
  private PDDefaultAppearanceString getWidgetDefaultAppearanceString(final PDAnnotationWidget widget)
      throws IOException {
   ...
   */
  private void initializeAppearanceContent(final PDAnnotationWidget widget, final PDAppearanceStream appearanceStream)
      throws IOException {
    try (ByteArrayOutputStream output = new ByteArrayOutputStream();
      ...
      if (appearanceCharacteristics != null) {
        ...
        if (lineWidth > 0 && borderColour != null) {
          ...
          final PDRectangle bbox = resolveBoundingBox(widget, appearanceStream);
          final PDRectangle clipRect = applyPadding(bbox,
              Math.max(AppearanceGeneratorHelper.DEFAULT_PADDING, lineWidth / 2));
   ...
   */
  private List<Object> tokenize(final PDAppearanceStream appearanceStream) throws IOException {
   ...
   */
  private void setAppearanceContent(final PDAnnotationWidget widget, final PDAppearanceStream appearanceStream)
      throws IOException {
    ...
    try (ByteArrayOutputStream output = new ByteArrayOutputStream()) {
      ...
      final List<Object> tokens = tokenize(appearanceStream);
      final int bmcIndex = tokens.indexOf(AppearanceGeneratorHelper.BMC);
      if (bmcIndex == -1) {
        // append to existing stream
        writer.writeTokens(tokens);
        writer.writeTokens(COSName.TX, AppearanceGeneratorHelper.BMC);
      } else {
      ...
      final int emcIndex = tokens.indexOf(AppearanceGeneratorHelper.EMC);
      if (emcIndex == -1) {
        // append EMC
        writer.writeTokens(AppearanceGeneratorHelper.EMC);
      } else {
   ...
   */
  private void insertGeneratedAppearance(final PDAnnotationWidget widget, final PDAppearanceStream appearanceStream,
    ...
    try (PDAppearanceContentStream contents = new PDAppearanceContentStream(appearanceStream, output)) {
      ...
      if (font.getName().contains("+")) {
        AppearanceGeneratorHelper.LOG.warn("Font '" + defaultAppearance.getFontName().getName() + "' of field '"
            + field.getFullyQualifiedName() + "' contains subsetted font '" + font.getName() + "'");
        AppearanceGeneratorHelper.LOG.warn("This may bring trouble with PDField.setValue(), PDAcroForm.flatten() or "
            + "PDAcroForm.refreshAppearances()");
        AppearanceGeneratorHelper.LOG.warn("You should replace this font with a non-subsetted font:");
        AppearanceGeneratorHelper.LOG
            .warn("PDFont font = PDType0Font.load(doc, new FileInputStream(fontfile), false);");
        AppearanceGeneratorHelper.LOG.warn("acroForm.getDefaultResources().put(COSName.getPDFName(\""
      ...
      final float fontScaleY = fontSize / AppearanceGeneratorHelper.FONTSCALE;
      final float fontBoundingBoxAtSize = font.getBoundingBox().getHeight() * fontScaleY;
      final float fontCapAtSize = font.getFontDescriptor().getCapHeight() * fontScaleY;
      final float fontDescentAtSize = font.getFontDescriptor().getDescent() * fontScaleY;
      ...
      if (shallComb()) {
        ...
        final PlainTextFormatter formatter = new PlainTextFormatter.Builder(contents).style(appearanceStyle)
            .text(textContent).width(contentRect.getWidth()).wrapLines(isMultiLine()).initialOffset(x, y)
            .textAlign(field.getQ()).build();
        formatter.format();
      }
   ...
   * @throws IOException
   */
  private void insertGeneratedCombAppearance(final PDAppearanceContentStream contents,
      final PDAppearanceStream appearanceStream, final PDFont font, final float fontSize) throws IOException {
    ...
    final float ascentAtFontSize = font.getFontDescriptor().getAscent() / AppearanceGeneratorHelper.FONTSCALE
        * fontSize;
    ...
    for (int i = 0; i < numChars; i++) {
      ...
      final float currCharWidth = font.getStringWidth(combString) / AppearanceGeneratorHelper.FONTSCALE * fontSize / 2;
  ...
  private void insertGeneratedListboxSelectionHighlight(final PDAppearanceContentStream contents,
      final PDAppearanceStream appearanceStream, final PDFont font, final float fontSize) throws IOException {
    ...
    final float highlightBoxHeight = font.getBoundingBox().getHeight() * fontSize / AppearanceGeneratorHelper.FONTSCALE;

    // the padding area
    final PDRectangle paddingEdge = applyPadding(appearanceStream.getBBox(), 1);

    for (final int selectedIndex : indexEntries) {
      contents.setNonStrokingColor(AppearanceGeneratorHelper.HIGHLIGHT_COLOR[0],
          AppearanceGeneratorHelper.HIGHLIGHT_COLOR[1], AppearanceGeneratorHelper.HIGHLIGHT_COLOR[2]);
  ...
  private void insertGeneratedListboxAppearance(final PDAppearanceContentStream contents,
      final PDAppearanceStream appearanceStream, final PDRectangle contentRect, final PDFont font, final float fontSize)
      throws IOException {
    contents.setNonStrokingColor(0);
    ...
    if (q == PDVariableText.QUADDING_CENTERED || q == PDVariableText.QUADDING_RIGHT) {
      ...
      final float stringWidth = font.getStringWidth(value) / AppearanceGeneratorHelper.FONTSCALE * fontSize;
    ...
    for (int i = topIndex; i < numOptions; i++) {
      ...
      if (i == topIndex) {
        yTextPos = yTextPos - font.getFontDescriptor().getAscent() / AppearanceGeneratorHelper.FONTSCALE * fontSize;
      } else {
        yTextPos = yTextPos - font.getBoundingBox().getHeight() / AppearanceGeneratorHelper.FONTSCALE * fontSize;
        contents.beginText();
      }
  ...
  private float calculateFontSize(final PDFont font, final PDRectangle contentRect) throws IOException {
    ...
    if (Float.compare(fontSize, 0) == 0) {
      if (isMultiLine())
        ...
        return AppearanceGeneratorHelper.DEFAULT_FONT_SIZE;
      else {
        final float yScalingFactor = AppearanceGeneratorHelper.FONTSCALE * font.getFontMatrix().getScaleY();
        final float xScalingFactor = AppearanceGeneratorHelper.FONTSCALE * font.getFontMatrix().getScaleX();
  ...
  private PDRectangle applyPadding(final PDRectangle box, final float padding) {
    return new PDRectangle(box.getLowerLeftX() + padding, box.getLowerLeftY() + padding, box.getWidth() - 2 * padding,
