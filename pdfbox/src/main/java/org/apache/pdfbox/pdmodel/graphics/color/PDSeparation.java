...
/**
 * A Separation color space used to specify either additional colorants or for
 * isolating the control of individual colour components of a device colour
 * space for a subtractive device. When such a space is the current colour
 * space, the current colour shall be a single-component value, called a tint,
 * that controls the given colorant or colour components only.
 *
...
public class PDSeparation extends PDSpecialColorSpace {
  ...
  /**
   * Map used to speed up {@link #toRGB(float[])}. Note that this class contains
   * three maps (this and the two in
   * {@link #toRGBImage(java.awt.image.WritableRaster) } and
   * {@link #toRGBImage2(java.awt.image.WritableRaster) }. The maps use different
   * key intervals. This map here is needed for shading, which produce more than
   * 256 different float values, which we cast to int so that the map can work.
   */
  ...
  public PDSeparation(final COSArray separation) throws IOException {
    array = separation;
    alternateColorSpace = PDColorSpace.create(array.getObject(PDSeparation.ALTERNATE_CS));
    tintTransform = PDFunction.create(array.getObject(PDSeparation.TINT_TRANSFORM));
  }
  ...
  public BufferedImage toRGBImage(final WritableRaster raster) throws IOException {
    ...
    final WritableRaster altRaster = Raster.createBandedRaster(DataBuffer.TYPE_BYTE, raster.getWidth(),
        raster.getHeight(), alternateColorSpace.getNumberOfComponents(), new Point(0, 0));
  ...
  public String getColorantName() {
    final COSName name = (COSName) array.getObject(PDSeparation.COLORANT_NAMES);
    return name.getName();
  }
  ...
  public void setAlternateColorSpace(final PDColorSpace colorSpace) {
    ...
    array.set(PDSeparation.ALTERNATE_CS, space);
  }
  ...
  public void setTintTransform(final PDFunction tint) {
    tintTransform = tint;
    array.set(PDSeparation.TINT_TRANSFORM, tint);
  }
