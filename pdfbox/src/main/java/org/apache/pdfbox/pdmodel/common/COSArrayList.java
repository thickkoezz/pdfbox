...
/**
 * This is an implementation of a List that will sync its contents to a
 * COSArray.
 *
...
public class COSArrayList<E> implements List<E> {
   ...
   * created and added to the parent dictionary as soon as the first element is
   * added to the array.
   *
   * @param dictionary    The dictionary that holds the item, and will hold the
   *                      array if an item is added.
  ...
  /**
   * This is a really special constructor. Sometimes the PDF spec says that a
   * dictionary entry can either be a single item or an array of those items. But
   * in the PDModel interface we really just want to always return a
   * java.util.List. In the case were we get the list and never modify it we don't
   * want to convert to COSArray and put one element, unless we append to the
   * list. So here we are going to create this object with a single item instead
   ...
   * @param dictionaryKey The key into the dictionary to set the item.
   */
  public COSArrayList(final E actualObject, final COSBase item, final COSDictionary dictionary,
      final COSName dictionaryKey) {
    array = new COSArray();
    array.add(item);
    actual = new ArrayList<>();
   ...
   */
  public static List<Integer> convertIntegerCOSArrayToList(final COSArray intArray) {
   ...
   */
  public static List<Float> convertFloatCOSArrayToList(final COSArray floatArray) {
    List<Float> retval = null;
    if (floatArray != null) {
      final List<Float> numbers = new ArrayList<>(floatArray.size());
      for (int i = 0; i < floatArray.size(); i++) {
        final COSBase base = floatArray.getObject(i);
   ...
   */
  public static List<String> convertCOSNameCOSArrayToList(final COSArray nameArray) {
   ...
   * @return The list of String objects.
   */
  public static List<String> convertCOSStringCOSArrayToList(final COSArray stringArray) {
    List<String> retval = null;
    if (stringArray != null) {
   ...
   */
  public static COSArray convertStringListToCOSNameCOSArray(final List<String> strings) {
   ...
   * @return An array of COSName objects
   */
  public static COSArray convertStringListToCOSStringCOSArray(final List<String> strings) {
    final COSArray retval = new COSArray();
    for (final String string : strings) {
  ...
  /**
   * This will convert a list of COSObjectables to an array list of COSBase
   * objects.
   *
   * @param cosObjectableList A list of COSObjectable.
   *
   * @return A list of COSBase.
   * @throws IllegalArgumentException if an object type is not supported for
   *                                  conversion to a COSBase object.
   */
  public static COSArray converterToCOSArray(final List<?> cosObjectableList) {
    ...
    if (cosObjectableList != null) {
      if (cosObjectableList instanceof COSArrayList) {
        // if it is already a COSArrayList then we don't want to recreate the array, we
        // want to reuse it.
        ...
        for (final Object next : cosObjectableList) {
          if (next instanceof String) {
            ...
            throw new IllegalArgumentException(
                "Error: Don't know how to convert type to COSBase '" + next.getClass().getName() + "'");
        }
      }
    }
    return array;
