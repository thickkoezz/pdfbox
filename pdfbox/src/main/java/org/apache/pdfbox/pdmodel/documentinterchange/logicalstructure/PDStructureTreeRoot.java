...
public class PDStructureTreeRoot extends PDStructureNode {
  ...
  public PDStructureTreeRoot() {
    super(PDStructureTreeRoot.TYPE);
  }
  ...
  /**
   * Returns the K entry. This can be a dictionary representing a structure
   * element, or an array of them.
   *
   * @return the K entry.
   */
  public COSBase getK() {
  ...
  public PDNameTreeNode<PDStructureElement> getIDTree() {
    final COSBase base = getCOSObject().getDictionaryObject(COSName.ID_TREE);
  ...
  public Map<String, Object> getRoleMap() {
    final COSBase rm = getCOSObject().getDictionaryObject(COSName.ROLE_MAP);
    if (rm instanceof COSDictionary) {
      try {
        return COSDictionaryMap.convertBasicTypesToMap((COSDictionary) rm);
      } catch (final IOException e) {
        PDStructureTreeRoot.LOG.error(e, e);
      }
    }
  ...
  public void setRoleMap(final Map<String, String> roleMap) {
    ...
    for (final Map.Entry<String, String> entry : roleMap.entrySet()) {
      rmDic.setName(entry.getKey(), entry.getValue());
    }
    getCOSObject().setItem(COSName.ROLE_MAP, rmDic);
  }
