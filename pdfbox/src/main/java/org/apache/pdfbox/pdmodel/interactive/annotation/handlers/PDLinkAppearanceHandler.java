...
public class PDLinkAppearanceHandler extends PDAbstractAppearanceHandler {
  ...
  public void generateNormalAppearance() {
    ...
    try (PDAppearanceContentStream contentStream = getNormalAppearanceAsContentStream()) {
      ...
      if (pathsArray != null) {
        ...
        for (int i = 0; i < pathsArray.length / 2; ++i) {
          if (!rect.contains(pathsArray[i * 2], pathsArray[i * 2 + 1])) {
            PDLinkAppearanceHandler.LOG
                .warn("At least one /QuadPoints entry (" + pathsArray[i * 2] + ";" + pathsArray[i * 2 + 1]
                    + ") is outside of rectangle, " + rect + ", /QuadPoints are ignored and /Rect is used instead");
            pathsArray = null;
            break;
          }
        }
      ...
      PDLinkAppearanceHandler.LOG.error(e);
    }
  }
