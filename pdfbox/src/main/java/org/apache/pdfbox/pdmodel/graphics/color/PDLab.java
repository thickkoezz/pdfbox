...
public final class PDLab extends PDCIEDictionaryBasedColorSpace {
  ...
  public PDColor getInitialColor() {
    if (initialColor == null) {
      initialColor = new PDColor(
          new float[] { 0, Math.max(0, getARange().getMin()), Math.max(0, getBRange().getMin()) }, this);
    }
    return initialColor;
  }
  ...
  /**
   * This will get the valid range for the "a" component. If none is found then
   * the default will be returned, which is -100..100.
   *
   * @return the "a" range.
   */
  ...
  /**
   * This will get the valid range for the "b" component. If none is found then
   * the default will be returned, which is -100..100.
   *
   * @return the "b" range.
   */
   ...
   *
   * @param range the new range for the "a" component, or null if defaults
   *              (-100..100) are to be set.
   */
   ...
   *
   * @param range the new range for the "b" component, or null if defaults
   *              (-100..100) are to be set.
   */
