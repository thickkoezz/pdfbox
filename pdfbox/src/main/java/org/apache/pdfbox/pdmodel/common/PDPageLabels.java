...
public class PDPageLabels implements COSObjectable {
  ...
  /**
   * Creates an page label dictionary for a document using the information in the
   * given COS dictionary.
   *
   * <p>
   * Note that the page label dictionary won't be automatically added to the
   * document; you will still need to do it manually (see
   ...
   * @see org.apache.pdfbox.pdmodel.PDDocumentCatalog#setPageLabels(PDPageLabels)
   * @throws IOException If something goes wrong during the number tree
   *                     conversion.
   */
  ...
  private void findLabels(final PDNumberTreeNode node) throws IOException {
    if (node.getKids() != null) {
      ...
      final Map<Integer, COSObjectable> numbers = node.getNumbers();
      for (final Entry<Integer, COSObjectable> i : numbers.entrySet()) {
        if (i.getKey() >= 0) {
   ...
   *
   * <p>
   * This will be always &gt;= 1, as the required default entry for the page range
   * starting at the first page is added automatically by this implementation (see
   * PDF32000-1:2008, p. 375).
   * </p>
   *
  ...
  /**
   * Returns the page label range starting at the given page, or {@code null} if
   * no such range is defined.
   *
   * @param startPage the 0-based page index representing the start page of the
   *                  page range the item is defined for.
   * @return the page label range or {@code null} if no label range is defined for
   *         the given start page.
   */
  ...
  public COSBase getCOSObject() {
    ...
    for (final Entry<Integer, PDPageLabelRange> i : labels.entrySet()) {
  ...
  /**
   * Returns a mapping with computed page labels as keys and corresponding 0-based
   * page indices as values. The returned map will contain at most as much entries
   * as the document has pages.
   *
   * <p>
   * <strong>NOTE:</strong> If the document contains duplicate page labels, the
   * returned map will contain <em>less</em> entries than the document has pages.
   * The page index returned in this case is the <em>highest</em> index among all
   * pages sharing the same label.
   * </p>
   *
  ...
  /**
   * Returns a mapping with 0-based page indices as keys and corresponding page
   * labels as values as an array. The array will have exactly as much entries as
  ...
  private void computeLabels(final LabelHandler handler) {
    final Iterator<Entry<Integer, PDPageLabelRange>> iterator = labels.entrySet().iterator();
    if (!iterator.hasNext())
      return;
    int pageIndex = 0;
    ...
    final LabelGenerator gen = new LabelGenerator(lastEntry.getValue(), doc.getNumberOfPages() - lastEntry.getKey());
    while (gen.hasNext()) {
      handler.newLabel(pageIndex, gen.next());
      pageIndex++;
    }
