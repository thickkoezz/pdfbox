...
abstract class Patch {
  ...
  /*
   * level = {levelU, levelV}, levelU defines the patch's u direction edges should
   * be divided into 2^levelU parts, level V defines the patch's v direction edges
   * should be divided into 2^levelV parts
   */
  protected int[] level;
   ...
   *
   * @param ctl   control points, size is 12 (for type 6 shading) or 16 (for type
   *              7 shading)
  ...
  /**
   * A line from point p1 to point p2 defines an equation, adjust the form of the
   * equation to let the rhs equals 0, then calculate the lhs value by plugging
   * the coordinate of p in the lhs expression.
   *
  ...
  protected List<ShadedTriangle> getShadedTriangles(final CoordinateColorPair[][] patchCC) {
    ...
    for (int i = 1; i < szV; i++) {
      for (int j = 1; j < szU; j++) {
        final Point2D p0 = patchCC[i - 1][j - 1].coordinate, p1 = patchCC[i - 1][j].coordinate,
            p2 = patchCC[i][j].coordinate, p3 = patchCC[i][j - 1].coordinate;
        boolean ll = true;
        if (overlaps(p0, p1) || overlaps(p0, p3)) {
          ll = false;
        } else {
          // p0, p1 and p3 are in counter clock wise order, p1 has priority over p0, p3
          // has priority over p1
          final Point2D[] llCorner = { p0, p1, p3 };
          final float[][] llColor = { patchCC[i - 1][j - 1].color, patchCC[i - 1][j].color, patchCC[i][j - 1].color };
          final ShadedTriangle tmpll = new ShadedTriangle(llCorner, llColor); // lower left triangle
          list.add(tmpll);
        }
        if (ll && (overlaps(p2, p1) || overlaps(p2, p3))) {
        } else {
          // p3, p1 and p2 are in counter clock wise order, p1 has priority over p3, p2
          // has priority over p1
          final Point2D[] urCorner = { p3, p1, p2 };
          final float[][] urColor = { patchCC[i][j - 1].color, patchCC[i - 1][j].color, patchCC[i][j].color };
  ...
  // whether two points p0 and p1 are degenerated into one point within the
  // coordinates' accuracy 0.001
