...
final class PDFieldFactory {
   ...
   */
  static PDField createField(final PDAcroForm form, final COSDictionary field, final PDNonTerminalField parent) {
    final String fieldType = PDFieldFactory.findFieldType(field);
    ...
    if (PDFieldFactory.FIELD_TYPE_CHOICE.equals(fieldType))
      return PDFieldFactory.createChoiceSubType(form, field, parent);
    else if (PDFieldFactory.FIELD_TYPE_TEXT.equals(fieldType))
      return new PDTextField(form, field, parent);
    else if (PDFieldFactory.FIELD_TYPE_SIGNATURE.equals(fieldType))
      return new PDSignatureField(form, field, parent);
    else if (PDFieldFactory.FIELD_TYPE_BUTTON.equals(fieldType))
