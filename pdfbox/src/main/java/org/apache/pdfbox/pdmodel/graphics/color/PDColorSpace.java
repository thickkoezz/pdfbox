...
public abstract class PDColorSpace implements COSObjectable {
  ...
  /**
   * Creates a color space given a name or array. Abbreviated device color names
   * are not supported here, please replace them first.
   *
   * @param colorSpace the color space COS object
   * @param resources  the current resources.
   * @return a new color space
   * @throws MissingResourceException if the color space is missing in the
   *                                  resources dictionary
   * @throws IOException              if the color space is unknown or cannot be
   *                                  created
   */
  public static PDColorSpace create(final COSBase colorSpace, final PDResources resources) throws IOException {
    return PDColorSpace.create(colorSpace, resources, false);
  }

  /**
   * Creates a color space given a name or array. Abbreviated device color names
   * are not supported here, please replace them first. This method is for PDFBox
   * internal use only, others should use {@link #create(COSBase, PDResources)}.
   *
   * @param colorSpace the color space COS object
   * @param resources  the current resources.
   ...
   *                                  created.
   */
  public static PDColorSpace create(final COSBase colorSpace, final PDResources resources, final boolean wasDefault)
      throws IOException {
  ...
  /**
   * Returns the (A)RGB equivalent of the given raster, using the given AWT color
  ...
  protected BufferedImage toRGBImageAWT(final WritableRaster raster, final ColorSpace colorSpace) {
    ...
    // ColorConvertOp
    final ColorModel colorModel = new ComponentColorModel(colorSpace, false, false, Transparency.OPAQUE,
