...
public class PDAnnotationWidget extends PDAnnotation {
  ...
  public PDAnnotationWidget() {
    getCOSObject().setName(COSName.SUBTYPE, PDAnnotationWidget.SUB_TYPE);
  }

  /**
   * Creates a PDWidget from a COSDictionary, expected to be a correct object
   * definition for a field in PDF.
   *
  ...
  public void setHighlightingMode(final String highlightingMode) {
    if (highlightingMode == null || "N".equals(highlightingMode) || "I".equals(highlightingMode)
        || "O".equals(highlightingMode) || "P".equals(highlightingMode) || "T".equals(highlightingMode)) {
  ...
  public PDAppearanceCharacteristicsDictionary getAppearanceCharacteristics() {
    final COSBase mk = getCOSObject().getDictionaryObject(COSName.MK);
  ...
  /**
   * Set the annotation action. As of PDF 1.6 this is only used for Widget
   * Annotations
   *
   ...
   * no additional actions for this field. As of PDF 1.6 this is only used for
   * Widget Annotations.
   *
   * @return The actions of the field.
   */
  public PDAnnotationAdditionalActions getActions() {
    final COSBase base = getCOSObject().getDictionaryObject(COSName.AA);
    if (base instanceof COSDictionary)
  ...
  /**
   * Set the parent field of a widget annotation. This is only required if this
   * widget annotation is one of multiple children in a field, and not to be set
   * otherwise. You will usually not need this, because in most cases, fields have
   * only one widget and share a common dictionary. A usage can be found in the
   * CreateMultiWidgetsForm example.
   *
   * @param field the parent field.
   * @throws IllegalArgumentException if setParent() was called for a field that
   *                                  shares a dictionary with its only widget.
   */
  public void setParent(final PDTerminalField field) {
    if (getCOSObject().equals(field.getCOSObject()))
      throw new IllegalArgumentException(
          "setParent() is not to be called for a field that shares a dictionary with its only widget");
