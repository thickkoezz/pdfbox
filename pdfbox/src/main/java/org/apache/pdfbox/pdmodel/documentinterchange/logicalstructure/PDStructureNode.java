...
public abstract class PDStructureNode implements COSObjectable {
  ...
  /**
   * Creates a node in the structure tree. Can be either a structure tree root, or
   * a structure element.
   *
  ...
  public List<Object> getKids() {
    ...
    final COSBase k = getCOSObject().getDictionaryObject(COSName.K);
    if (k instanceof COSArray) {
      for (final COSBase kid : (COSArray) k) {
        final Object kidObject = createObject(kid);
        if (kidObject != null) {
          kidObjects.add(kidObject);
        }
      }
    } else {
      final Object kidObject = createObject(k);
      if (kidObject != null) {
        kidObjects.add(kidObject);
      }
    }
  ...
  public void setKids(final List<Object> kids) {
    getCOSObject().setItem(COSName.K, COSArrayList.converterToCOSArray(kids));
  }
  ...
  protected void appendKid(final COSBase object) {
    ...
    if (k == null) {
      ...
      array.add(k);
      array.add(object);
      getCOSObject().setItem(COSName.K, array);
    }
  }
   ...
   *
   * @param structureElement the structure element
   * @return <code>true</code> if the kid was removed, <code>false</code>
   *         otherwise
   */
  public boolean removeKid(final PDStructureElement structureElement) {
    final boolean removed = removeObjectableKid(structureElement);
    if (removed) {
  ...
  protected boolean removeKid(final COSBase object) {
    ...
    if (k == null)
      ...
      if (onlyKid) {
        getCOSObject().setItem(COSName.K, null);
        return true;
      }
      return false;
    }
  ...
  /**
   * Creates an object for a kid of this structure node. The type of object
   * depends on the type of the kid. It can be
   * <ul>
