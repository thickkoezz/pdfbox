...
abstract class PDAbstractContentStream implements Closeable {
   ...
   *                               if you attempt to nest beginText calls.
   * @throws IllegalStateException If the method was not allowed to be called at
   *                               this time.
   */
   ...
   *
   * @throws IOException           If there is an error writing to the stream or
   *                               if you attempt to nest endText calls.
   * @throws IllegalStateException If the method was not allowed to be called at
   *                               this time.
   */
  ...
  public void setFont(final PDFont font, final float fontSize) throws IOException {
    ...
    if (font.willBeSubset()) {
      if (document != null) {
        ...
        PDAbstractContentStream.LOG.warn("attempting to use subset font " + font.getName() + " without proper context");
      }
    }
    ...
    if (font instanceof PDType0Font) {
      ...
      if (gsubData != GsubData.NO_DATA_FOUND) {
        final GsubWorker gsubWorker = gsubWorkerFactory.getGsubWorker(pdType0Font.getCmapLookup(), gsubData);
   ...
   * with the given interspersed positioning. This allows the user to efficiently
   * position each glyph or sequence of glyphs.
   *
   * @param textWithPositioningArray An array consisting of String and Float
   *                                 types. Each String is output to the page
   *                                 using the current text matrix. Using the
   *                                 default coordinate system, each interspersed
   *                                 number adjusts the current text matrix by
   *                                 translating to the left or down for
   *                                 horizontal and vertical text respectively.
   *                                 The number is expressed in thousands of a
   *                                 text space unit, and may be negative.
   *
  ...
  /**
   * The Td operator. Move to the start of the next line, offset from the start of
   ...
   */
  public void drawImage(final PDImageXObject image, final float x, final float y, final float width, final float height)
      throws IOException {
    if (inTextMode)
   ...
   * @throws IllegalStateException If the method was called within a text block.
   */
  public void drawImage(final PDInlineImage inlineImage, final float x, final float y, final float width,
  ...
  public void setStrokingColor(final PDColor color) throws IOException {
    ...
    if (color.getColorSpace() instanceof PDPattern || color.getColorSpace() instanceof PDSeparation
        || color.getColorSpace() instanceof PDDeviceN || color.getColorSpace() instanceof PDICCBased) {
  ...
  public void setStrokingColor(final int r, final int g, final int b) throws IOException {
    if (isOutside255Interval(r) || isOutside255Interval(g) || isOutside255Interval(b))
      throw new IllegalArgumentException(
          "Parameters must be within 0..255, but are " + String.format("(%d,%d,%d)", r, g, b));
    writeOperand(r / 255f);
    writeOperand(g / 255f);
    writeOperand(b / 255f);
  ...
  public void setStrokingColor(final float c, final float m, final float y, final float k) throws IOException {
    if (isOutsideOneInterval(c) || isOutsideOneInterval(m) || isOutsideOneInterval(y) || isOutsideOneInterval(k))
      throw new IllegalArgumentException(
          "Parameters must be within 0..1, but are " + String.format("(%.2f,%.2f,%.2f,%.2f)", c, m, y, k));
    writeOperand(c);
    writeOperand(m);
    writeOperand(y);
    writeOperand(k);
  ...
  /**
   * Append a cubic Bézier curve to the current path. The curve extends from the
   * current point to the point (x3, y3), using the current point and (x2, y2) as
   ...
   * {@link #fill() } and the second with {@link #stroke() }.
   *
   * @throws IOException           If the content stream could not be written
   * @throws IllegalStateException If the method was called within a text block.
   */
   ...
   * to determine the region to fill. This shall have the same effect as the
   * sequence {@link #closePath() } and then {@link #fillAndStroke() }.
   *
   * @throws IOException           If the content stream could not be written
   * @throws IllegalStateException If the method was called within a text block.
   */
   ...
   * {@link #closePath() } and then {@link #fillAndStrokeEvenOdd() }.
   *
   * @throws IOException           If the content stream could not be written
   * @throws IllegalStateException If the method was called within a text block.
   */
  ...
  /**
   * Intersects the current clipping path with the current path, using the nonzero
   * rule.
   *
   * @throws IOException           If the content stream could not be written
   * @throws IllegalStateException If the method was called within a text block.
   ...
   *
   * @param lineJoinStyle 0 for miter join, 1 for round join, and 2 for bevel
   *                      join.
   * @throws IOException              If the content stream could not be written.
   * @throws IllegalStateException    If the method was called within a text
   *                                  block.
   * @throws IllegalArgumentException If the parameter is not a valid line join
   *                                  style.
   */
   ...
   *
   * @param lineCapStyle 0 for butt cap, 1 for round cap, and 2 for projecting
   *                     square cap.
   * @throws IOException              If the content stream could not be written.
   * @throws IllegalStateException    If the method was called within a text
   *                                  block.
   * @throws IllegalArgumentException If the parameter is not a valid line cap
   *                                  style.
   */
   ...
   * @param comment
   * @throws IOException              If the content stream could not be written.
   * @throws IllegalArgumentException If the comment contains a newline. This is
   *                                  not allowed, because the next line could be
   *                                  ordinary PDF content.
   */
  ...
  protected void writeOperand(final float real) throws IOException {
    final int byteCount = NumberFormatUtil.formatFloatFast(real, formatDecimal.getMaximumFractionDigits(),
        formatBuffer);
  ...
  public void close() throws IOException {
    if (inTextMode) {
      PDAbstractContentStream.LOG.warn("You did not call endText(), some viewers won't display your text");
    }
    outputStream.close();
  }
  ...
  /**
   * Set the word spacing. The value shall be added to the horizontal or vertical
   * component of the ASCII SPACE character, depending on the writing mode.
   * <p>
   ...
   * every occurrence of the single-byte character code 32 in a string when using
   * a simple font or a composite font that defines code 32 as a single-byte code.
   * It shall not apply to occurrences of the byte value 32 in multiple-byte
   * codes."
   *
   ...
   *
   * @param scale number specifying the percentage of the normal width. Default
  ...
  /**
   * Set the text rendering mode. This determines whether showing text shall cause
   * glyph outlines to be stroked, filled, used as a clipping boundary, or some
  ...
  /**
   * Set the text rise value, i.e. move the baseline up or down. This is useful
   * for drawing superscripts or subscripts.
   *
   * @param rise Specifies the distance, in unscaled text space units, to move the
   *             baseline up or down from its default location. 0 restores the
   *             default location.
   * @throws IOException
   */
  ...
  private byte[] encodeForGsub(final GsubWorker gsubWorker, final Set<Integer> glyphIds, final PDType0Font font,
  ...
  private List<Integer> applyGSUBRules(final GsubWorker gsubWorker, final ByteArrayOutputStream out,
      final PDType0Font font, final String word) throws IOException {
