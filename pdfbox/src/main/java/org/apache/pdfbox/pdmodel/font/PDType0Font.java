...
public class PDType0Font extends PDFont implements PDVectorFont {
   ...
   * @param ttf
   * @param embedSubset
   * @param closeTTF    whether to close the ttf parameter after embedding. Must
   *                    be true when the ttf parameter was created in the load()
   *                    method, false when the ttf parameter was passed to the
   *                    load() method.
   * @param vertical
   * @throws IOException
   */
   ...
   *
   * @param doc   The PDF document that will hold the embedded font.
   * @param input An input stream of a TrueType font. It will be closed before
   *              returning.
   * @return A Type0 font with a CIDFontType2 descendant.
   ...
   *
   * @param doc         The PDF document that will hold the embedded font.
   * @param input       An input stream of a TrueType font. It will be closed
   *                    before returning.
  ...
  private void readEncoding() throws IOException {
    ...
    if (encoding instanceof COSName) {
      ...
      if (cMap == null)
        ...
        PDType0Font.LOG.warn("Invalid Encoding CMap in font " + getName());
      }
    }
    ...
    if (ros != null) {
      isDescendantCJK = "Adobe".equals(ros.getRegistry())
          && ("GB1".equals(ros.getOrdering()) || "CNS1".equals(ros.getOrdering()) || "Japan1".equals(ros.getOrdering())
  ...
  private void fetchCMapUCS2() throws IOException {
    ...
    if (isCMapPredefined && !(name == COSName.IDENTITY_H || name == COSName.IDENTITY_V) || isDescendantCJK) {
      ...
      // todo: not sure how to interpret the PDF spec here, do we always override? or
      // only when Identity-H/V?
      String strName = null;
      if (isDescendantCJK) {
        strName = descendantFont.getCIDSystemInfo().getRegistry() + "-"
            + descendantFont.getCIDSystemInfo().getOrdering() + "-" + descendantFont.getCIDSystemInfo().getSupplement();
      } else if (name != null) {
  ...
  public String toUnicode(final int code) throws IOException {
    ...
    if ((isCMapPredefined || isDescendantCJK) && cMapUCS2 != null) {
      ...
      // a) Map the character code to a character identifier (CID) according to the
      // font?s CMap
      ...
      if (PDType0Font.LOG.isWarnEnabled() && !noUnicode.contains(code)) {
        // if no value has been produced, there is no way to obtain Unicode for the
        // character.
        final String cid = "CID+" + codeToCID(code);
        PDType0Font.LOG.warn("No Unicode mapping for " + cid + " (" + code + ") in font " + getName());
