...
public final class GlyphList {
  ...
  private static GlyphList load(final String filename, final int numberOfEntries) {
    final String path = "/org/apache/pdfbox/resources/glyphlist/";
    try {
    ...
    try {
      ...
      if (location != null)
        throw new UnsupportedOperationException(
            "glyphlist_ext is no longer supported, " + "use GlyphList.DEFAULT.addGlyphs(Properties) instead");
    } catch (final SecurityException e) // can occur on System.getProperty
    {
      // PDFBOX-1946 ignore and continue
      GlyphList.LOG.debug("Error getting system property 'glyphlist_ext' - ignoring", e);
    }
  }
      ...
      while (in.ready()) {
        ...
        if (line != null && !line.startsWith("#")) {
          ...
          if (nameToUnicode.containsKey(name)) {
            GlyphList.LOG.warn("duplicate value for " + name + " -> " + parts[1] + " " + nameToUnicode.get(name));
          }
          ...
          final boolean forceOverride = WinAnsiEncoding.INSTANCE.contains(name)
              || MacRomanEncoding.INSTANCE.contains(name) || MacExpertEncoding.INSTANCE.contains(name)
              || SymbolEncoding.INSTANCE.contains(name) || ZapfDingbatsEncoding.INSTANCE.contains(name);
        ...
        try {
          for (int chPos = 3; chPos + 4 <= nameLength; chPos += 4) {
            ...
            if (codePoint > 0xD7FF && codePoint < 0xE000) {
              GlyphList.LOG.warn("Unicode character name with disallowed code area: " + name);
            } else {
          ...
          GlyphList.LOG.warn("Not a number in Unicode character name: " + name);
        }
