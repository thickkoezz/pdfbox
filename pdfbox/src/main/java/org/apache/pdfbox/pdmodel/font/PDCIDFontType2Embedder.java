...
/**
 * Embedded PDCIDFontType2 builder. Helper class to populate a PDCIDFontType2
 * and its parent PDType0Font from a TTF.
 *
 * @author Keiji Suzuki
 * @author John Hewson
 */
final class PDCIDFontType2Embedder extends TrueTypeEmbedder {
  ...
  protected void buildSubset(final InputStream ttfSubset, final String tag, final Map<Integer, Integer> gidToCid)
      throws IOException {
    // build CID2GIDMap, because the content stream has been written with the old
    // GIDs
    final Map<Integer, Integer> cidToGid = new HashMap<>(gidToCid.size());
    for (final Map.Entry<Integer, Integer> entry : gidToCid.entrySet()) {
      final int newGID = entry.getKey();
      final int oldGID = entry.getValue();
      cidToGid.put(oldGID, newGID);
    }
    // build unicode mapping before subsetting as the subsetted font won't have a
    // cmap
  ...
  private void addNameTag(final String tag) throws IOException {
   ...
   * including those that don't have a GID.
   */
  ...
  private boolean buildVerticalHeader(final COSDictionary cidFont) throws IOException {
    ...
    if (vhea == null) {
      PDCIDFontType2Embedder.LOG.warn("Font to be subset is set to vertical, but has no 'vhea' table");
      return false;
    }
  ...
  /**
   * Builds vertical metrics with a custom CIDToGIDMap (for embedding font
   * subset).
   */
  private void buildVerticalMetrics(final Map<Integer, Integer> cidToGid) throws IOException {
    ...
    // used by a conforming
    // reader. The only way to specify vertical metrics in PDF shall be by means of
    // the DW2 and W2
