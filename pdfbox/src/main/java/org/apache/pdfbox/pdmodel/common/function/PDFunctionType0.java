...
public class PDFunctionType0 extends PDFunction {
  ...
  /**
   * An array of 2 x m numbers specifying the linear mapping of input values into
   * the domain of the function's sample table. Default value: [ 0 (Size0 - 1) 0
   * (Size1 - 1) ...].
   */
  private COSArray encode = null;
  /**
   * An array of 2 x n numbers specifying the linear mapping of sample values into
   * the range appropriate for the function's output values. Default value: same
   * as the value of Range
   */
  ...
  /**
   * The "Size" entry, which is the number of samples in each input dimension of
   * the sample table.
   *
  ...
  private int[][] getSamples() {
    if (samples == null) {
      int arraySize = 1;
      final int numberOfInputValues = getNumberOfInputParameters();
      ...
      try {
        ...
        // Successive values are adjacent in the bit stream; there is no padding at byte
        // boundaries.
        try (ImageInputStream mciis = new MemoryCacheImageInputStream(getPDStream().createInputStream())) {
          for (int i = 0; i < arraySize; i++) {
            for (int k = 0; k < numberOfOutputValues; k++) {
              // TODO will this cast work properly for 32 bitsPerSample or should we use
              // long[]?
   ...
   * specifying linear and cubic spline interpolation, respectively. Default is 1.
   * See p.170 in PDF spec 1.7.
   *
  ...
  /**
   * Set the number of bits that the output value will take up. Valid values are
   * 1,2,4,8,12,16,24,32.
   *
  ...
  /**
   * calculate array index (structure described in p.171 PDF spec 1.7) in multiple
   * dimensions.
   *
   ...
   * <a href=
   * "http://harmoniccode.blogspot.de/2011/04/bilinear-color-interpolation.html">bilinear
   * interpolation</a> and here's a
   * <a href="https://en.wikipedia.org/wiki/Trilinear_interpolation">trilinear
  ...
  private class Rinterpol {
     ...
     * recursively twice.
     *
     * @param coord coord partially set coordinate (not set from step upwards); gets
     *              fully filled in the last call ("leaf"), where it is used to get
     *              the correct sample
