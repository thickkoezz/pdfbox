...
public final class PDAcroForm implements COSObjectable {
   ...
   *
   * If these are missing create default entries similar to Adobe Reader / Adobe
   * Acrobat
   *
   */
  ...
  /**
   * This method will import an entire FDF document into the PDF document that
  ...
  public void flatten() throws IOException {
    // for dynamic XFA forms there is no flatten as this would mean to do a
    // rendering
    // from the XFA content into a static PDF.
    if (xfaIsDynamic()) {
      PDAcroForm.LOG.warn("Flatten for a dynamix XFA form is not supported");
      return;
    }
   ...
   *
   * @param fields
   * @param refreshAppearances if set to true the appearances for the form field
  ...
  public void flatten(final List<PDField> fields, final boolean refreshAppearances) throws IOException {
    ...
    if (!refreshAppearances && getNeedAppearances()) {
      PDAcroForm.LOG
          .warn("acroForm.getNeedAppearances() returns true, " + "visual field appearances may not have been set");
      PDAcroForm.LOG.warn(
    ...
    final Map<COSDictionary, Map<COSDictionary, PDAnnotationWidget>> pagesWidgetsMap = buildPagesWidgetsMap(fields);

    // preserve all non widget annotations
    for (final PDPage page : document.getPages()) {
      final Map<COSDictionary, PDAnnotationWidget> widgetsForPageMap = pagesWidgetsMap.get(page.getCOSObject());
      ...
      for (final PDAnnotation annotation : page.getAnnotations()) {
        if (widgetsForPageMap != null && widgetsForPageMap.get(annotation.getCOSObject()) == null) {
          ...
          final boolean needsTranslation = resolveNeedsTranslation(appearanceStream);

          // scale the appearance stream - mainly needed for images
          // in buttons and signatures
          final boolean needsScaling = resolveNeedsScaling(appearanceStream);
          ...
          if (needsScaling) {
            final PDRectangle bbox = appearanceStream.getBBox();
            final PDRectangle fieldRect = annotation.getRectangle();

            if (Float.compare(bbox.getWidth() - fieldRect.getWidth(), 0) != 0
                && Float.compare(bbox.getHeight() - fieldRect.getHeight(), 0) != 0) {
              final float xScale = fieldRect.getWidth() / bbox.getWidth();
              final float yScale = fieldRect.getHeight() / bbox.getHeight();
              final Matrix scalingMatrix = Matrix.getScaleInstance(xScale, yScale);
  ...
  /**
   * Refreshes the appearance streams and appearance dictionaries for the widget
   * annotations of all fields.
   *
   * @throws IOException
   */
   ...
   * documents root fields might either be terminal fields, non-terminal fields or
   * a mixture of both. Non-terminal fields mark branches which contents can be
   * retrieved using {@link PDNonTerminalField#getChildren()}.
   *
   * @return A list of the documents root fields, never null. If there are no
   *         fields then this method returns an empty list.
   */
  public List<PDField> getFields() {
    final COSArray cosFields = (COSArray) dictionary.getDictionaryObject(COSName.FIELDS);
    if (cosFields == null)
  ...
  /**
   * This will tell this form to cache the fields into a Map structure for fast
   * access via the getField method. The default is false. You would want this to
   * be false if you were changing the COSDictionary behind the scenes, otherwise
   * setting this to true is acceptable.
   *
  ...
  /**
   * True if the viewing application should construct the appearances of all field
  ...
  /**
   * This will get the document-wide default value for the quadding/justification
  ...
  /**
   * This will set the document-wide default value for the quadding/justification
   * of variable text fields. See the QUADDING constants of
   * {@link PDVariableText}.
   *
  ...
  public boolean isSignaturesExist() {
    return dictionary.getFlag(COSName.SIG_FLAGS, PDAcroForm.FLAG_SIGNATURES_EXIST);
  }
  ...
  public void setSignaturesExist(final boolean signaturesExist) {
    dictionary.setFlag(COSName.SIG_FLAGS, PDAcroForm.FLAG_SIGNATURES_EXIST, signaturesExist);
  }

  /**
   * Determines if AppendOnly is set.
   *
   * @return true if the document contains signatures that may be invalidated if
   *         the file is saved.
   */
  public boolean isAppendOnly() {
    return dictionary.getFlag(COSName.SIG_FLAGS, PDAcroForm.FLAG_APPEND_ONLY);
  }
  ...
  public void setAppendOnly(final boolean appendOnly) {
    dictionary.setFlag(COSName.SIG_FLAGS, PDAcroForm.FLAG_APPEND_ONLY, appendOnly);
  }
   ...
   */
  private boolean resolveNeedsTranslation(final PDAppearanceStream appearanceStream) {
    ...
    if (resources != null && resources.getXObjectNames().iterator().hasNext()) {
      ...
      final Iterator<COSName> xObjectNames = resources.getXObjectNames().iterator();
      ...
      while (xObjectNames.hasNext()) {
        try {
          ...
          final PDXObject xObject = resources.getXObject(xObjectNames.next());
          if (xObject instanceof PDFormXObject) {
            final PDRectangle bbox = ((PDFormXObject) xObject).getBBox();
            final float llX = bbox.getLowerLeftX();
            final float llY = bbox.getLowerLeftY();
          ...
          PDAcroForm.LOG.debug("Couldn't resolve possible need for translation - ignoring, content might be misplaced",
              e);
        }
      }
   ...
   */
  private boolean resolveNeedsScaling(final PDAppearanceStream appearanceStream) {
  ...
  private Map<COSDictionary, Map<COSDictionary, PDAnnotationWidget>> buildPagesWidgetsMap(final List<PDField> fields) {
    final Map<COSDictionary, Map<COSDictionary, PDAnnotationWidget>> pagesAnnotationsMap = new HashMap<>();
    ...
    if (hasMissingPageRef) {
      PDAcroForm.LOG.warn("There has been a widget with a missing page reference. Please report to the PDFBox project");
    }
  ...
  private void removeFields(final List<PDField> fields) {
    for (final PDField field : fields) {
      if (field.getParent() == null) {
        ...
        final COSArray kids = (COSArray) field.getParent().getCOSObject().getDictionaryObject(COSName.KIDS);
        for (int i = 0; i < kids.size(); i++) {
          final COSDictionary element = (COSDictionary) kids.getObject(i);
          if (field.getCOSObject().equals(element)) {
            kids.remove(i);
          }
        }
