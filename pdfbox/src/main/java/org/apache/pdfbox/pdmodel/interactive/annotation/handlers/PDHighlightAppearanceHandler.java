...
public class PDHighlightAppearanceHandler extends PDAbstractAppearanceHandler {
  ...
  public void generateNormalAppearance() {
    ...
    // TODO padding should consider the curves too; needs to know in advance where
    // the curve is
    ...
    try (PDAppearanceContentStream cs = getNormalAppearanceAsContentStream()) {
      ...
      final PDFormXObject frm1 = new PDFormXObject(new COSStream());
      final PDFormXObject frm2 = new PDFormXObject(new COSStream());
      ...
      try (PDFormContentStream frm2CS = new PDFormContentStream(frm2)) {
        ...
        while (of + 7 < pathsArray.length) {
          ...
          // for "curvy" highlighting, two Bézier control points are used that seem to
          // have a
          ...
          if (Float.compare(pathsArray[of + 0], pathsArray[of + 4]) == 0) {
            // horizontal highlight
            frm2CS.curveTo(pathsArray[of + 4] - delta, pathsArray[of + 5] + delta, pathsArray[of + 0] - delta,
                pathsArray[of + 1] - delta, pathsArray[of + 0], pathsArray[of + 1]);
          ...
          if (Float.compare(pathsArray[of + 2], pathsArray[of + 6]) == 0) {
            // horizontal highlight
            frm2CS.curveTo(pathsArray[of + 2] + delta, pathsArray[of + 3] - delta, pathsArray[of + 6] + delta,
                pathsArray[of + 7] + delta, pathsArray[of + 6], pathsArray[of + 7]);
      ...
      PDHighlightAppearanceHandler.LOG.error(ex);
    }
  }
