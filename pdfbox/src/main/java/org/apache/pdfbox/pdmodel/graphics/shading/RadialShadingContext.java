...
public class RadialShadingContext extends ShadingContext implements PaintContext {
   ...
   *                     color conversion.
   */
  public RadialShadingContext(final PDShadingType3 shading, final ColorModel colorModel, final AffineTransform xform,
      final Matrix matrix, final Rectangle deviceBounds) throws IOException {
    super(shading, colorModel, xform, matrix);
    radialShadingType = shading;
    coords = shading.getCoords().toFloatArray();

    // domain values
    if (radialShadingType.getDomain() != null) {
    ...
    try {
      ...
      RadialShadingContext.LOG.error(ex.getMessage() + ", matrix: " + matrix, ex);
  ...
  /**
   * Calculate the color on the line that connects two circles' centers and store
  ...
  private float[] calculateInputValues(final double x, final double y) {
    ...
    // circle moving
    // between the start circle and the end circle as a function of s are as
    // follows:
    //
    // xc(s) = x0 + s * (x1 - x0) yc(s) = y0 + s * (y1 - y0) r(s) = r0 + s * (r1 -
    // r0)
    //
    // Given a geometric coordinate position (x, y) in or along the gradient fill,
    // the
    // corresponding value of s can be determined by solving the quadratic
    // constraint equation:
    //
