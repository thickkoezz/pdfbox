...
public final class LosslessFactory {
   ...
   * this segment) However a new colorspace will be created for each image. So if
   * you create a PDF with several such images, consider replacing the colorspace
  ...
  public static PDImageXObject createFromImage(final PDDocument document, final BufferedImage image)
    ...
    if (image.getType() == BufferedImage.TYPE_BYTE_GRAY && image.getColorModel().getPixelSize() <= 8
      ...
      if (LosslessFactory.usePredictorEncoder) {
        ...
        if (pdImageXObject != null) {
          if (pdImageXObject.getColorSpace() == PDDeviceRGB.INSTANCE && pdImageXObject.getBitsPerComponent() < 16
            ...
            final PDImageXObject pdImageXObjectClassic = LosslessFactory.createFromRGBImage(image, document);
  ...
  private static PDImageXObject createFromGrayImage(final BufferedImage image, final PDDocument document)
    ...
    return LosslessFactory.prepareImageXObject(document, baos.toByteArray(), image.getWidth(), image.getHeight(), bpc,
        PDDeviceGray.INSTANCE);
  }

  private static PDImageXObject createFromRGBImage(final BufferedImage image, final PDDocument document)
      throws IOException {
    final int height = image.getHeight();
    final int width = image.getWidth();
    final int[] rgbLineBuffer = new int[width];
    ...
    final PDImageXObject pdImage = LosslessFactory.prepareImageXObject(document, imageData, image.getWidth(),
        image.getHeight(), bpc, deviceColorSpace);
    if (transparency != Transparency.OPAQUE) {
      final PDImageXObject pdMask = LosslessFactory.prepareImageXObject(document, alphaImageData, image.getWidth(),
  ...
  private static PDImageXObject prepareImageXObject(final PDDocument document, final byte[] byteArray, final int width,
      final int height, final int bitsPerComponent, final PDColorSpace initColorSpace) throws IOException {
    ...
    return new PDImageXObject(document, encodedByteStream, COSName.FLATE_DECODE, width, height, bitsPerComponent,
        initColorSpace);
  }
  ...
  private static class PredictorEncoder {
    ...
    PredictorEncoder(final PDDocument document, final BufferedImage image) {
      ...
      componentsPerPixel = image.getColorModel().getNumComponents();
      transferType = image.getRaster().getTransferType();
      bytesPerComponent = transferType == DataBuffer.TYPE_SHORT || transferType == DataBuffer.TYPE_USHORT ? 2 : 1;

      // Only the bytes we need in the output (excluding alpha)
      bytesPerPixel = image.getColorModel().getNumColorComponents() * bytesPerComponent;

      height = image.getHeight();
      width = image.getWidth();
      imageType = image.getType();
      hasAlpha = image.getColorModel().getNumComponents() != image.getColorModel().getNumColorComponents();
      alphaImageData = hasAlpha ? new byte[width * height * bytesPerComponent] : null;
      ...
      dataRawRowNone = new byte[dataRowByteCount];
      dataRawRowSub = new byte[dataRowByteCount];
      dataRawRowUp = new byte[dataRowByteCount];
      dataRawRowAverage = new byte[dataRowByteCount];
      dataRawRowPaeth = new byte[dataRowByteCount];
      ...
      //
      // x => current pixel
      aValues = new byte[bytesPerPixel];
      cValues = new byte[bytesPerPixel];
      bValues = new byte[bytesPerPixel];
      xValues = new byte[bytesPerPixel];
    }

    /**
     * Tries to compress the image using a predictor.
     *
     * @return the image or null if it is not possible to encoded the image (e.g.
     *         not supported raster format etc.)
     */
    PDImageXObject encode() throws IOException {
      ...
      final ByteArrayOutputStream stream = new ByteArrayOutputStream(height * width * bytesPerPixel / 2);
      ...
      for (int rowNum = 0; rowNum < height; rowNum++) {
        ...
        for (int indexInTransferRow = 0; indexInTransferRow < elementsInTransferRow; indexInTransferRow += elementsInRowPerPixel, alphaPtr += bytesPerComponent) {
          ...
          if (transferRowByte != null) {
            ...
            // This must be short[]
            PredictorEncoder.copyShortsToBytes(transferRowShort, indexInTransferRow, xValues, alphaImageData, alphaPtr);
            PredictorEncoder.copyShortsToBytes(prevRowShort, indexInTransferRow, bValues, null, 0);
          }
          ...
          for (int bytePtr = 0; bytePtr < length; bytePtr++) {
            ...
            dataRawRowSub[writerPtr] = PredictorEncoder.pngFilterSub(x, a);
            dataRawRowUp[writerPtr] = PredictorEncoder.pngFilterUp(x, b);
            dataRawRowAverage[writerPtr] = PredictorEncoder.pngFilterAverage(x, a, b);
            dataRawRowPaeth[writerPtr] = PredictorEncoder.pngFilterPaeth(x, a, b, c);
            writerPtr++;
          }
    ...
    private PDImageXObject preparePredictorPDImage(final ByteArrayOutputStream stream, final int bitsPerComponent)
        throws IOException {
      ...
      if (srcCspace instanceof ICC_ColorSpace) {
        ...
        if (profile != ICC_Profile.getInstance(ColorSpace.CS_sRGB)) {
          ...
          try (OutputStream outputStream = pdProfile.getPDStream().createOutputStream(COSName.FLATE_DECODE)) {
      ...
      final PDImageXObject imageXObject = new PDImageXObject(document, new ByteArrayInputStream(stream.toByteArray()),
          COSName.FLATE_DECODE, w, h, bitsPerComponent, pdColorSpace);
     ...
     * would need to do a brute force check how all the different encoded rows
     * compress in the zip stream together. You have would have to check
    ...
    private byte[] chooseDataRowToWrite() {
      ...
      long estCompressSum = PredictorEncoder.estCompressSum(dataRawRowNone);
      final long estCompressSumSub = PredictorEncoder.estCompressSum(dataRawRowSub);
      final long estCompressSumUp = PredictorEncoder.estCompressSum(dataRawRowUp);
      final long estCompressSumAvg = PredictorEncoder.estCompressSum(dataRawRowAverage);
      final long estCompressSumPaeth = PredictorEncoder.estCompressSum(dataRawRowPaeth);
