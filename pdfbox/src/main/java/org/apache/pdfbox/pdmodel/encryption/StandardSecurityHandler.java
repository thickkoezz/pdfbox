...
public final class StandardSecurityHandler extends SecurityHandler {
  ...
  private static final byte[] ENCRYPT_PADDING = { (byte) 0x28, (byte) 0xBF, (byte) 0x4E, (byte) 0x5E, (byte) 0x4E,
   ...
   * 1.7 Supplement ExtensionLevel: 3</a> and <a href=
   * "http://intranet.pdfa.org/wp-content/uploads/2016/08/ISO_DIS_32000-2-DIS4.pdf">PDF
   * Spec 2.0</a>.
   *
  ...
  private int computeVersionNumber() {
    if (keyLength == 40)
      return StandardSecurityHandler.DEFAULT_VERSION;
  ...
  /**
   * Computes the revision version of the StandardSecurityHandler to use regarding
   * the version number and the permissions bits set. See PDF Spec 1.6 p98
   *
  ...
  public void prepareForDecryption(final PDEncryption encryption, final COSArray documentIDArray,
      final DecryptionMaterial decryptionMaterial) throws InvalidPasswordException, IOException {
    ...
    if (isOwnerPassword(password.getBytes(passwordCharset), userKey, ownerKey, dicPermissions, documentIDBytes,
      ...
      encryptionKey = computeEncryptedKey(computedPassword, ownerKey, userKey, oe, ue, dicPermissions, documentIDBytes,
          dicRevision, dicLength, encryptMetadata, true);
    } else if (isUserPassword(password.getBytes(passwordCharset), userKey, ownerKey, dicPermissions, documentIDBytes,
        dicRevision, dicLength, encryptMetadata)) {
      currentAccessPermission = new AccessPermission(dicPermissions);
      currentAccessPermission.setReadOnly();
      setCurrentAccessPermission(currentAccessPermission);

      encryptionKey = computeEncryptedKey(password.getBytes(passwordCharset), ownerKey, userKey, oe, ue, dicPermissions,
          documentIDBytes, dicRevision, dicLength, encryptMetadata, false);
    } else
  ...
  private void validatePerms(final PDEncryption encryption, final int dicPermissions, final boolean encryptMetadata)
      throws IOException {
    try {
      ...
      if (perms[9] != 'a' || perms[10] != 'd' || perms[11] != 'b') {
        StandardSecurityHandler.LOG.warn("Verification of permissions failed (constant)");
      }
      ...
      final int permsP = perms[0] & 0xFF | (perms[1] & 0xFF) << 8 | (perms[2] & 0xFF) << 16 | (perms[3] & 0xFF) << 24;

      if (permsP != dicPermissions) {
        StandardSecurityHandler.LOG.warn("Verification of permissions failed (" + String.format("%08X", permsP) + " != "
            + String.format("%08X", dicPermissions) + ")");
      }

      if (encryptMetadata && perms[8] != 'T' || !encryptMetadata && perms[8] != 'F') {
        StandardSecurityHandler.LOG.warn("Verification of permissions failed (EncryptMetadata)");
      }
  ...
  public void prepareDocumentForEncryption(final PDDocument document) throws IOException {
    ...
    encryptionDictionary.setFilter(StandardSecurityHandler.FILTER);
    ...
    if (revision == 6) {
      ...
      prepareEncryptionDictRev2345(ownerPassword, userPassword, encryptionDictionary, permissionInt, document, revision,
          length);
    }
  ...
  private void prepareEncryptionDictRev6(final String ownerPassword, final String userPassword,
      final PDEncryption encryptionDictionary, final int permissionInt) throws IOException {
    try {
      ...
      final byte[] userPasswordBytes = StandardSecurityHandler.truncate127(userPassword.getBytes(Charsets.UTF_8));
      ...
      final byte[] hashU = StandardSecurityHandler.computeHash2B(
          StandardSecurityHandler.concat(userPasswordBytes, userValidationSalt), userPasswordBytes, null);
      final byte[] u = StandardSecurityHandler.concat(hashU, userValidationSalt, userKeySalt);

      // Algorithm 8b: Compute UE
      final byte[] hashUE = StandardSecurityHandler
          .computeHash2B(StandardSecurityHandler.concat(userPasswordBytes, userKeySalt), userPasswordBytes, null);
      cipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(hashUE, "AES"),
          // "an initialization vector of zero"
          new IvParameterSpec(new byte[16]));
      final byte[] ue = cipher.doFinal(encryptionKey);

      // Algorithm 9a: Compute O
      final byte[] ownerPasswordBytes = StandardSecurityHandler.truncate127(ownerPassword.getBytes(Charsets.UTF_8));
      ...
      final byte[] hashO = StandardSecurityHandler.computeHash2B(
          StandardSecurityHandler.concat(ownerPasswordBytes, ownerValidationSalt, u), ownerPasswordBytes, u);
      final byte[] o = StandardSecurityHandler.concat(hashO, ownerValidationSalt, ownerKeySalt);

      // Algorithm 9b: Compute OE
      final byte[] hashOE = StandardSecurityHandler
          .computeHash2B(StandardSecurityHandler.concat(ownerPasswordBytes, ownerKeySalt, u), ownerPasswordBytes, u);
      cipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(hashOE, "AES"),
          // "an initialization vector of zero"
          new IvParameterSpec(new byte[16]));
      final byte[] oe = cipher.doFinal(encryptionKey);
  ...
  private void prepareEncryptionDictRev2345(final String ownerPassword, final String userPassword,
      final PDEncryption encryptionDictionary, final int permissionInt, final PDDocument document, final int revision,
    ...
    if (idArray == null || idArray.size() < 2) {
      ...
      final byte[] id = md.digest(toString().getBytes(Charsets.ISO_8859_1));
    ...
    final byte[] ownerBytes = computeOwnerPassword(ownerPassword.getBytes(Charsets.ISO_8859_1),
        userPassword.getBytes(Charsets.ISO_8859_1), revision, length);

    final byte[] userBytes = computeUserPassword(userPassword.getBytes(Charsets.ISO_8859_1), ownerBytes, permissionInt,
        id.getBytes(), revision, length, true);

    encryptionKey = computeEncryptedKey(userPassword.getBytes(Charsets.ISO_8859_1), ownerBytes, null, null, null,
        permissionInt, id.getBytes(), revision, length, true, false);
  ...
  public boolean isOwnerPassword(final byte[] ownerPassword, final byte[] user, final byte[] owner,
    ...
    if (encRevision == 6 || encRevision == 5) {
      final byte[] truncatedOwnerPassword = StandardSecurityHandler.truncate127(ownerPassword);
      ...
      byte[] hash;
      if (encRevision == 5) {
        hash = StandardSecurityHandler.computeSHA256(truncatedOwnerPassword, oValidationSalt, user);
      } else {
   ...
   *
   * @throws IOException If there is an error accessing data while generating the
   *                     user password.
   */
  public byte[] getUserPassword(final byte[] ownerPassword, final byte[] owner, final int encRevision, final int length)
      throws IOException {
   ...
   * @param encryptMetadata  The encryption metadata
   * @param isOwnerPassword  whether the password given is the owner password (for
   *                         revision 6)
   *
   ...
   */
  public byte[] computeEncryptedKey(final byte[] password, final byte[] o, final byte[] u, final byte[] oe,
      final byte[] ue, final int permissions, final byte[] id, final int encRevision, final int keyLengthInBytes,
      final boolean encryptMetadata, final boolean isOwnerPassword) throws IOException {
    if (encRevision == 6 || encRevision == 5)
  ...
  private byte[] computeEncryptedKeyRev56(final byte[] password, final boolean isOwnerPassword, final byte[] o,
      final byte[] u, final byte[] oe, final byte[] ue, final int encRevision) throws IOException {
    ...
    if (isOwnerPassword) {
      ...
      if (encRevision == 5) {
        hash = StandardSecurityHandler.computeSHA256(password, oKeySalt, u);
      } else {
      ...
      if (encRevision == 5) {
        hash = StandardSecurityHandler.computeSHA256(password, uKeySalt, null);
      } else {
   ...
   */
  public byte[] computeUserPassword(final byte[] password, final byte[] owner, final int permissions, final byte[] id,
      final int encRevision, final int keyLengthInBytes, final boolean encryptMetadata) throws IOException {
    final ByteArrayOutputStream result = new ByteArrayOutputStream();
    final byte[] encKey = computeEncryptedKey(password, owner, null, null, null, permissions, id, encRevision,
        keyLengthInBytes, encryptMetadata, true);

    if (encRevision == 2) {
      encryptDataRC4(encKey, StandardSecurityHandler.ENCRYPT_PADDING, result);
    } else if (encRevision == 3 || encRevision == 4) {
      final MessageDigest md = MessageDigests.getMD5();
   ...
   */
  public byte[] computeOwnerPassword(final byte[] ownerPassword, final byte[] userPassword, final int encRevision,
      final int length) throws IOException {
  ...
  private byte[] truncateOrPad(final byte[] password) {
    ...
    System.arraycopy(StandardSecurityHandler.ENCRYPT_PADDING, 0, padded, bytesBeforePad,
        StandardSecurityHandler.ENCRYPT_PADDING.length - bytesBeforePad);
    return padded;
  }
  ...
  private boolean isUserPassword234(final byte[] password, final byte[] user, final byte[] owner, final int permissions,
      final byte[] id, final int encRevision, final int length, final boolean encryptMetadata) throws IOException {
    final byte[] passwordBytes = computeUserPassword(password, owner, permissions, id, encRevision, length,
        encryptMetadata);
    if (encRevision == 2)
  ...
  private boolean isUserPassword56(final byte[] password, final byte[] user, final int encRevision) throws IOException {
    final byte[] truncatedPassword = StandardSecurityHandler.truncate127(password);
    ...
    byte[] hash;
    if (encRevision == 5) {
      hash = StandardSecurityHandler.computeSHA256(truncatedPassword, uValidationSalt, null);
    } else {
  ...
  public boolean isUserPassword(final String password, final byte[] user, final byte[] owner, final int permissions,
      ...
      throws IOException {
    if (encRevision == 6 || encRevision == 5)
      return isUserPassword(password.getBytes(Charsets.UTF_8), user, owner, permissions, id, encRevision,
          keyLengthInBytes, encryptMetadata);
    else
      return isUserPassword(password.getBytes(Charsets.ISO_8859_1), user, owner, permissions, id, encRevision,
          keyLengthInBytes, encryptMetadata);
  }
  ...
  public boolean isOwnerPassword(final String password, final byte[] user, final byte[] owner, final int permissions,
      final byte[] id, final int encRevision, final int keyLengthInBytes, final boolean encryptMetadata)
      throws IOException {
    return isOwnerPassword(password.getBytes(Charsets.ISO_8859_1), user, owner, permissions, id, encRevision,
        keyLengthInBytes, encryptMetadata);
  }
  ...
  private byte[] computeHash2A(final byte[] password, final byte[] salt, final byte[] u) throws IOException {
    ...
    final byte[] truncatedPassword = StandardSecurityHandler.truncate127(password);
    final byte[] input = StandardSecurityHandler.concat(truncatedPassword, salt, userKey);
  ...
  private static byte[] computeHash2B(final byte[] input, final byte[] password, final byte[] userKey)
    ...
    try {
      ...
      for (int round = 0; round < 64 || (e[e.length - 1] & 0xFF) > round - 32; round++) {
        ...
        final String nextHash = StandardSecurityHandler.HASHES_2B[remainder.intValue()];
  ...
  private static void logIfStrongEncryptionMissing() {
    try {
      if (Cipher.getMaxAllowedKeyLength("AES") != Integer.MAX_VALUE) {
        StandardSecurityHandler.LOG.warn("JCE unlimited strength jurisdiction policy files are not installed");
      }
    } catch (final NoSuchAlgorithmException ex) {
      StandardSecurityHandler.LOG.debug("AES Algorithm not available", ex);
    }
  }
