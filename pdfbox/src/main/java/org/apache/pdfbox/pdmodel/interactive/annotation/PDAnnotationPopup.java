...
/**
 * This is the class that represents a popup annotation. Introduced in PDF 1.3
 * specification
 *
 * @author Paul King
 */
public class PDAnnotationPopup extends PDAnnotation {
  ...
  public PDAnnotationPopup() {
    getCOSObject().setName(COSName.SUBTYPE, PDAnnotationPopup.SUB_TYPE);
  }
  ...
  public PDAnnotationMarkup getParent() {
    ...
    try {
      am = (PDAnnotationMarkup) PDAnnotation
          .createAnnotation(getCOSObject().getDictionaryObject(COSName.PARENT, COSName.P));
    } catch (final IOException ioe) {
      PDAnnotationPopup.LOG.debug("An exception while trying to get the parent markup - ignoring", ioe);
