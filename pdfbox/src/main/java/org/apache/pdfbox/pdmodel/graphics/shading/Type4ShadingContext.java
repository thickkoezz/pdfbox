...
class Type4ShadingContext extends GouraudShadingContext {
  ...
  Type4ShadingContext(final PDShadingType4 shading, final ColorModel cm, final AffineTransform xform,
    ...
    Type4ShadingContext.LOG.debug("Type4ShadingContext");

    bitsPerFlag = shading.getBitsPerFlag();
    // TODO handle cases where bitperflag isn't 8
    Type4ShadingContext.LOG.debug("bitsPerFlag: " + bitsPerFlag);
    setTriangleList(collectTriangles(shading, xform, matrix));
    createPixelTable(deviceBounds);
  }

  @SuppressWarnings("squid:S1166")
  private List<ShadedTriangle> collectTriangles(final PDShadingType4 freeTriangleShadingType,
    ...
    try (ImageInputStream mciis = new MemoryCacheImageInputStream(stream.createInputStream())) {
      ...
      try {
        ...
        Type4ShadingContext.LOG.error(ex);
      }
      ...
      while (!eof) {
        ...
        try {
          ...
          case 0:
            ...
            flag = (byte) (mciis.readBits(bitsPerFlag) & 3);
            if (flag != 0) {
              Type4ShadingContext.LOG.error("bad triangle: " + flag);
            }
          ...
          case 2:
            ...
            if (lastIndex < 0) {
              Type4ShadingContext.LOG.error("broken data stream: " + list.size());
            } else {
            ...
            break;
          default:
            Type4ShadingContext.LOG.warn("bad flag: " + flag);
            break;
          }
        } catch (final EOFException ex) {
          eof = true;
