...
final class ToUnicodeWriter {
  ...
  public void writeTo(final OutputStream out) throws IOException {
    final BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(out, Charsets.US_ASCII));
    ...
    final int batchCount = (int) Math.ceil(srcFrom.size() / (double) ToUnicodeWriter.MAX_ENTRIES_PER_OPERATOR);
    for (int batch = 0; batch < batchCount; batch++) {
      final int count = batch == batchCount - 1 ? srcFrom.size() - ToUnicodeWriter.MAX_ENTRIES_PER_OPERATOR * batch
          : ToUnicodeWriter.MAX_ENTRIES_PER_OPERATOR;
      writer.write(count + " beginbfrange\n");
      for (int j = 0; j < count; j++) {
        final int index = batch * ToUnicodeWriter.MAX_ENTRIES_PER_OPERATOR + j;
        writer.write('<');
