...
public class PDSignatureField extends PDTerminalField {
  ...
  private String generatePartialName() {
    ...
    final Set<String> sigNames = new HashSet<>();
    for (final PDField field : getAcroForm().getFieldTree()) {
   ...
   */
  @Deprecated
  public void setSignature(final PDSignature value) throws IOException {
    setValue(value);
  }
  ...
  /**
   * <b>This will throw an UnsupportedOperationException if used as the signature
   * fields value can't be set using a String</b>
   *
   ...
   */
  @Override
  public void setValue(final String value) throws UnsupportedOperationException {
    throw new UnsupportedOperationException(
        "Signature fields don't support setting the value as String " + "- use setValue(PDSignature value) instead");
  }
  ...
  /**
   * <p>
   * (Optional; PDF 1.) A seed value dictionary containing information that
   * constrains the properties of a signature that is applied to the field.
   * </p>
   *
  ...
  void constructAppearances() throws IOException {
    ...
    if (widget != null) {
      ...
      if (widget.getRectangle() == null || Float.compare(widget.getRectangle().getHeight(), 0) == 0
          && Float.compare(widget.getRectangle().getWidth(), 0) == 0 || widget.isNoView() || widget.isHidden())
        return;

      // TODO: implement appearance generation for signatures
      PDSignatureField.LOG.warn(
          "Appearance generation for signature fields not yet implemented - you need to generate/update that manually");
    }
  }
}
