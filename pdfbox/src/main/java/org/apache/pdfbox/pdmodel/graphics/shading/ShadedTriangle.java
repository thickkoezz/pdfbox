...
class ShadedTriangle {
  ...
  /*
   * degree = 3 describes a normal triangle, degree = 2 when a triangle
   * degenerates to a line, degree = 1 when a triangle degenerates to a point
   */
  private final int degree;

  // describes a rasterized line when a triangle degerates to a line, otherwise
  // null
  ...
  ShadedTriangle(final Point2D[] p, final float[][] c) {
    ...
    if (degree == 2) {
      if (overlaps(corner[1], corner[2]) && !overlaps(corner[0], corner[2])) {
        final Point p0 = new Point((int) Math.round(corner[0].getX()), (int) Math.round(corner[0].getY()));
        final Point p1 = new Point((int) Math.round(corner[2].getX()), (int) Math.round(corner[2].getY()));
        line = new Line(p0, p1, color[0], color[2]);
      } else {
        final Point p0 = new Point((int) Math.round(corner[1].getX()), (int) Math.round(corner[1].getY()));
        final Point p1 = new Point((int) Math.round(corner[2].getX()), (int) Math.round(corner[2].getY()));
  ...
  public boolean contains(final Point2D p) {
    ...
    /*
     * if corner[0] and point p are on different sides of line from corner[1] to
     * corner[2], p is outside of the triangle
     */
    if (pv0 * v0 < 0)
      return false;
    final double pv1 = edgeEquationValue(p, corner[2], corner[0]);
    /*
     * if vertex corner[1] and point p are on different sides of line from corner[2]
     * to corner[0], p is outside of the triangle
     */
    if (pv1 * v1 < 0)
      return false;
    final double pv2 = edgeEquationValue(p, corner[0], corner[1]);
    /*
     * only left one case: if corner[1] and point p are on different sides of line
     * from corner[2] to corner[0], p is outside of the triangle, otherwise p is
  ...
  /*
   * check whether two points overlaps each other, as points' coordinates are of
   * type double, the coordinates' accuracy used here is 0.001
   */
  ...
  /*
   * two points can define a line equation, adjust the form of the equation to let
   * the rhs equals 0, calculate the lhs value by plugging the coordinate of p in
   * the lhs expression
   */
  ...
  private double getArea(final Point2D a, final Point2D b, final Point2D c) {
    return Math.abs((c.getX() - b.getX()) * (c.getY() - a.getY()) - (c.getX() - a.getX()) * (c.getY() - b.getY()))
        / 2.0;
  }
