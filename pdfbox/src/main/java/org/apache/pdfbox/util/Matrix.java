...
public final class Matrix implements Cloneable {
  static final float[] DEFAULT_SINGLE = { 1, 0, 0, // a b 0 sx hy 0 note: hx and hy are reversed vs. the PDF spec as we
                                                   // use
   ...
   */
  public Matrix() {
    single = new float[Matrix.DEFAULT_SINGLE.length];
    System.arraycopy(Matrix.DEFAULT_SINGLE, 0, single, 0, Matrix.DEFAULT_SINGLE.length);
  }
   ...
   * matrices are discussed in 8.3.3, "Common Transformations" and 8.3.4,
   * "Transformation Matrices" of the PDF specification. For simple purposes
   * (rotate, scale, translate) it is recommended to use the static methods below.
   *
  ...
  /**
   * Convenience method to be used when creating a matrix from unverified data. If
   * the parameter is a COSArray with at least six numbers, a Matrix object is
   * created from the first six numbers and returned. If not, then the identity
   * Matrix is returned.
   *
  ...
  /**
   * This method resets the numbers in this Matrix to the original values, which
  ...
  public void translate(final Vector vector) {
    final Matrix m = Matrix.getTranslateInstance(vector.getX(), vector.getY());
    concatenate(m);
  }
  ...
  public void translate(final float tx, final float ty) {
    final Matrix m = Matrix.getTranslateInstance(tx, ty);
    concatenate(m);
  }
  ...
  public void scale(final float sx, final float sy) {
    final Matrix m = Matrix.getScaleInstance(sx, sy);
    concatenate(m);
  }
  ...
  public void rotate(final double theta) {
    final Matrix m = Matrix.getRotateInstance(theta, 0, 0);
    concatenate(m);
  }

  /**
   * This will take the current matrix and multiply it with a matrix that is
   * passed in.
   *
  ...
  /**
   * This method multiplies this Matrix with the specified other Matrix, storing
   * the product in the specified result Matrix. By reusing Matrix instances like
   * this, multiplication chains can be executed without having to create many
   * temporary Matrix objects.
   * <p>
   * It is allowed to have (other == this) or (result == this) or indeed (other ==
   * result) but if this is done, the backing float[] matrix values may be copied
   * in order to ensure a correct product.
   *
   * @param other  the second operand Matrix in the multiplication
   * @param result the Matrix instance into which the result should be stored. If
   *               result is null, a new Matrix instance is created.
  ...
  public Matrix multiply(final Matrix other, Matrix result) {
    ...
    if (other != null && other.single != null) {
      ...
      // There is the possibility then to overwrite one of the operands with result
      // values
      ...
      if (this == result) {
        final float[] thisOrigVals = new float[single.length];
        System.arraycopy(single, 0, thisOrigVals, 0, single.length);
   ...
   * @deprecated This method is due to be removed, please contact us if you make
   *             use of it.
   */
  @Deprecated
  public Matrix extractScaling() {
    final Matrix matrix = new Matrix();
    matrix.single[0] = single[0];
    matrix.single[4] = single[4];
    return matrix;
  }
   ...
   */
  public static Matrix getScaleInstance(final float sx, final float sy) {
    final Matrix matrix = new Matrix();
    matrix.single[0] = sx;
    matrix.single[4] = sy;
    return matrix;
  ...
  public Matrix extractTranslating() {
    final Matrix matrix = new Matrix();
    matrix.single[6] = single[6];
    matrix.single[7] = single[7];
    return matrix;
  }
   ...
   */
  @Deprecated
  public static Matrix getTranslatingInstance(final float tx, final float ty) {
    return Matrix.getTranslateInstance(tx, ty);
  }
   ...
   * @return A new matrix with just the x/y translating.
   */
  public static Matrix getTranslateInstance(final float tx, final float ty) {
    final Matrix matrix = new Matrix();
    matrix.single[6] = tx;
    matrix.single[7] = ty;
    return matrix;
  ...
  /**
   * Returns the x-scaling factor of this matrix. This is calculated from the
   * scale and shear.
   *
  ...
  public float getScalingFactorX() {
     ...
     *
     * The rotation matrix multiplied with the scaling matrix is: ( x 0 0) ( cos sin
     * 0) ( x*cos x*sin 0) ( 0 y 0) * (-sin cos 0) = (-y*sin y*cos 0) ( 0 0 1) ( 0 0
     * 1) ( 0 0 1)
     *
     * So, if you want to deduce x from the matrix you take M(0,0) = x*cos and
     * M(0,1) = x*sin and use the theorem of Pythagoras
     *
     * sqrt(M(0,0)^2+M(0,1)^2) = sqrt(x2*cos2+x2*sin2) = sqrt(x2*(cos2+sin2)) = <-
     * here is the trick cos2+sin2 is one sqrt(x2) = abs(x)
     */
  ...
  /**
   * Returns the y-scaling factor of this matrix. This is calculated from the
   * scale and shear.
   *
  ...
  /**
   * Get the x position in the matrix. This method is deprecated as it is
   * incorrectly named.
   *
  ...
  public String toString() {
    final String sb = "" + "[" + single[0] + "," + single[1] + "," + single[3] + "," + single[4] + "," + single[6] + ","
        + single[7] + "]";
    return sb;
  }
  ...
  public boolean equals(final Object obj) {
    ...
    if (getClass() != obj.getClass())
      return false;
    return Arrays.equals(single, ((Matrix) obj).single);
  }
}
