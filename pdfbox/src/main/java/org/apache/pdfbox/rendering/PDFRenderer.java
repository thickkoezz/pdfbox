...
/**
 * Renders a PDF document to an AWT BufferedImage. This class may be overridden
 * in order to perform custom rendering.
 *
 * @author John Hewson
 */
public class PDFRenderer {
  ...
  public PDFRenderer(final PDDocument document) {
    ...
    if (!PDFRenderer.kcmsLogged) {
      PDFRenderer.suggestKCMS();
      PDFRenderer.kcmsLogged = true;
    }
  }
  ...
  public void setAnnotationsFilter(final AnnotationFilter annotationsFilter) {
    annotationFilter = annotationsFilter;
  }

  /**
   * Value indicating if the renderer is allowed to subsample images before
  ...
  /**
   * Sets a value instructing the renderer whether it is allowed to subsample
   * images before drawing. The subsampling frequency is determined according to
   * image size and requested scale.
   *
   * Subsampling may be faster and less memory-intensive in some cases, but it may
   * also lead to loss of quality, especially in images with high spatial
   * frequency.
  ...
  /**
   * Set the rendering hints. Use this to influence rendering quality and speed.
   * If you don't set them yourself or pass null, PDFBox will decide <b><u>at
   * runtime</u></b> depending on the destination.
   *
  ...
  /**
   * Returns the given page as an RGB image at the given scale. A scale of 1 will
   * render at 72 DPI.
   *
   * @param pageIndex the zero-based index of the page to be converted
   ...
   */
  public BufferedImage renderImage(final int pageIndex, final float scale, final ImageType imageType,
      final RenderDestination destination) throws IOException {
    ...
    // PDFBOX-4518 the maximum size (w*h) of a buffered image is limited to
    // Integer.MAX_VALUE
    ...
    if (imageType != ImageType.ARGB && hasBlendMode(page)) {
      // PDFBOX-4095: if the PDF has blending on the top level, draw on transparent
      // background
      ...
      // Finally when the page has been rendered, PDF.js draws the RGBA canvas on a
      // white canvas.
    ...
    final RenderingHints actualRenderingHints = renderingHints == null ? createDefaultRenderingHints(g)
        : renderingHints;
    final PageDrawerParameters parameters = new PageDrawerParameters(this, page, subsamplingAllowed, destination,
   ...
   * @throws IOException if the PDF cannot be read
   */
  public void renderPageToGraphics(final int pageIndex, final Graphics2D graphics, final float scaleX,
      final float scaleY) throws IOException {
   ...
   */
  public void renderPageToGraphics(final int pageIndex, final Graphics2D graphics, final float scaleX,
      final float scaleY, final RenderDestination destination) throws IOException {
    final PDPage page = document.getPage(pageIndex);
    ...
    // the end-user may provide a custom PageDrawer
    final RenderingHints actualRenderingHints = renderingHints == null ? createDefaultRenderingHints(graphics)
        : renderingHints;
    final PageDrawerParameters parameters = new PageDrawerParameters(this, page, subsamplingAllowed, destination,
        actualRenderingHints);
    final PageDrawer drawer = createPageDrawer(parameters);
  ...
  private void transform(final Graphics2D graphics, final PDPage page, final float scaleX, final float scaleY) {
    ...
    // TODO should we be passing the scale to PageDrawer rather than messing with
    // Graphics?
    final int rotationAngle = page.getRotation();
    final PDRectangle cropBox = page.getCropBox();
  ...
  private RenderingHints createDefaultRenderingHints(final Graphics2D graphics) {
    ...
    r.put(RenderingHints.KEY_INTERPOLATION, isBitonal(graphics) ? RenderingHints.VALUE_INTERPOLATION_NEAREST_NEIGHBOR
        : RenderingHints.VALUE_INTERPOLATION_BICUBIC);
    r.put(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_QUALITY);
    r.put(RenderingHints.KEY_ANTIALIASING,
        isBitonal(graphics) ? RenderingHints.VALUE_ANTIALIAS_OFF : RenderingHints.VALUE_ANTIALIAS_ON);
    return r;
  }
  ...
  /**
   * Returns the image to which the current page is being rendered. May be null if
   * the page is rendered to a Graphics2D object instead of a BufferedImage.
   */
  ...
  private static void suggestKCMS() {
    ...
    if (PDFRenderer.isMinJdk8() && !"sun.java2d.cmm.kcms.KcmsServiceProvider".equals(cmmProperty)) {
      try {
        ...
        if (version == null || PDFRenderer.isGoodVersion(version, "1.8.0_(\\d+)", 191)
            || PDFRenderer.isGoodVersion(version, "9.0.(\\d+)", 4))
          return;
        PDFRenderer.LOG.info("Your current java version is: " + version);
        PDFRenderer.LOG.info("To get higher rendering speed on old java 1.8 or 9 versions,");
        PDFRenderer.LOG.info("  update to the latest 1.8 or 9 version (>= 1.8.0_191 or >= 9.0.4),");
        PDFRenderer.LOG.info("  or");
        PDFRenderer.LOG.info("  use the option -Dsun.java2d.cmm=sun.java2d.cmm.kcms.KcmsServiceProvider");
        PDFRenderer.LOG
