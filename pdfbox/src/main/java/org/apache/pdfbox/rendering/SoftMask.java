...
class SoftMask implements Paint {
  ...
  private static final ColorModel ARGB_COLOR_MODEL = new BufferedImage(1, 1, BufferedImage.TYPE_INT_ARGB)
      .getColorModel();
   ...
   *                         device space
   * @param backdropColor    the color to be used outside the transparency group’s
   *                         bounding box; if null, black will be used.
   * @param transferFunction the transfer function, may be null.
   */
  SoftMask(final Paint paint, final BufferedImage mask, final Rectangle2D bboxDevice, final PDColor backdropColor,
    ...
    if (backdropColor != null) {
      try {
        ...
        // keep default
        SoftMask.LOG.debug("Couldn't convert backdropColor to RGB - keeping default", ex);
      }
    }
  }
          ...
          if (x1 + x >= 0 && y1 + y >= 0 && x1 + x < mask.getWidth() && y1 + y < mask.getHeight()) {
            ...
            if (transferFunction != null) {
              ...
              try {
                ...
                SoftMask.LOG.debug("Couldn't apply transferFunction - treating as outside", ex);
