 ...
 * <p>
 * Non-isolated groups require that the group backdrop (copied from parent group
 * or page) is used as the initial contents of the image to which the group is
 * rendered. This allows blend modes to blend the group contents with the
 * graphics behind the group. Finally when the group rendering is done, backdrop
 * removal must be computed (see
 * {@link #removeBackdrop(java.awt.image.BufferedImage, int, int)
 * removeBackdrop}). It ensures the backdrop is not rendered twice on the parent
 * but it leaves the effects of blend modes.
 * <p>
 * This class renders the group contents to two images. <code>groupImage</code>
 * is initialized with the backdrop and group contents are drawn over it.
 * <code>groupAlphaImage</code> is initially fully transparent and it
 * accumulates the total alpha of the group contents excluding backdrop.
 * <p>
 * If a non-isolated group uses only the blend mode Normal, it can be optimized
 * and rendered like an isolated group; backdrop usage and removal are not
 * needed.
 */
...
class GroupGraphics extends Graphics2D {
  ...
  GroupGraphics(final BufferedImage groupImage, final Graphics2D groupGraphics) {
    ...
    groupG2D = groupGraphics;
    groupAlphaImage = new BufferedImage(groupImage.getWidth(), groupImage.getHeight(), BufferedImage.TYPE_INT_ARGB);
    alphaG2D = groupAlphaImage.createGraphics();
  }
  ...
  /**
   * Computes backdrop removal. The backdrop removal equation is given in section
   * 11.4.4 in the PDF 32000-1:2008 standard. It returns the final color
   * <code>C</code> for each pixel in the group:<br>
   ...
   * <code>alphagn</code> is the group alpha excluding backdrop (read the alpha
   * channel from <code>groupAlphaImage</code>)<br>
   * <p>
   * The alpha of the result is equal to <code>alphagn</code>, i.e., the alpha
   * channel of <code>groupAlphaImage</code>.
   * <p>
   * The <code>backdrop</code> image may be much larger than
   * <code>groupImage</code> if, for example, the current page is used as the
   * backdrop. Only a specific rectangular region of <code>backdrop</code> is used
   * in the backdrop removal: upper-left corner is at
  ...
  void removeBackdrop(final BufferedImage backdrop, final int offsetX, final int offsetY) {
    ...
    if (groupType == BufferedImage.TYPE_INT_ARGB && groupAlphaType == BufferedImage.TYPE_INT_ARGB
        && (backdropType == BufferedImage.TYPE_INT_ARGB || backdropType == BufferedImage.TYPE_INT_RGB)
        && groupDataBuffer instanceof DataBufferInt && groupAlphaDataBuffer instanceof DataBufferInt
