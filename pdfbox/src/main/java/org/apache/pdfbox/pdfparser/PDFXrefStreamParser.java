...
/**
 * This will parse a PDF 1.5 (or better) Xref stream and extract the xref
...
public class PDFXrefStreamParser extends BaseParser {
   ...
   */
  public PDFXrefStreamParser(final COSStream stream, final COSDocument document, final XrefTrailerResolver resolver)
      throws IOException {
  ...
  public void parse() throws IOException {
    final COSBase w = stream.getDictionaryObject(COSName.W);
    if (!(w instanceof COSArray))
      throw new IOException("/W array is missing in Xref stream");
    final COSArray xrefFormat = (COSArray) w;
    ...
    final List<Long> objNums = new ArrayList<>();
     ...
     */
    final int w0 = xrefFormat.getInt(0, 0);
    final int w1 = xrefFormat.getInt(1, 0);
    final int w2 = xrefFormat.getInt(2, 0);
    final int lineSize = w0 + w1 + w2;
    ...
    while (!seqSource.isEOF() && objIter.hasNext()) {
      ...
      if (w0 == 0) {
        ...
        type = 0;
        /*
         * Grabs the number of bytes specified for the first column in the W array and
         * stores it.
         */
        for (int i = 0; i < w0; i++) {
      ...
      case 2:
        /*
         * object stored in object stream: 2nd argument is object number of object
         * stream 3rd argument is index of object within object stream
         *
         * For sequential PDFParser we do not need this information because These
         * objects are handled by the dereferenceObjects() method since they're only
         * pointing to object numbers
         *
         * However for XRef aware parsers we have to know which objects contain object
         * streams. We will store this information in normal xref mapping table but add
         * object stream number with minus sign in order to distinguish from file
         * offsets
         */
        int objstmObjNr = 0;
        for (int i = 0; i < w1; i++) {
