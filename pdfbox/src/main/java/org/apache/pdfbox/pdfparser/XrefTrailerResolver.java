 ...
 * xref/trailer information after all objects are read using startxref and
 * 'Prev' information (unused XRef/trailer objects are discarded).
 *
 * In case of missing startxref or wrong startxref pointer all XRef/trailer
 * objects are used to create xref table / trailer dictionary in order they
 * occur.
 *
 * For each new xref object/XRef stream method
 * {@link #nextXrefObj(long, XRefType)} must be called with start byte position.
 * All following calls to {@link #setXRef(COSObjectKey, long)} or
 * {@link #setTrailer(COSDictionary)} will add the data for this byte position.
 *
 * After all objects are parsed the startxref position must be provided using
 * {@link #setStartxref(long)}. This is used to build the chain of active
...
public class XrefTrailerResolver {
  ...
  /**
   * Populate XRef HashMap of current XRef object. Will add an Xreftable entry
  ...
  public void setXRef(final COSObjectKey objKey, final long offset) {
    if (curXrefTrailerObj == null) {
      // should not happen...
      XrefTrailerResolver.LOG
          .warn("Cannot add XRef entry for '" + objKey.getNumber() + "' because XRef start was not signalled.");
      return;
    }
    // PDFBOX-3506 check before adding to the map, to avoid entries from the table
    // being
  ...
  /**
   * Sets the byte position of the first XRef (has to be called after very last
   * startxref was read). This is used to resolve chain of active XRef/trailer.
   *
   * In case startxref position is not found we output a warning and use all
   * XRef/trailer objects combined in byte position order. Thus for incomplete PDF
   * documents with missing startxref one could call this method with parameter
   * value -1.
   *
  ...
  public void setStartxref(final long startxrefBytePosValue) {
    ...
    if (curObj == null) {
      ...
      XrefTrailerResolver.LOG
          .warn("Did not found XRef object at specified startxref position " + startxrefBytePosValue);
  ...
  /**
   * Returns object numbers which are referenced as contained in object stream
   * with specified object number.
   *
   * This will scan resolved xref table for all entries having negated stream
   * object number as value.
   *
   * @param objstmObjNr object number of object stream for which contained object
   *                    numbers should be returned
   *
   * @return set of object numbers referenced for given object stream or
   *         <code>null</code> if {@link #setStartxref(long)} was not called
   *         before so that no resolved xref table exists
   */
  public Set<Long> getContainedObjectNumbers(final int objstmObjNr) {
    ...
    for (final Entry<COSObjectKey, Long> xrefEntry : resolvedXrefTrailer.xrefTable.entrySet()) {
