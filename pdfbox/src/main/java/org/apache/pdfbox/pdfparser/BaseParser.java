...
public abstract class BaseParser {
  ...
  private COSBase parseCOSDictionaryValue() throws IOException {
    final long numOffset = seqSource.getPosition();
    ...
    final long genOffset = seqSource.getPosition();
    final COSBase generationNumber = parseDirObject();
    skipSpaces();
    readExpectedChar('R');
    if (!(value instanceof COSInteger)) {
      BaseParser.LOG.error("expected number, actual=" + value + " at offset " + numOffset);
      return COSNull.NULL;
    }
    if (!(generationNumber instanceof COSInteger)) {
      BaseParser.LOG.error("expected number, actual=" + value + " at offset " + genOffset);
      return COSNull.NULL;
    }
  ...
  private COSBase getObjectFromPool(final COSObjectKey key) throws IOException {
    if (document == null)
      throw new IOException("object reference " + key + " at offset " + seqSource.getPosition() + " in content stream");
  ...
  protected COSDictionary parseCOSDictionary() throws IOException {
    ...
    while (!done) {
      skipSpaces();
      final char c = (char) seqSource.peek();
      if (c == '>') {
        done = true;
      } else if (c == '/') {
        parseCOSDictionaryNameValuePair(obj);
      } else {
        // invalid dictionary, we were expecting a /Name, read until the end or until we
        // can recover
        BaseParser.LOG
            .warn("Invalid dictionary, found: '" + c + "' but expected: '/' at offset " + seqSource.getPosition());
  ...
  /**
   * Keep reading until the end of the dictionary object or the file has been hit,
   * or until a '/' has been found.
   *
   * @return true if the end of the object or the file has been found, false if
   *         not, i.e. that the caller can continue to parse the dictionary at the
   *         current position.
   *
    ...
    while (c != -1 && c != '/' && c != '>') {
      ...
      if (c == BaseParser.E) {
        ...
        if (c == BaseParser.N) {
          ...
          if (c == BaseParser.D) {
            c = seqSource.read();
            final boolean isStream = c == BaseParser.S && seqSource.read() == BaseParser.T
                && seqSource.read() == BaseParser.R && seqSource.read() == BaseParser.E
                && seqSource.read() == BaseParser.A && seqSource.read() == BaseParser.M;
            final boolean isObj = !isStream && c == BaseParser.O && seqSource.read() == BaseParser.B
                && seqSource.read() == BaseParser.J;
            if (isStream || isObj)
  ...
  private void parseCOSDictionaryNameValuePair(final COSDictionary obj) throws IOException {
    ...
    if ((char) seqSource.peek() == 'd') {
      ...
      if (!potentialDEF.equals(BaseParser.DEF)) {
    ...
    if (value == null) {
      BaseParser.LOG.warn("Bad dictionary declaration at offset " + seqSource.getPosition());
    } else {
  ...
  protected void skipWhiteSpaces() throws IOException {
    ...
    while (BaseParser.ASCII_SPACE == whitespace) {
      whitespace = seqSource.read();
    }

    if (BaseParser.ASCII_CR == whitespace) {
      whitespace = seqSource.read();
      if (BaseParser.ASCII_LF != whitespace) {
        seqSource.unread(whitespace);
        // The spec says this is invalid but it happens in the real
        // world so we must support it.
      }
    } else if (BaseParser.ASCII_LF != whitespace) {
      // we are in an error.
  ...
  /**
   * This is really a bug in the Document creators code, but it caused a crash in
   * PDFBox, the first bug was in this format: /Title ( (5) /Creator which was
   * patched in 1 place.
   *
   * However it missed the case where the number of opening and closing
   * parenthesis isn't balanced
   *
   * The second bug was in this format /Title (c:\) /Producer
   *
   * This patch moves this code out of the parseCOSString method, so it can be
   * used twice.
   *
  ...
  private int checkForEndOfString(final int bracesParameter) throws IOException {
    ...
    final int amountRead = seqSource.read(nextThreeBytes);
    ...
    if (amountRead == 3 && nextThreeBytes[0] == BaseParser.ASCII_CR) {
      if (nextThreeBytes[1] == BaseParser.ASCII_LF && nextThreeBytes[2] == '/' || nextThreeBytes[2] == '>'
  ...
  protected COSString parseCOSString() throws IOException {
    final char nextChar = (char) seqSource.read();
    if (nextChar == '<')
      return parseCOSHexString();
    else if (nextChar != '(')
      throw new IOException("parseCOSString string should start with '(' or '<' and not '" + nextChar + "' at offset "
    ...
    while (braces > 0 && c != -1) {
      ...
      if (ch == ')') {
        ...
        // patched by ram
        final char next = (char) seqSource.read();
        switch (next) {
        case 'n':
          out.write('\n');
          break;
  ...
  /**
   * This will parse a PDF HEX string with fail fast semantic meaning that we stop
   * if a not allowed character is found. This is necessary in order to detect
   * malformed input and be able to skip to next object start.
   *
  ...
  private COSString parseCOSHexString() throws IOException {
    ...
    while (true) {
      int c = seqSource.read();
      if (BaseParser.isHexDigit((char) c)) {
        sBuf.append((char) c);
      } else if (c == '>') {
        break;
      } else if (c < 0)
  ...
  protected COSArray parseCOSArray() throws IOException {
    final long startPosition = seqSource.getPosition();
    readExpectedChar('[');
    ...
    while ((i = seqSource.peek()) > 0 && (char) i != ']') {
      ...
      if (pbo != null) {
        ...
        BaseParser.LOG
            .warn("Corrupt object reference at offset " + seqSource.getPosition() + ", start offset: " + startPosition);

        // This could also be an "endobj" or "endstream" which means we can assume that
        // the array has ended.
        final String isThisTheEnd = readString();
        seqSource.unread(isThisTheEnd.getBytes(Charsets.ISO_8859_1));
        if (BaseParser.ENDOBJ_STRING.equals(isThisTheEnd) || BaseParser.ENDSTREAM_STRING.equals(isThisTheEnd))
          return po;
      }
      skipSpaces();
    }
  ...
  protected boolean isEndOfName(final int ch) {
    return ch == BaseParser.ASCII_SPACE || ch == BaseParser.ASCII_CR || ch == BaseParser.ASCII_LF || ch == 9
  ...
  protected COSName parseCOSName() throws IOException {
    ...
    while (c != -1) {
      final int ch = c;
      if (ch == '#') {
        final int ch1 = seqSource.read();
        final int ch2 = seqSource.read();
        ...
        if (BaseParser.isHexDigit((char) ch1) && BaseParser.isHexDigit((char) ch2)) {
          final String hex = "" + (char) ch1 + (char) ch2;
          try {
          ...
          if (ch2 == -1 || ch1 == -1) {
            BaseParser.LOG.error("Premature EOF in BaseParser#parseCOSName");
            c = -1;
            break;
          }
  ...
  private boolean isValidUTF8(final byte[] input) {
    try {
      ...
      BaseParser.LOG.debug("Character could not be decoded using Charsets.UTF_8 - returning false", e);
      return false;
    }
  }
  ...
  protected COSBoolean parseBoolean() throws IOException {
    ...
    final char c = (char) seqSource.peek();
    if (c == 't') {
      final String trueString = new String(seqSource.readFully(4), Charsets.ISO_8859_1);
      if (!trueString.equals(BaseParser.TRUE))
        throw new IOException(
            "Error parsing boolean: expected='true' actual='" + trueString + "' at offset " + seqSource.getPosition());
      else {
        retval = COSBoolean.TRUE;
      }
    } else if (c == 'f') {
      final String falseString = new String(seqSource.readFully(5), Charsets.ISO_8859_1);
      if (!falseString.equals(BaseParser.FALSE))
        throw new IOException(
            "Error parsing boolean: expected='true' actual='" + falseString + "' at offset " + seqSource.getPosition());
      else {
  ...
  protected COSBase parseDirObject() throws IOException {
    ...
    skipSpaces();
    final int nextByte = seqSource.peek();
    char c = (char) nextByte;
    switch (c) {
    case '<': {
      // pull off first left bracket
      final int leftBracket = seqSource.read();
    ...
    case 'n': {
      // null
      readExpectedString(BaseParser.NULL);
      retval = COSNull.NULL;
      break;
    }
    case 't': {
      ...
      if (trueString.equals(BaseParser.TRUE)) {
        ...
        throw new IOException(
            "expected true actual='" + trueString + "' " + seqSource + "' at offset " + seqSource.getPosition());
      break;
    }
    case 'f': {
      final String falseString = new String(seqSource.readFully(5), Charsets.ISO_8859_1);
      if (falseString.equals(BaseParser.FALSE)) {
        ...
        throw new IOException(
            "expected false actual='" + falseString + "' " + seqSource + "' at offset " + seqSource.getPosition());
      break;
    }
    case 'R':
      seqSource.read();
    ...
    default: {
      if (Character.isDigit(c) || c == '-' || c == '+' || c == '.') {
        ...
        if (badString.isEmpty()) {
          final int peek = seqSource.peek();
          // we can end up in an infinite loop otherwise
          throw new IOException("Unknown dir object c='" + c + "' cInt=" + (int) c + " peek='" + (char) peek
              + "' peekInt=" + peek + " at offset " + seqSource.getPosition());
        }

        // if it's an endstream/endobj, we want to put it back so the caller will see it
        if (BaseParser.ENDOBJ_STRING.equals(badString) || BaseParser.ENDSTREAM_STRING.equals(badString)) {
   ...
   * @throws IOException if the String char is not the expected value or if an I/O
   *                     error occurs.
   */
  ...
  /**
   * Reads given pattern from {@link #seqSource}. Skipping whitespace at start and
   * end if wanted.
   *
   * @param expectedString pattern to be skipped
   * @param skipSpaces     if set to true spaces before and after the string will
   *                       be skipped
  ...
  protected final void readExpectedString(final char[] expectedString, final boolean skipSpaces) throws IOException {
    ...
    for (final char c : expectedString) {
      if (seqSource.read() != c)
        throw new IOException("Expected string '" + new String(expectedString) + "' but missed at character '" + c
   ...
   * @throws IOException if the read char is not the expected value or if an I/O
   *                     error occurs.
   */
  protected void readExpectedChar(final char ec) throws IOException {
    final char c = (char) seqSource.read();
    if (c != ec)
  ...
  protected String readString(final int length) throws IOException {
    ...
    while (!isWhitespace(c) && !isClosing(c) && c != -1 && buffer.length() < length && c != '[' && c != '<' && c != '('
        && c != '/') {
      buffer.append((char) c);
      c = seqSource.read();
    }
  ...
  /**
   * This will read bytes until the first end of line marker occurs. NOTE: The EOL
   * marker may consists of 1 (CR or LF) or 2 (CR and CL) bytes which is an
   * important detail if one wants to unread the line.
   *
  ...
  private boolean isLF(final int c) {
    return BaseParser.ASCII_LF == c;
  }

  private boolean isCR(final int c) {
    return BaseParser.ASCII_CR == c;
  }
  ...
  protected boolean isWhitespace(final int c) {
    return c == 0 || c == 9 || c == 12 || c == BaseParser.ASCII_LF || c == BaseParser.ASCII_CR
        || c == BaseParser.ASCII_SPACE;
  }
  ...
  protected boolean isSpace(final int c) {
    return BaseParser.ASCII_SPACE == c;
  }
  ...
  protected boolean isDigit() throws IOException {
    return BaseParser.isDigit(seqSource.peek());
  }
  ...
  protected static boolean isDigit(final int c) {
    return c >= BaseParser.ASCII_ZERO && c <= BaseParser.ASCII_NINE;
  }
  ...
  /**
   * This will read a long from the Stream and throw an {@link IOException} if the
   * long value is negative or has more than 10 digits (i.e. : bigger than
  ...
  protected long readObjectNumber() throws IOException {
    ...
    if (retval < 0 || retval >= BaseParser.OBJECT_NUMBER_THRESHOLD)
   ...
   * {@link IllegalArgumentException} if the integer value has more than the
   * maximum object revision (i.e. : bigger than
  ...
  protected int readGenerationNumber() throws IOException {
    ...
    if (retval < 0 || retval > BaseParser.GENERATION_NUMBER_THRESHOLD)
  ...
  protected int readInt() throws IOException {
    ...
    try {
      ...
      throw new IOException(
          "Error: Expected an integer type at offset " + seqSource.getPosition() + ", instead got '" + intBuffer + "'",
          e);
    }
    return retval;
  }
  ...
  /**
   * This method is used to read a token by the {@linkplain #readInt()} method and
   * the {@linkplain #readLong()} method.
   *
  ...
  protected final StringBuilder readStringNumber() throws IOException {
    int lastByte = 0;
    final StringBuilder buffer = new StringBuilder();
    while ((lastByte = seqSource.read()) != BaseParser.ASCII_SPACE && lastByte != BaseParser.ASCII_LF
        && lastByte != BaseParser.ASCII_CR && lastByte != 60 && // see sourceforge bug 1714707
      ...
      if (buffer.length() > BaseParser.MAX_LENGTH_LONG)
        throw new IOException(
            "Number '" + buffer + "' is getting too long, stop reading at offset " + seqSource.getPosition());
    }
    if (lastByte != -1) {
