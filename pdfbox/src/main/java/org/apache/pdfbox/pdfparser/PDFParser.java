...
public class PDFParser extends COSParser {
   ...
   * @throws IOException If something went wrong.
   */
  public PDFParser(final RandomAccessRead source, final ScratchFile scratchFile) throws IOException {
   ...
   * @throws IOException If something went wrong.
   */
  public PDFParser(final RandomAccessRead source, final String decryptionPassword, final ScratchFile scratchFile)
      throws IOException {
   ...
   * @param source             input representing the pdf.
   * @param decryptionPassword password to be used for decryption.
   * @param keyStore           key store to be used for decryption when using
   *                           public key security
   * @param alias              alias to be used for decryption when using public
   *                           key security
   *
   * @throws IOException If something went wrong.
   */
   ...
   *                           closed on {@link COSDocument#close()}
   *
   * @throws IOException If something went wrong.
   */
  public PDFParser(final RandomAccessRead source, final String decryptionPassword, final InputStream keyStore,
      final String alias, final ScratchFile scratchFile) throws IOException {
    super(source, decryptionPassword, keyStore, alias);
    fileLen = source.length();
    init(scratchFile);
  }

  private void init(final ScratchFile scratchFile) throws IOException {
    final String eofLookupRangeStr = System.getProperty(COSParser.SYSPROP_EOFLOOKUPRANGE);
    if (eofLookupRangeStr != null) {
      try {
        setEOFLookupRange(Integer.parseInt(eofLookupRangeStr));
      } catch (final NumberFormatException nfe) {
        PDFParser.LOG.warn("System property " + COSParser.SYSPROP_EOFLOOKUPRANGE
  ...
  /**
   * This will get the PD document that was parsed. When you are done with this
   * document you must call close() on it to release resources.
   *
  ...
  public PDDocument getPDDocument() throws IOException {
    final PDDocument doc = new PDDocument(getDocument(), source, getAccessPermission());
    doc.setEncryptionDictionary(getEncryption());
    return doc;
  }

  /**
   * The initial parse will first parse only the trailer, the xrefstart and all
   * xref tables to have a pointer (offset) to all the pdf's objects. It can
   * handle linearized pdfs, which will have an xref at the end pointing to an
   * xref at the beginning of the file. Last the root object is parsed.
   *
  ...
  protected void initialParse() throws InvalidPasswordException, IOException {
    ...
    final COSBase base = parseTrailerValuesDynamically(trailer);
    if (!(base instanceof COSDictionary))
      throw new IOException("Expected root dictionary, but got this: " + base);
    final COSDictionary root = (COSDictionary) base;
    ...
    final COSBase infoBase = trailer.getDictionaryObject(COSName.INFO);
  ...
  /**
   * This will parse the stream and populate the COSDocument object. This will
   * close the keystore stream when it is done parsing.
   *
   * @throws InvalidPasswordException If the password is incorrect.
   * @throws IOException              If there is an error reading from the stream
   *                                  or corrupt data is found.
   */
