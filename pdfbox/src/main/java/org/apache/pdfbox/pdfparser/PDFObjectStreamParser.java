...
/**
 * This will parse a PDF 1.5 object stream and extract all of the objects from
 * the stream.
 *
...
public class PDFObjectStreamParser extends BaseParser {
  ...
  /**
   * This will parse the tokens in the stream. This will close the stream when it
   * is finished parsing.
   *
  ...
  public void parse() throws IOException {
    try {
      ...
      final int numberOfObjects = stream.getInt("N");
      if (numberOfObjects == -1)
        throw new IOException("/N entry missing in object stream");
      final List<Long> objectNumbers = new ArrayList<>(numberOfObjects);
      ...
      while ((cosObject = parseDirObject()) != null) {
        ...
        if (objectCounter >= objectNumbers.size()) {
          PDFObjectStreamParser.LOG.error("/ObjStm (object stream) has more objects than /N " + numberOfObjects);
          break;
        }
        ...
        if (PDFObjectStreamParser.LOG.isDebugEnabled()) {
          PDFObjectStreamParser.LOG.debug("parsed=" + object);
        }
