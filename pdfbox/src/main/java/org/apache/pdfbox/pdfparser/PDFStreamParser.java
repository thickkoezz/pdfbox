...
public class PDFStreamParser extends BaseParser {
  ...
  private final byte[] binCharTestArr = new byte[PDFStreamParser.MAX_BIN_CHAR_TEST_LENGTH];
   ...
   */
  public PDFStreamParser(final InputStream stream) throws IOException {
  ...
  /**
   * This will parse all the tokens in the stream. This will close the stream when
   * it is finished parsing. You can then access these with {@link #getTokens()
   * getTokens()}.
   *
   ...
   *
   * @return The next token in the stream or null if there are no more tokens in
   *         the stream.
   *
  ...
  public Object parseNextToken() throws IOException {
    ...
    case '.': {
      /*
       * We will be filling buf with the rest of the number. Only allow 1 "." and "-"
       * and "+" at start of number.
       */
    ...
    case 'B': {
      ...
      if (next.equals(OperatorName.BEGIN_INLINE_IMAGE)) {
        final Operator beginImageOP = (Operator) retval;
        ...
        if (nextToken instanceof Operator) {
          ...
          if (imageData.getImageData() == null || imageData.getImageData().length == 0) {
            PDFStreamParser.LOG.warn("empty inline image at stream offset " + seqSource.getPosition());
          }
    ...
    case 'I': {
      ...
      final String id = "" + (char) seqSource.read() + (char) seqSource.read();
      if (!id.equals(OperatorName.BEGIN_INLINE_IMAGE_DATA))
        throw new IOException(
            "Error: Expected operator 'ID' actual='" + id + "' at stream offset " + seqSource.getPosition());
      ...
      while (!(lastByte == 'E' && currentByte == 'I' && hasNextSpaceOrReturn() && hasNoFollowingBinData(seqSource))
          && !seqSource.isEOF()) {
  ...
  /**
   * Looks up an amount of bytes if they contain only ASCII characters (no control
   * sequences etc.), and that these ASCII characters begin with a sequence of 1-3
   * non-blank characters between blanks
   *
   ...
   */
  private boolean hasNoFollowingBinData(final SequentialSource pdfSource) throws IOException {
    // as suggested in PDFBOX-1164
    final int readBytes = pdfSource.read(binCharTestArr, 0, PDFStreamParser.MAX_BIN_CHAR_TEST_LENGTH);
    ...
    if (readBytes > 0) {
      for (int bIdx = 0; bIdx < readBytes; bIdx++) {
        ...
        if (startOpIdx == -1 && !(b == 0 || b == 9 || b == 0x20 || b == 0x0a || b == 0x0d)) {
          startOpIdx = bIdx;
        } else if (startOpIdx != -1 && endOpIdx == -1 && (b == 0 || b == 9 || b == 0x20 || b == 0x0a || b == 0x0d)) {
          endOpIdx = bIdx;
        }
      }
      ...
      if (endOpIdx != -1 && startOpIdx != -1) {
        // usually, the operator here is Q, sometimes EMC (PDFBOX-2376), S
        // (PDFBOX-3784).
      ...
      if (readBytes == PDFStreamParser.MAX_BIN_CHAR_TEST_LENGTH) {
        // a PDF operator is 1-3 bytes long
        if (startOpIdx != -1 && endOpIdx == -1) {
          endOpIdx = PDFStreamParser.MAX_BIN_CHAR_TEST_LENGTH;
        }
    ...
    if (!noBinData) {
      PDFStreamParser.LOG.warn(
          "ignoring 'EI' assumed to be in the middle of inline image at stream offset " + pdfSource.getPosition());
    }
    return noBinData;
  }
  ...
  protected String readOperator() throws IOException {
    ...
    while (nextChar != -1 && // EOF
        !isWhitespace(nextChar) && !isClosing(nextChar) && nextChar != '[' && nextChar != '<' && nextChar != '('
        && nextChar != '/' && (nextChar < '0' || nextChar > '9')) {
      final char currentChar = (char) seqSource.read();
