...
public class Overlay implements Closeable {
   ...
   *
   * @param specificPageOverlayFile Optional map of overlay files for specific
   *                                pages. The page numbers are 1-based. The map
   *                                must be empty (but not null) if no specific
   *                                mappings are used.
   *
   * @return The modified input PDF document, which has to be saved and closed by
   *         the caller. If the input document was passed by
   *         {@link #setInputPDF(PDDocument) setInputPDF(PDDocument)} then it is
   ...
   *
   * @param specificPageOverlayDocuments Optional map of overlay documents for
   *                                     specific pages. The page numbers are
   *                                     1-based. The map must be empty (but not
   *                                     null) if no specific mappings are used.
   *
   * @return The modified input PDF document, which has to be saved and closed by
   *         the caller. If the input document was passed by
   *         {@link #setInputPDF(PDDocument) setInputPDF(PDDocument)} then it is
  ...
  /**
   * Close all input documents which were used for the overlay and opened by this
   * class.
   *
   * @throws IOException if something went wrong
   */
  ...
  private Map<Integer, LayoutPage> getLayoutPages(final PDDocument doc) throws IOException {
    ...
    for (int i = 0; i < numberOfPages; i++) {
      ...
      layoutPages.put(i,
          new LayoutPage(page.getMediaBox(), createCombinedContentStream(contents), resources.getCOSObject()));
    }
    return layoutPages;
  }
  ...
  private void overlayPage(final PDPage page, final LayoutPage layoutPage, final COSArray array) throws IOException {
    PDResources resources = page.getResources();
    if (resources == null) {
      resources = new PDResources();
      page.setResources(resources);
    }
    final COSName xObjectId = createOverlayXObject(page, layoutPage, layoutPage.overlayContentStream);
  ...
  private COSName createOverlayXObject(final PDPage page, final LayoutPage layoutPage, final COSStream contentStream) {
  ...
  private COSStream createOverlayStream(final PDPage page, final LayoutPage layoutPage, final COSName xObjectId)
    ...
    final AffineTransform at = calculateAffineTransform(page, layoutPage.overlayMediaBox);
  ...
  /**
   * Calculate the transform to be used when positioning the overlay. The default
   * implementation centers on the destination. Override this method to do your
   * own, e.g. move to a corner, or rotate.
   *
   ...
   *
   * @param inputFile the file to be overlayed. The {@link PDDocument} object
   *                  gathered from opening this file will be returned by
   *                  {@link #overlay(java.util.Map) overlay(Map&lt;Integer,
   *                  String&gt;)}.
   */
   ...
   *
   * @param inputPDF the PDF to be overlayed. This will be the object that is
   *                 returned by {@link #overlay(java.util.Map)
   ...
   *
   * @param allPagesOverlayPDF the all pages overlay PDF. This should not be a
   *                           PDDocument that you created on the fly, it should
   *                           be saved first, if it contains any fonts that are
   *                           subset.
   */
