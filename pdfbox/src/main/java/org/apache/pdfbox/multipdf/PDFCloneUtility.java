...
class PDFCloneUtility {
  ...
  // but we need a really huge file to test this. A test with the file from
  // PDFBOX-4477
   ...
   * identified by the destination parameter.
   *
  ...
  void cloneMerge(final COSObjectable base, final COSObjectable target) throws IOException {
    ...
    if (retval != null)
      return;
    // we are done, it has already been converted. // ### Is that correct for
    // cloneMerge???
