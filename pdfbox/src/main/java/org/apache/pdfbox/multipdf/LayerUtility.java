...
/**
 * This class allows to import pages as Form XObjects into a document and use
 * them to create layers (optional content groups). It should used only on
 * loaded documents, not on generated documents because these can contain
 * unfinished parts, e.g. font subsetting information.
 */
public class LayerUtility {
  ...
  public LayerUtility(final PDDocument document) {
    targetDoc = document;
    cloner = new PDFCloneUtility(document);
  }
   ...
   * content is appended. This method lets you add a q/Q pair around the existing
   * page's content.
   *
   * @param page the page
  ...
  /**
   * Imports a page from some PDF file as a Form XObject so it can be placed on
   * another page in the target document.
   * <p>
   * You may want to call {@link #wrapInSaveRestore(PDPage)
   * wrapInSaveRestore(PDPage)} before invoking the Form XObject to make sure that
   * the graphics state is reset.
   *
  ...
  public PDFormXObject importPageAsForm(final PDDocument sourceDoc, final PDPage page) throws IOException {
    ...
    transferDict(page.getCOSObject(), form.getCOSObject(), LayerUtility.PAGE_TO_FORM_FILTER, true);
  ...
  /**
   * Places the given form over the existing content of the indicated page (like
   * an overlay). The form is enveloped in a marked content section to indicate
   * that it's part of an optional content group (OCG), here used as a layer. This
   * optional group is returned and can be enabled and disabled through methods on
   * {@link PDOptionalContentProperties}.
   * <p>
   * You may want to call {@link #wrapInSaveRestore(PDPage)
   * wrapInSaveRestore(PDPage)} before calling this method to make sure that the
   * graphics state is reset.
   *
   * @param targetPage the target page
   * @param form       the form to place
   * @param transform  the transformation matrix that controls the placement of
   *                   your form. You'll need this if your page has a crop box
   *                   different than the media box, or if these have negative
   ...
   * @throws IOException if an I/O error occurs
   */
  public PDOptionalContentGroup appendFormAsLayer(final PDPage targetPage, final PDFormXObject form,
      final AffineTransform transform, final String layerName) throws IOException {
    ...
    if ((cropBox.getLowerLeftX() < 0 || cropBox.getLowerLeftY() < 0) && transform.isIdentity()) {
      // PDFBOX-4044
      LayerUtility.LOG.warn("Negative cropBox " + cropBox + " and identity transform may make your form invisible");
    }

    final PDOptionalContentGroup layer = new PDOptionalContentGroup(layerName);
    ocprops.addGroup(layer);

    try (PDPageContentStream contentStream = new PDPageContentStream(targetDoc, targetPage, AppendMode.APPEND,
        !LayerUtility.DEBUG)) {
  ...
  private void transferDict(final COSDictionary orgDict, final COSDictionary targetDict, final Set<String> filter,
  ...
  /**
   * Imports OCProperties from source document to target document so hidden layers
   * can still be hidden after import.
   *
   * @param sourceDoc The source PDF document that contains the /OCProperties to
   *                  be copied.
   * @throws IOException If an I/O error occurs.
   */
  private void importOcProperties(final PDDocument srcDoc) throws IOException {
    ...
    if (dstOCProperties == null) {
      dstCatalog.setOCProperties(
          new PDOptionalContentProperties((COSDictionary) cloner.cloneForNewDocument(srcOCProperties)));
    } else {
