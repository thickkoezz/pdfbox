...
/**
 * A string object, which may be a text string, a PDFDocEncoded string, ASCII
 * string, or byte string.
 *
 * <p>
 * Text strings are used for character strings that contain information intended
 * to be human-readable, such as text annotations, bookmark names, article
 * names, document information, and so forth.
 *
 * <p>
 * PDFDocEncoded strings are used for characters that are represented in a
 * single byte.
 *
 * <p>
...
public final class COSString extends COSBase {
  ...
  public static COSString parseHex(final String hex) throws IOException {
    ...
    for (int i = 0; i < length; i += 2) {
      try {
        ...
        if (COSString.FORCE_PARSING) {
          COSString.LOG.warn("Encountered a malformed hex string");
  ...
  /**
   * Creates a new PDF string from a byte array. This method can be used to read a
   * string from an existing PDF file, or to create a new byte string.
   *
  ...
  public COSString(final String text) {
    ...
    if (isOnlyPDFDocEncoding) {
      ...
      final byte[] data = text.getBytes(Charsets.UTF_16BE);
  ...
  /**
   * Sets whether or not to force the string is to be written in hex form. This is
  ...
  /**
   * This will take this string and create a hex representation of the bytes that
   * make the string.
   *
