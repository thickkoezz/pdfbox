...
public class COSStream extends COSDictionary implements Closeable {
   ...
   * <p>
   * Try to avoid using this constructor because it creates a new scratch file in
   * memory. Instead, use {@link COSDocument#createCOSStream()
   * document.getDocument().createCOSStream()} which will use the existing scratch
   * file (in memory or in temp file) of the document.
   * </p>
   */
  public COSStream() {
  ...
  /**
   * Creates a new stream with an empty dictionary. Data is stored in the given
   * scratch file.
   *
   ...
   * catching cases where a user tries to use a COSStream which has outlived its
   * COSDocument.
   */
  ...
  /**
   * Ensures {@link #randomAccess} is not <code>null</code> by creating a buffer
   * from {@link #scratchFile} if needed.
   *
   * @param forInputStream if <code>true</code> and {@link #randomAccess} is
   *                       <code>null</code> a debug message is logged - input
   *                       stream should be retrieved after data being written to
   *                       stream
   * @throws IOException
   */
  private void ensureRandomAccessExists(final boolean forInputStream) throws IOException {
    if (randomAccess == null) {
      if (forInputStream && COSStream.LOG.isDebugEnabled()) {
        // no data written to stream - maybe this should be an exception
        COSStream.LOG.debug("Create InputStream called without data being written before to stream.");
      }
  ...
  /**
   * Returns a new InputStream which reads the encoded PDF stream data. Experts
   * only!
   *
  ...
  public COSInputStream createInputStream(final DecodeOptions options) throws IOException {
    ...
    if (isWriting)
      throw new IllegalStateException("Cannot read while there is an open stream writer");
    ensureRandomAccessExists(true);
    final InputStream input = new RandomAccessInputStream(randomAccess);
  ...
  /**
   * Returns a new OutputStream for writing stream data, using the current
   * filters.
   *
  ...
  public OutputStream createOutputStream(final COSBase filters) throws IOException {
    ...
    final OutputStream cosOut = new COSOutputStream(getFilterList(), this, randomOut, scratchFile);
    isWriting = true;
   ...
   * You probably don't want this but want to use the createUnfilteredStream,
   * which is used to write raw bytes to.
   *
  ...
  public long getLength() {
    if (isWriting)
      throw new IllegalStateException("There is an open OutputStream associated with "
          + "this COSStream. It must be closed before querying" + "length of this COSStream.");
    return getInt(COSName.LENGTH, 0);
  }

  /**
   * This will return the filters to apply to the byte stream. The method will
   * return - null if no filters are to be applied - a COSName if one filter is to
   * be applied - a COSArray containing COSNames if multiple filters are to be
   * applied
   *
  ...
  public String toTextString() {
    ...
    try {
      ...
      COSStream.LOG.debug("An exception occured trying to get the content - returning empty string instead", e);
      return "";
    } finally {
