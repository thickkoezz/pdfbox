...
public final class COSName extends COSBase implements Comparable<COSName> {
  ...
  /**
   * Private constructor. This will limit the number of COSName objects. that are
   * created.
   *
  ...
  private COSName(final String aName, final boolean staticValue) {
    name = aName;
    if (staticValue) {
      COSName.commonNameMap.put(aName, this);
    } else {
      COSName.nameMap.put(aName, this);
    }
  ...
  /**
   * Not usually needed except if resources need to be reclaimed in a long running
   * process.
   */
  public static synchronized void clearResources() {
    // Clear them all
    COSName.nameMap.clear();
  }
}
